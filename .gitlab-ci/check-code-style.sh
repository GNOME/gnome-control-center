...
LINEUP_PARAMETERS="$SOURCE_ROOT/.gitlab-ci/lineup-parameters"
...
pushd $SOURCE_ROOT
for DIR in global-shortcuts-provider panels search-provider shell tests
do
   # Aligning prototypes is not working yet, so avoid headers
   for FILE in $(find "$DIR" -name "*.c" ! -path "*/contrib/*")
    do
        "$UNCRUSTIFY" -q -c ".gitlab-ci/uncrustify.cfg" -f "$FILE" -o "$FILE.uncrustify"
