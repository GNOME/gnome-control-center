...
static void update_state (CcLockButton *button);
...
G_DEFINE_TYPE (CcLockButton, cc_lock_button, GTK_TYPE_BIN);

static void
cc_lock_button_finalize (GObject *object)
{
  CcLockButton *button = CC_LOCK_BUTTON (object);
  CcLockButtonPrivate *priv = button->priv;
  ...
  G_OBJECT_CLASS (cc_lock_button_parent_class)->finalize (object);
}

static void
cc_lock_button_get_property (GObject    *object,
                             guint       property_id,
                             GValue     *value,
                             GParamSpec *pspec)
{
...
static void
cc_lock_button_set_property (GObject      *object,
                             guint         property_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
cc_lock_button_init (CcLockButton *button)
{
...
static void
cc_lock_button_constructed (GObject *object)
{
  CcLockButton *button = CC_LOCK_BUTTON (object);
  CcLockButtonPrivate *priv = button->priv;
  ...
  if (G_OBJECT_CLASS (cc_lock_button_parent_class)->constructed != NULL)
    G_OBJECT_CLASS (cc_lock_button_parent_class)->constructed (object);
}

static void
cc_lock_button_get_preferred_width (GtkWidget *widget,
                                    gint      *minimum,
                                    gint      *natural)
{
  CcLockButtonPrivate *priv = CC_LOCK_BUTTON (widget)->priv;

  gtk_widget_get_preferred_width (priv->box, minimum, natural);
}

static void
cc_lock_button_get_preferred_height (GtkWidget *widget,
                                     gint      *minimum,
                                     gint      *natural)
{
  CcLockButtonPrivate *priv = CC_LOCK_BUTTON (widget)->priv;
...
static void
cc_lock_button_size_allocate (GtkWidget     *widget,
...
static void
cc_lock_button_class_init (CcLockButtonClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  gobject_class->finalize     = cc_lock_button_finalize;
  gobject_class->get_property = cc_lock_button_get_property;
  gobject_class->set_property = cc_lock_button_set_property;
  gobject_class->constructed  = cc_lock_button_constructed;

  widget_class->get_preferred_width = cc_lock_button_get_preferred_width;
  widget_class->get_preferred_height = cc_lock_button_get_preferred_height;
  widget_class->size_allocate = cc_lock_button_size_allocate;

  g_type_class_add_private (klass, sizeof (CcLockButtonPrivate));
 ...
 *
 * Returns: a new #CcLockButton
 *
 * Since: 3.0
 */
GtkWidget *
cc_lock_button_new (GPermission *permission)
{
...
static void
update_state (CcLockButton *button)
{
  CcLockButtonPrivate *priv = button->priv;
  gint page;
  const gchar *tooltip;
...
on_permission_changed (GPermission *permission,
                       ...
                       gpointer     user_data)
{
  CcLockButton *button = CC_LOCK_BUTTON (user_data);
...
static void
handle_click (CcLockButton *button)
{
  CcLockButtonPrivate *priv = button->priv;
...
{
  handle_click (CC_LOCK_BUTTON (user_data));
}
...
/**
 * cc_lock_button_get_permission:
 * @button: a #CcLockButton
 *
 ...
 */
GPermission *
cc_lock_button_get_permission (CcLockButton *button)
{
