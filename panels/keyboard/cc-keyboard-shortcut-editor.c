...
static void
set_shortcut_editor_page (CcKeyboardShortcutEditor *self, ShortcutEditorPage page)
{
    switch (page) {
    case PAGE_CUSTOM:
...
cc_keyboard_shortcut_get_info_action_label (CcKeyboardShortcutEditor *self)
{
    ...
    const char *info_action_complete =
        _("Press <b>Esc</b> to cancel. Press <b>Backspace</b> to disable the keyboard shortcut for this action.");
...
static void
apply_custom_item_fields (CcKeyboardShortcutEditor *self, CcKeyboardItem *item)
{
    ...
    if (cc_keyboard_item_get_item_type (item) == CC_KEYBOARD_ITEM_TYPE_GSETTINGS_PATH) {
        g_settings_set_string (cc_keyboard_item_get_settings (item), "name",
                               gtk_editable_get_text (GTK_EDITABLE (self->name_entry)));
        g_settings_set_string (cc_keyboard_item_get_settings (item), "command",
...
static void
set_header_mode (CcKeyboardShortcutEditor *self, HeaderMode mode)
{
...
setup_custom_shortcut (CcKeyboardShortcutEditor *self)
{
    ...
    accel_valid =
        is_valid_binding (self->custom_combo) && is_valid_accel (self->custom_combo) && !self->custom_is_modifier;
    ...
    if (is_custom) {
        ...
        valid = accel_valid && g_utf8_strlen (gtk_editable_get_text (GTK_EDITABLE (self->name_entry)), -1) > 0
    ...
    collision_item = cc_keyboard_manager_get_collision (self->manager, self->item, self->custom_combo);
    self->edited = TRUE;
...
static void
cc_keyboard_shortcut_editor_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_keyboard_shortcut_editor_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcKeyboardShortcutEditor *self = CC_KEYBOARD_SHORTCUT_EDITOR (object);
...
static gboolean
on_key_pressed_cb (CcKeyboardShortcutEditor *self, guint keyval, guint keycode, GdkModifierType state,
                   GtkEventControllerKey *key_controller)
{
    ...
    normalize_keyval_and_mask (keycode, state, gtk_event_controller_key_get_group (key_controller), &keyval_lower,
                               &real_mask);
...
cc_keyboard_shortcut_editor_class_init (CcKeyboardShortcutEditorClass *klass)
{
     ...
     */
    properties[PROP_KEYBOARD_ITEM] = g_param_spec_object (
        "keyboard-item", "Keyboard item", "The keyboard item being edited", CC_TYPE_KEYBOARD_ITEM, G_PARAM_READWRITE);
    ...
    properties[PROP_MANAGER] =
        g_param_spec_object ("manager", "Keyboard manager", "The keyboard manager", CC_TYPE_KEYBOARD_MANAGER,
                             G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

    g_object_class_install_properties (object_class, N_PROPS, properties);

    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/keyboard/cc-keyboard-shortcut-editor.ui");
...
{
    return g_object_new (CC_TYPE_KEYBOARD_SHORTCUT_EDITOR, "manager", manager, NULL);
}
 ...
 */
void
cc_keyboard_shortcut_editor_set_item (CcKeyboardShortcutEditor *self, CcKeyboardItem *item)
{
...
void
cc_keyboard_shortcut_editor_set_mode (CcKeyboardShortcutEditor *self, CcShortcutEditorMode mode)
{
    g_return_if_fail (CC_IS_KEYBOARD_SHORTCUT_EDITOR (self));
    ...
    if (mode == CC_SHORTCUT_EDITOR_CREATE) {
        ...
        /* Headerbar */
        adw_dialog_set_title (ADW_DIALOG (self), is_custom ? _("Custom Shortcut") : _("Edit Shortcut"));
