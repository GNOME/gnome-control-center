...
static void cc_bar_chart_group_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec);
static void cc_bar_chart_group_set_property (GObject *object, guint property_id, const GValue *value,
                                             GParamSpec *pspec);
static void cc_bar_chart_group_dispose (GObject *object);
static void cc_bar_chart_group_size_allocate (GtkWidget *widget, int width, int height, int baseline);
static void cc_bar_chart_group_measure (GtkWidget *widget, GtkOrientation orientation, int for_size, int *minimum,
                                        int *natural, int *minimum_baseline, int *natural_baseline);
static gboolean cc_bar_chart_group_focus (GtkWidget *widget, GtkDirectionType direction);
static void gesture_click_pressed_cb (GtkGestureClick *gesture, guint n_press, double x, double y, gpointer user_data);

static gboolean find_index_for_bar (CcBarChartGroup *self, CcBarChartBar *bar, unsigned int *out_idx);
static gboolean bar_is_focusable (CcBarChartBar *bar);
static CcBarChartBar *get_adjacent_focusable_bar (CcBarChartGroup *self, CcBarChartBar *bar, int direction);
...
cc_bar_chart_group_class_init (CcBarChartGroupClass *klass)
{
     ...
     * instead.
     */
    props[PROP_SELECTABLE] = g_param_spec_boolean (
        "selectable", NULL, NULL, TRUE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
     ...
     */
    props[PROP_IS_SELECTED] = g_param_spec_boolean (
        "is-selected", NULL, NULL, FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
     ...
     * #CcBarChart.
     */
    props[PROP_SCALE] = g_param_spec_double ("scale", NULL, NULL, -G_MAXDOUBLE, G_MAXDOUBLE, 1.0,
                                             G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);

    g_object_class_install_properties (object_class, G_N_ELEMENTS (props), props);

    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/wellbeing/cc-bar-chart-group.ui");
...
static void
cc_bar_chart_group_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_bar_chart_group_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    CcBarChartGroup *self = CC_BAR_CHART_GROUP (object);
...
static void
cc_bar_chart_group_size_allocate (GtkWidget *widget, int width, int height, int baseline)
{
...
static void
cc_bar_chart_group_measure (GtkWidget *widget, GtkOrientation orientation, int for_size, int *minimum, int *natural,
                            int *minimum_baseline, int *natural_baseline)
{
    ...
    if (orientation == GTK_ORIENTATION_HORIZONTAL) {
        ...
        for (unsigned int i = 0; i < self->bars->len; i++) {
            ...
            gtk_widget_measure (GTK_WIDGET (bar), orientation, -1, &bar_minimum_width, &bar_natural_width, NULL, NULL);
        ...
        *natural_baseline = -1;
        ...
        for (unsigned int i = 0; i < self->bars->len; i++) {
            ...
            gtk_widget_measure (GTK_WIDGET (bar), orientation, -1, &bar_minimum_height, &bar_natural_height, NULL,
                                NULL);
...
static gboolean
cc_bar_chart_group_focus (GtkWidget *widget, GtkDirectionType direction)
{
    CcBarChartGroup *self = CC_BAR_CHART_GROUP (widget);
    ...
    if (focus_child != NULL) {
        ...
        if (CC_IS_BAR_CHART_BAR (focus_child) && (direction == GTK_DIR_LEFT || direction == GTK_DIR_TAB_BACKWARD))
            next_focus_bar = get_adjacent_focusable_bar (self, CC_BAR_CHART_BAR (focus_child), -1);
        else if (CC_IS_BAR_CHART_BAR (focus_child) && (direction == GTK_DIR_RIGHT || direction == GTK_DIR_TAB_FORWARD))
            next_focus_bar = get_adjacent_focusable_bar (self, CC_BAR_CHART_BAR (focus_child), 1);
    } else {
...
static void
gesture_click_pressed_cb (GtkGestureClick *gesture, guint n_press, double x, double y, gpointer user_data)
{
    ...
    if (!CC_IS_BAR_CHART_BAR (bar) || !find_index_for_bar (self, CC_BAR_CHART_BAR (bar), &bar_idx))
        bar = NULL;
...
static gboolean
find_index_for_bar (CcBarChartGroup *self, CcBarChartBar *bar, unsigned int *out_idx)
{
...
get_adjacent_focusable_bar (CcBarChartGroup *self, CcBarChartBar *bar, int direction)
{
    ...
    while (!((direction == -1 && i == 0) || (direction == 1 && i >= self->bars->len - 1))) {
 ...
 */
CcBarChartBar *const *
cc_bar_chart_group_get_bars (CcBarChartGroup *self, size_t *out_n_bars)
{
 ...
 */
void
cc_bar_chart_group_insert_bar (CcBarChartGroup *self, int idx, CcBarChartBar *bar)
{
 ...
 */
void
cc_bar_chart_group_remove_bar (CcBarChartGroup *self, CcBarChartBar *bar)
{
 ...
 */
void
cc_bar_chart_group_set_selectable (CcBarChartGroup *self, gboolean selectable)
{
    g_return_if_fail (CC_IS_BAR_CHART_GROUP (self));
...
static void
set_or_unset_selection_state_flags (CcBarChartGroup *self, gboolean set)
{
    ...
    if (selected_widget != NULL) {
        ...
        gtk_accessible_update_state (GTK_ACCESSIBLE (selected_widget), GTK_ACCESSIBLE_STATE_SELECTED, set, -1);
    }
}
 ...
 */
void
cc_bar_chart_group_set_is_selected (CcBarChartGroup *self, gboolean is_selected)
{
    g_return_if_fail (CC_IS_BAR_CHART_GROUP (self));
 ...
 */
gboolean
cc_bar_chart_group_get_selected_index (CcBarChartGroup *self, size_t *out_index)
{
    g_return_val_if_fail (CC_IS_BAR_CHART_GROUP (self), FALSE);
 ...
 */
void
cc_bar_chart_group_set_selected_index (CcBarChartGroup *self, gboolean is_selected, size_t idx)
{
    g_return_if_fail (CC_IS_BAR_CHART_GROUP (self));
 ...
 */
void
cc_bar_chart_group_set_scale (CcBarChartGroup *self, double scale)
{
    g_return_if_fail (CC_IS_BAR_CHART_GROUP (self));
