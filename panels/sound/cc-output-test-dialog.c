...
// FIXME: Support PA_CHANNEL_POSITION_MONO
...
cc_output_test_dialog_dispose (GObject *object)
{
  ...
  g_clear_pointer (&self->context, ca_context_destroy);
...
cc_output_test_dialog_init (CcOutputTestDialog *self)
{
  ...
  ca_context_create (&self->context);
  ca_context_set_driver (self->context, "pulse");
  ca_context_change_props (self->context,
  ...
  g_object_get (G_OBJECT (settings),
                ...
                NULL);
  if (theme_name != NULL)
    ca_context_change_props (self->context, CA_PROP_CANBERRA_XDG_THEME_NAME, theme_name, NULL);
