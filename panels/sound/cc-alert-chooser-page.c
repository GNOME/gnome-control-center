...
update_dir_mtime (const char *dir_path)
{
    ...
    if (!g_file_set_attribute_uint64 (dir, G_FILE_ATTRIBUTE_TIME_MODIFIED, g_date_time_to_unix (now),
                                      G_FILE_QUERY_INFO_NONE, NULL, &mtime_error)) {
        g_warning ("Failed to update directory modification time for %s: %s", dir_path, mtime_error->message);
    }
}
...
play_sound (CcAlertChooserPage *self, const gchar *name)
{
    ...
    if (!gsound_context_play_simple (self->context, NULL, &error, GSOUND_ATTR_MEDIA_FILENAME, path, NULL)) {
...
static void
set_button_active (CcAlertChooserPage *self, GtkCheckButton *button, gboolean active)
{
