...
get_alert_name (void)
{
  ...
  if (info == NULL)
  {
    if (!g_error_matches (error, G_FILE_ERROR, G_FILE_ERROR_NOENT))
      g_warning ("Failed to remove check sound theme symlink %s: %s", path, error->message);
    return NULL;
  }
                  ...
                  const gchar    *name)
{
  ...
  //FIXMEg_key_file_set_string (theme_file, "Sound Theme", "Inherits", parent);
