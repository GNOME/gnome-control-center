...
{
  gtk_image_set_from_icon_name (self->image, get_icon_name (self), GTK_ICON_SIZE_DIALOG);
}
...
play_sound (CcSpeakerTestButton *self)
{
  ...
  /* Play the per-channel sound name */
  ...
  /* Fall back to a generic test signal */
  ca_proplist_sets (proplist, CA_PROP_EVENT_ID, "audio-test-signal");
  error = ca_context_play_full (self->context, TEST_SOUND_ID, proplist, finish_cb, self);
  if (error == 0)
    return TRUE;

  /* Fall back to the system bell */
  ca_proplist_sets (proplist, CA_PROP_EVENT_ID, "bell");
  error = ca_context_play_full (self->context, TEST_SOUND_ID, proplist, finish_cb, self);
  if (error == 0)
    return TRUE;
