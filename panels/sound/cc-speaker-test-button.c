...
static gboolean play_sound (CcSpeakerTestButton *self);
...
static void
finish_cb (GObject *object, GAsyncResult *result, gpointer userdata)
{
...
play_sound (CcSpeakerTestButton *self)
{
    ...
    gsound_context_play_full (self->context, self->cancellable, finish_cb, self, GSOUND_ATTR_MEDIA_ROLE, "test",
                              ...
                              GSOUND_ATTR_CANBERRA_ENABLE, "1", GSOUND_ATTR_EVENT_ID, events[self->event_index], NULL);
    self->event_index++;
...
GtkWidget *
cc_speaker_test_button_new (GSoundContext *context, pa_channel_position_t position)
{
    ...
    gtk_widget_set_tooltip_text (GTK_WIDGET (self), pa_channel_position_to_pretty_string (position));

    return GTK_WIDGET (self);
}

void
cc_speaker_test_button_set_channel_position (CcSpeakerTestButton *self, pa_channel_position_t position)
{
