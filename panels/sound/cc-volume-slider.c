...
static void
update_mute_button_tooltip (CcVolumeSlider *self, gdouble volume)
{
    const gchar *tooltip;
...
update_ranges (CcVolumeSlider *self)
{
    ...
    if (self->is_amplified) {
        gtk_adjustment_set_upper (self->volume_adjustment,
                                  gvc_mixer_control_get_vol_max_amplified (self->mixer_control));
        gtk_scale_add_mark (self->volume_scale, vol_max_norm, GTK_POS_BOTTOM, C_("volume", "100%"));
    } else {
...
static void
mute_cb (GtkWidget *widget, const char *action_name, GVariant *parameter)
{
    ...
    if (!gvc_mixer_stream_get_is_muted (self->stream) && gvc_mixer_stream_get_volume (self->stream) == 0.0) {
...
void
cc_volume_slider_set_mixer_control (CcVolumeSlider *self, GvcMixerControl *mixer_control)
{
...
cc_volume_slider_set_stream (CcVolumeSlider *self, GvcMixerStream *stream, CcStreamType type)
{
    ...
    if (stream != NULL) {
        ...
        self->notify_volume_handler_id =
            g_signal_connect_object (stream, "notify::volume", G_CALLBACK (notify_volume_cb), self, G_CONNECT_SWAPPED);
        self->notify_is_muted_handler_id = g_signal_connect_object (
...
void
cc_volume_slider_set_is_amplified (CcVolumeSlider *self, gboolean is_amplified)
{
    g_return_if_fail (CC_IS_VOLUME_SLIDER (self));
