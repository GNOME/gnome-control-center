...
struct _CcVolumeSlider {
  ...
  guint            notify_volume_handler;
  guint            notify_is_muted_handler;
};

G_DEFINE_TYPE (CcVolumeSlider, cc_volume_slider, GTK_TYPE_BOX)

// FIXME: Support scrolling with mouse wheel
                             ...
                             GvcMixerStream *stream)
{
  ...
  if (self->stream != NULL) {
    g_signal_handler_disconnect (self->stream, self->notify_volume_handler);
    self->notify_volume_handler = 0;
    g_signal_handler_disconnect (self->stream, self->notify_is_muted_handler);
    self->notify_is_muted_handler = 0;
  }
  g_clear_object (&self->stream);
  ...
  if (stream != NULL) {
    ...
    self->notify_volume_handler = g_signal_connect_object (stream,
                                                           "notify::volume",
                                                           G_CALLBACK (notify_volume_cb),
                                                           self, G_CONNECT_SWAPPED);
    self->notify_is_muted_handler = g_signal_connect_object (stream,
                                                             "notify::is-muted",
