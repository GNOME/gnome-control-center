...
G_DEFINE_TYPE (CcPhotoDialog, cc_photo_dialog, GTK_TYPE_POPOVER)
...
static void
cc_photo_dialog_crop (CcPhotoDialog *self,
                      GdkPixbuf     *pixbuf)
{
        GtkWidget *dialog;
                       ...
                       CcPhotoDialog *self)
{
        ...
        cc_photo_dialog_crop (self, pixbuf2);
...
static void
cc_photo_dialog_select_file (CcPhotoDialog *self)
{
        GtkWidget *chooser;
        const gchar *folder;
        GtkWidget *preview;
...
static void
webcam_icon_selected (CcPhotoDialog *self)
{
        GtkWidget *window;
...
static void
update_photo_menu_status (CcPhotoDialog *self)
{
...
#ifdef HAVE_CHEESE
static void
setup_cheese_camera_device_monitor (CcPhotoDialog *self)
{
...
static void
setup_photo_popup (CcPhotoDialog *self)
{
        GFile *file, *dir;
        GFileInfo *info;
...
static void
popup_icon_menu (GtkToggleButton *button, CcPhotoDialog *self)
{
...
CcPhotoDialog *
cc_photo_dialog_new (GtkWidget *button)
{
        CcPhotoDialog *self;

        self = g_object_new (CC_TYPE_PHOTO_DIALOG,
                             "relative-to", button,
                             NULL);
...
static void
cc_photo_dialog_dispose (GObject *object)
{
        CcPhotoDialog *self = CC_PHOTO_DIALOG (object);
        ...
        G_OBJECT_CLASS (cc_photo_dialog_parent_class)->dispose (object);
}

static void
cc_photo_dialog_init (CcPhotoDialog *self)
{
        gtk_widget_init_template (GTK_WIDGET (self));
}

static void
cc_photo_dialog_class_init (CcPhotoDialogClass *klass)
{
        GtkWidgetClass *wclass = GTK_WIDGET_CLASS (klass);
        GObjectClass *oclass = G_OBJECT_CLASS (klass);

        gtk_widget_class_set_template_from_resource (wclass, "/org/gnome/control-center/user-accounts/cc-photo-dialog.ui");

        gtk_widget_class_bind_template_child (wclass, CcPhotoDialog, flowbox);
        gtk_widget_class_bind_template_child (wclass, CcPhotoDialog, take_picture_button);

        gtk_widget_class_bind_template_callback (wclass, cc_photo_dialog_select_file);
#ifdef HAVE_CHEESE
        gtk_widget_class_bind_template_callback (wclass, webcam_icon_selected);
#endif

        oclass->dispose = cc_photo_dialog_dispose;
}

void
cc_photo_dialog_set_user (CcPhotoDialog *self,
                          ActUser       *user)
{
