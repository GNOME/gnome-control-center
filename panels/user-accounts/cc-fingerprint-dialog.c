...
CcFingerprintDialog *
cc_fingerprint_dialog_new (GtkWindow            *window,
                           CcFingerprintManager *manager)
{
...
cc_fingerprint_dialog_dispose (GObject *object)
{
  ...
  g_clear_pointer (&self->views_history, g_list_free);
      ...
      if (!dbus_error || !g_str_has_suffix (dbus_error, ".Error.NoEnrolledPrints"))
        {
          ...
          error_message = g_strdup_printf (_("Impossible to list prints for device %s: %s"),
  ...
  if (error)
    {
      ...
      error_message = g_strdup_printf (_("Impossible to delete prints for device %s: %s"),
                                       cc_fprintd_device_get_name (self->device),
                                       error->message);
      g_warning ("%s", error_message);
      notify_error (self, error_message);
                           ...
                           const char          *style_class)
{
  ...
  g_return_if_fail (!style_class ||
                    !g_str_equal (style_class, "warning") ||
                    !g_str_equal (style_class, "error") ||
                    !g_str_equal (style_class, "success") ||
                    !g_str_equal (style_class, "completed"));

  icon_name = "fingerprint-detection-symbolic";
  style_context = gtk_widget_get_style_context (self->enroll_result_icon);

  gtk_style_context_remove_class (style_context, "warning");
  gtk_style_context_remove_class (style_context, "error");
  gtk_style_context_remove_class (style_context, "success");
  gtk_style_context_remove_class (style_context, "completed");

  /* Use enum for errors, and wiggle on warning! */

  if (style_class)
    {
      gtk_style_context_add_class (style_context, style_class);

      if (g_str_equal (style_class, "warning") ||
          g_str_equal (style_class, "error"))
        icon_name = "fingerprint-detection-warning-symbolic";
      else if (g_str_equal (style_class, "completed"))
...
stage_passed_timeout_cb (gpointer user_data)
{
  ...
  set_enroll_result_message (self, NULL, NULL);
  ...
  if (g_str_equal (result, "enroll-stage-passed") || completed)
    {
      ...
      if (!completed)
        {
          set_enroll_result_message (self, NULL, "success");
  ...
  else if (!done)
    {
      ...
      set_enroll_result_message (self, message, NULL);
    }
      ...
      if (completed)
        {
          ...
          set_enroll_result_message (self, _("Complete"), "completed");
      ...
      else
        {
          ...
          if (g_str_equal (result, "enroll-disconnected"))
            ...
            message = _("Problem Reading Finger");

          self->enrolling_finger = NULL;
          set_enroll_result_message (self, message, "warning");
        }
    }
}
  ...
  if (error)
    {
      ...
      self->enrolling_finger = NULL;
      error_message = g_strdup_printf (_("Impossible to start enrollment %s: %s"),
                                       cc_fprintd_device_get_name (self->device),
                                       error->message);
      g_warning ("%s", error_message);
      notify_error (self, error_message);
      set_enroll_result_message (self, _("Failure reading Finger"), "error");
  ...
  if (error)
    {
      ...
      error_message = g_strdup_printf (_("Impossible to stop enrollment %s: %s"),
                                       cc_fprintd_device_get_name (self->device),
                                       error->message);
      g_warning ("%s", error_message);
      notify_error (self, error_message);
                       ...
                       const char          *finger_id)
{
  ...
  scan_type = cc_fprintd_device_get_scan_type (self->device);
  is_swipe = g_str_equal (scan_type, "swipe");
  ret = TR (finger_str_to_msg (finger_id, NULL, is_swipe));
...
populate_prints_gallery (CcFingerprintDialog *self)
{
  ...
  add_print_label = _("Add a Finger");
  ...
  if (error)
    {
      ...
      error_message = g_strdup_printf (_("Impossible to release fingerprint device %s: %s"),
                                       cc_fprintd_device_get_name (self->device),
                                       error->message);
      g_warning ("%s", error_message);
      ...
      if (self->device_claimed)
        {
          ...
          if (self->enrolling_finger)
            {
              set_enroll_result_message (self, _("Problem Reading Finger"), "error");
              self->enrolling_finger = NULL;
            }
  ...
  if (error)
    {
      ...
      error_message = g_strdup_printf (_("Impossible to claim fingerprint device %s: %s"),
                                       cc_fprintd_device_get_name (self->device),
                                       error->message);
      g_warning ("%s", error_message);
      notify_error (self, error_message);
...
on_stack_child_changed (CcFingerprintDialog *self)
{
  ...
  GtkWidget *previous_view = self->views_history ? self->views_history->data : NULL;
  ...
  if (visible_child == self->prints_manager)
    {
      gtk_widget_set_visible (GTK_WIDGET (self->back_button),
                              previous_view == self->device_selector);
  ...
  self->views_history = g_list_prepend (self->views_history, visible_child);
}
...
{
  g_return_if_fail (g_list_length (self->views_history) > 1);

  notify_error (self, NULL);
  gtk_stack_set_visible_child (self->stack, g_list_nth_data (self->views_history, 1));
}
