                        ...
                        gpointer user_data)
{
  ...
  if (g_error_matches (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND))
    adw_action_row_set_subtitle (row, _("This folder no longer exists or is unmounted"));
...
create_row_for_place (CcSearchLocationsDialog *self, Place *place)
{
  ...
  if (g_str_equal (place->settings_key, TRACKER_KEY_SINGLE_DIRECTORIES))
    adw_action_row_set_subtitle (row, _("Setting for this folder does not apply to subfolders"));
