...
static gint
list_sort_func (gconstpointer a, gconstpointer b, gpointer user_data)
{
...
static gint
propagate_compare_func (gconstpointer a, gconstpointer b, gpointer user_data)
{
    CcSearchPanel *self = user_data;
...
search_panel_propagate_sort_order (CcSearchPanel *self)
{
    ...
    g_settings_set_strv (self->search_settings, "sort-order", (const gchar **) sort_order->pdata);
}

static void
search_panel_move_selected (CcSearchPanel *self, gboolean down)
{
...
static void
row_moved_cb (CcSearchPanel *self, CcSearchPanelRow *dest_row, CcSearchPanelRow *row)
{
...
static GVariant *
switch_settings_mapping_set_generic (const GValue *value, const GVariantType *expected_type, GtkWidget *row,
                                     gboolean default_enabled)
{
    ...
    apps = g_settings_get_strv (self->search_settings, default_enabled ? "disabled" : "enabled");
...
static GVariant *
switch_settings_mapping_set_default_enabled (const GValue *value, const GVariantType *expected_type, gpointer user_data)
{
    return switch_settings_mapping_set_generic (value, expected_type, user_data, TRUE);
}

static GVariant *
switch_settings_mapping_set_default_disabled (const GValue *value, const GVariantType *expected_type,
                                              gpointer user_data)
{
    return switch_settings_mapping_set_generic (value, expected_type, user_data, FALSE);
}

static gboolean
switch_settings_mapping_get_generic (GValue *value, GVariant *variant, GtkWidget *row, gboolean default_enabled)
{
    GAppInfo *app_info = cc_search_panel_row_get_app_info (CC_SEARCH_PANEL_ROW (row));
...
static gboolean
switch_settings_mapping_get_default_enabled (GValue *value, GVariant *variant, gpointer user_data)
{
    return switch_settings_mapping_get_generic (value, variant, user_data, TRUE);
}

static gboolean
switch_settings_mapping_get_default_disabled (GValue *value, GVariant *variant, gpointer user_data)
{
    return switch_settings_mapping_get_generic (value, variant, user_data, FALSE);
}
...
search_panel_update_enabled_move_actions (CcSearchPanel *self)
{
    ...
    for (child = gtk_widget_get_first_child (GTK_WIDGET (self->list_box)); child;
...
static void
search_panel_add_one_app_info (CcSearchPanel *self, GAppInfo *app_info, gboolean default_enabled)
{
    ...
    if (default_enabled) {
        g_settings_bind_with_mapping (self->search_settings, "disabled", cc_search_panel_row_get_switch (row), "active",
                                      G_SETTINGS_BIND_DEFAULT, switch_settings_mapping_get_default_enabled,
                                      switch_settings_mapping_set_default_enabled, row, NULL);
    } else {
        g_settings_bind_with_mapping (self->search_settings, "enabled", cc_search_panel_row_get_switch (row), "active",
                                      G_SETTINGS_BIND_DEFAULT, switch_settings_mapping_get_default_disabled,
                                      switch_settings_mapping_set_default_disabled, row, NULL);
    }
}

static void
search_panel_add_one_provider (CcSearchPanel *self, GFile *provider)
{
    ...
    if (error != NULL) {
        g_warning ("Error loading %s: %s - search provider will be ignored", path, error->message);
        return;
    }
    ...
    if (error != NULL) {
        g_warning ("Unable to read desktop ID from %s: %s - search provider will be ignored", path, error->message);
        return;
    }
    ...
    if (app_info == NULL) {
        g_debug ("Could not find application with desktop ID '%s' referenced in '%s', ignoring", desktop_id, path);
        return;
    }
...
static void
search_providers_discover_ready (GObject *source, GAsyncResult *result, gpointer user_data)
{
...
static GList *
search_providers_discover_one_directory (const gchar *system_dir, GCancellable *cancellable)
{
    ...
    enumerator = g_file_enumerate_children (providers_location, "standard::type,standard::name,standard::content-type",
                                            G_FILE_QUERY_INFO_NONE, cancellable, &error);

    if (error != NULL) {
        if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_NOT_FOUND)
            && !g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
            g_warning ("Error opening %s: %s - search provider configuration won't be possible", providers_path,
                       error->message);
...
static void
search_providers_discover_thread (GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
    GList *providers = NULL;
    ...
    for (idx = 0; system_data_dirs[idx] != NULL; idx++) {
        providers =
            g_list_concat (search_providers_discover_one_directory (system_data_dirs[idx], cancellable), providers);
...
populate_search_providers (CcSearchPanel *self)
{
    ...
    task = g_task_new (self, cc_panel_get_cancellable (CC_PANEL (self)), search_providers_discover_ready, self);
...
cc_search_panel_init (CcSearchPanel *self)
{
    ...
    gtk_list_box_set_sort_func (GTK_LIST_BOX (self->list_box), (GtkListBoxSortFunc) list_sort_func, self, NULL);
    ...
    g_settings_bind (self->search_settings, "disable-external", self->app_search_row, "active",
                     G_SETTINGS_BIND_DEFAULT | G_SETTINGS_BIND_INVERT_BOOLEAN);

    g_object_bind_property (self->app_search_row, "active", self->search_group, "sensitive",
                            G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);

    self->sort_order = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, NULL);
...
cc_search_panel_class_init (CcSearchPanelClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class, "/org/gnome/control-center/search/cc-search-panel.ui");
