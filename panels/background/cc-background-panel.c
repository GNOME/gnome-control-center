...
#define COLOR_SCHEME_PATH_ID "org.gnome.settings-daemon.plugins.color-scheme"
...
struct _CcBackgroundPanel
{
  ...
  GSettings *color_scheme_settings;

  GDBusProxy *proxy;
  GDBusProxy *color_scheme_proxy;
...
schedule_update_state (CcBackgroundPanel *self)
{
  ...
  enabled = g_settings_get_boolean (self->color_scheme_settings, "enabled");
  automatic = g_settings_get_boolean (self->color_scheme_settings, "schedule-automatic");

  adw_combo_row_set_selected (ADW_COMBO_ROW (self->schedule_row), enabled ? automatic ? 1 : 2 : 0);

  if (automatic && self->color_scheme_proxy != NULL)
    {
      gdouble sunset_value, sunrise_value;

      g_autoptr(GVariant) sunset = NULL;
      g_autoptr(GVariant) sunrise = NULL;
      sunset = g_dbus_proxy_get_cached_property (self->color_scheme_proxy, "Sunset");
      sunrise = g_dbus_proxy_get_cached_property (self->color_scheme_proxy, "Sunrise");
  ...
  g_settings_bind (self->color_scheme_settings, "schedule-automatic",
...
schedule_location_changed (CcBackgroundPanel *self)
{
  ...
  if (automatic && !location_enabled)
    g_settings_set_boolean (self->color_scheme_settings, "schedule-automatic", FALSE);
}
...
on_schedule_row_selected_cb (CcBackgroundPanel *self)
{
  ...
  g_settings_set_boolean (self->color_scheme_settings, "enabled", enabled);
  g_settings_set_boolean (self->color_scheme_settings, "schedule-automatic", automatic);
...
on_beginning_time_updated_cb (CcBackgroundPanel *self)
{
  ...
  g_settings_set_double (self->color_scheme_settings, "schedule-from", value);
}
...
on_end_time_updated_cb (CcBackgroundPanel *self)
{
  ...
  g_settings_set_double (self->color_scheme_settings, "schedule-to", value);
}

static void
color_scheme_changed_cb (CcBackgroundPanel *self)
{
  schedule_update_state (self);
}

static void
color_scheme_got_proxy_cb (GObject      *source_object,
                           GAsyncResult *res,
                           gpointer      user_data)
{
  ...
  self->color_scheme_proxy = proxy;

  g_signal_connect_object (G_OBJECT (self->color_scheme_proxy), "g-properties-changed",
...
cc_background_panel_dispose (GObject *object)
{
  ...
  g_clear_object (&self->color_scheme_settings);
...
cc_background_panel_init (CcBackgroundPanel *self)
{
  ...
  self->color_scheme_settings = g_settings_new (COLOR_SCHEME_PATH_ID);
  ...
  g_signal_connect_object (self->color_scheme_settings, "changed",
                           G_CALLBACK (color_scheme_changed_cb),
                           self, G_CONNECT_SWAPPED);
  
  g_settings_bind (self->color_scheme_settings, "enabled",
                   self->beginning_row, "visible", G_SETTINGS_BIND_GET);

  g_settings_bind (self->color_scheme_settings, "enabled",
                   self->end_row, "visible", G_SETTINGS_BIND_GET);

  g_settings_bind (self->color_scheme_settings, "schedule-from",
                   self->beginning_row, "time", G_SETTINGS_BIND_DEFAULT);
  g_settings_bind (self->color_scheme_settings, "schedule-to",
