                    ...
                    GFileInfo      *info)
{
  ...
  needs_download = !g_file_has_parent (file, self->backgrounds_folder);
                        ...
                        gpointer      user_data)
{
  ...
  g_debug ("Adding file %s (%d)", g_file_info_get_name (file_info), G_IS_FILE (self->backgrounds_folder));
  ...
  for (l = file_infos; l; l = l->next)
    {
      ...
      g_debug ("Found file %s", g_file_info_get_name (info));
...
load_backgrounds (BgRecentSource *self)
{
  ...
  g_debug ("Enumerating files under %s", backgrounds_path);
                   ...
                   gpointer      user_data)
{
  ...
  if (error)
  ...
  else
    {
      ...
      g_debug ("Successfully copied background: %s", original_file);
    }
                    ...
                    gpointer      user_data)
{
  ...
  if (error)
    {
      if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED))
        g_critical ("Failed to delete file: %s", error->message);
    }
  else
    {
      g_autofree gchar *original_file = g_file_get_path (G_FILE (source));
      g_debug ("Successfully deleted background: %s", original_file);
    }
