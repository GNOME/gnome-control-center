...
struct _CcDeviceComboBox
{
  ...
  guint            added_handler;
  guint            removed_handler;
  guint            active_update_handler;
  gboolean         is_output;
};
                                       ...
                                       gboolean          is_output)
{
  ...
  if (self->mixer_control != NULL) {
    g_signal_handler_disconnect (self->mixer_control, self->added_handler);
    self->added_handler = 0;
    g_signal_handler_disconnect (self->mixer_control, self->removed_handler);
    self->removed_handler = 0;
    g_signal_handler_disconnect (self->mixer_control, self->active_update_handler);
    self->active_update_handler = 0;
  }
  g_clear_object (&self->mixer_control);
  ...
  self->added_handler = g_signal_connect_object (self->mixer_control,
                                                 added_signal,
                                                 G_CALLBACK (device_added_cb),
                                                 self, G_CONNECT_SWAPPED);
  self->removed_handler = g_signal_connect_object (self->mixer_control,
                                                   removed_signal,
                                                   G_CALLBACK (device_removed_cb),
                                                   self, G_CONNECT_SWAPPED);
  self->active_update_handler = g_signal_connect_object (self->mixer_control,
