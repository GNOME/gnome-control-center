...
update_ui (CcWifiConnectionRow *self)
{
  ...
  if (security != NM_AP_SEC_UNKNOWN && security != NM_AP_SEC_NONE)
	...
	gtk_widget_set_tooltip_text (GTK_WIDGET (self->encrypted_icon), _("Insecure network"));
	}
      else
	{
        icon_name = "network-wireless-encrypted-symbolic";
        gtk_widget_set_tooltip_text (GTK_WIDGET (self->encrypted_icon), _("WEP encrypted network"));
	}
