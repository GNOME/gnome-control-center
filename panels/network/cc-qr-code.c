                        ...
                        gint      scale)
{
  ...
  /* QRcode_encodeString (const char *string, int version, QRecLevel level,
   *                      QRencodeMode hint, int casesensitive)
   * QR_ECLEVEL_L: Low level of correction
   * QR_MODE_8: 8 bit data mode.
   */
  qr_code = QRcode_encodeString (self->text, 2, QR_ECLEVEL_L, QR_MODE_8, 1);
  ...
  pixel_size = MAX (1, size / (qr_code->width));
  padding = (size - qr_code->width * pixel_size) / 2;
  ...
  for (int row = 0; row < qr_code->width; row++)
    {
      for (int column = 0; column < qr_code->width; column++)
        {
          if (*qr_data & 1)
