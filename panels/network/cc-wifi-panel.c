...
get_wifi_password (NMConnection *c)
{
  ...
  if (g_str_equal (sec_type, "WEP"))
    {
      ...
      /*
       * TYPE_KEY with valid password of length 10 and 26 will be
       * hexadecimal passwords, we shouldn’t double quote such
       * passwords, assumes password is valid.
       */
      if (type == NM_WEP_KEY_TYPE_KEY &&
          (length == 10 || length == 26))
          quote = FALSE;
    }
  else
  ...
  return escape_string (password, quote);
}
                           ...
                           NMConnection *c)
{
  ...
  str = escape_string (ssid_text, TRUE);
  g_string_append (string, str);
  g_string_append_c (string, ';');
  g_free (str);
