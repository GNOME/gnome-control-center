...
static gboolean
get_ignore_hosts (GValue *value, GVariant *variant, gpointer user_data)
{
...
static GVariant *
set_ignore_hosts (const GValue *value, const GVariantType *expected_type, gpointer user_data)
{
    const char *sv;
    char **strv;
...
static void
cc_net_proxy_page_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_net_proxy_page_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcNetProxyPage *self = (CcNetProxyPage *) object;
...
cc_net_proxy_page_class_init (CcNetProxyPageClass *klass)
{
    ...
    properties[PROP_STATE_TEXT] =
        g_param_spec_string ("state-text", "Proxy state text", "Human readable Proxy state text", NULL,
                             G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_MODIFIED] = g_param_spec_boolean ("modified", "Proxy settings modified", "Proxy settings modified",
                                                      FALSE, G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    properties[PROP_ENABLED] =
        g_param_spec_boolean ("enabled", "", "", FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
static void
proxy_bind_settings (CcNetProxyPage *self, const char *type, gpointer url_entry, gpointer port_adjustment)
{
    ...
    g_settings_bind (settings, "host", url_entry, "text", G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (settings, "port", port_adjustment, "value", G_SETTINGS_BIND_DEFAULT);
}
...
cc_net_proxy_page_init (CcNetProxyPage *self)
{
    ...
    g_signal_connect_object (self->settings, "notify::has-unapplied", G_CALLBACK (proxy_settings_has_unapplied_cb),
                             self, G_CONNECT_SWAPPED | G_CONNECT_AFTER);

    g_signal_connect_object (self->settings, "changed", G_CALLBACK (proxy_settings_changed_cb), self,
                             G_CONNECT_SWAPPED | G_CONNECT_AFTER);
    proxy_settings_changed_cb (self);

    g_settings_bind (self->settings, "autoconfig-url", self->proxy_url_entry, "text", G_SETTINGS_BIND_DEFAULT);
    ...
    g_settings_bind_with_mapping (self->settings, "ignore-hosts", self->proxy_ignore_entry, "text",
                                  G_SETTINGS_BIND_DEFAULT, get_ignore_hosts, set_ignore_hosts, NULL, NULL);
...
void
cc_net_proxy_page_set_enabled (CcNetProxyPage *self, gboolean enable)
{
    ProxyMode mode;
