                       ...
                       gboolean secrets_only)
{
	...
	g_signal_connect_swapped (G_OBJECT (combo), "changed", G_CALLBACK (inner_auth_combo_changed_cb), method);
	return combo;
}
                     ...
                     gboolean secrets_only)
{
	...
	widget = GTK_WIDGET (gtk_builder_get_object (parent->builder, "eap_peap_ca_cert_not_required_checkbox"));
	g_assert (widget);
	g_signal_connect_swapped (G_OBJECT (widget), "toggled", G_CALLBACK (ca_cert_not_required_toggled), method);
