               ...
               GDesktopAppInfo *app_info)
{
        ...
        if (strcmp (type, INPUT_SOURCE_TYPE_IBUS) == 0)
                cc_input_row_set_icon (row, "system-run-symbolic");
...
update_buttons (CcRegionPanel *self)
{
        ...
        if (selected == NULL) {
                ...
                gtk_widget_set_sensitive (self->move_up_input, gtk_list_box_row_get_index (GTK_LIST_BOX_ROW (selected)) > 0);
                gtk_widget_set_sensitive (self->move_down_input, gtk_list_box_row_get_index (GTK_LIST_BOX_ROW (selected)) < n_rows - 1);
        }
