...
update_buttons (CcRegionPanel *self)
{
        ...
        if (selected == NULL) {
                ...
                gtk_widget_set_sensitive (GTK_WIDGET (self->move_up_input_button), gtk_list_box_row_get_index (selected) > 0);
                gtk_widget_set_sensitive (GTK_WIDGET (self->move_down_input_button), gtk_list_box_row_get_index (selected) < n_rows - 1);
        }
