                               ...
                               LocaleInfo     *info)
{
  ...
  gtk_widget_show_all (GTK_WIDGET (self->input_sources_listbox));
...
get_locale_infos (CcInputChooser *self)
{
  ...
  for (locale = locale_ids; *locale; ++locale)
	...
	simple_locale = g_strdup_printf ("%s.UTF-8", lang_code);
      ...
      g_hash_table_replace (self->locales, simple_locale, info);
  ...
  g_hash_table_replace (self->locales, info->id, info);
