                                  ...
                                  LocaleInfo     *info)
{
  ...
  if (info->default_input_source_row)
    gtk_container_add (GTK_CONTAINER (chooser->input_sources_listbox), GTK_WIDGET (info->default_input_source_row));

  g_hash_table_iter_init (&iter, info->layout_rows_by_id);
  while (g_hash_table_iter_next (&iter, (gpointer *) &id, (gpointer *) &row))
    gtk_container_add (GTK_CONTAINER (chooser->input_sources_listbox), row);
...
show_input_sources_for_locale (CcInputChooser *chooser,
                               LocaleInfo     *info)
{
  remove_all_children (GTK_CONTAINER (chooser->input_sources_listbox));
  ...
  gtk_container_add (GTK_CONTAINER (chooser->input_sources_listbox), GTK_WIDGET (info->back_row));

  add_input_source_rows_for_locale (chooser, info);

  gtk_widget_show_all (GTK_WIDGET (chooser->input_sources_listbox));

  gtk_adjustment_set_value (chooser->scroll_adjustment,
                            gtk_adjustment_get_lower (chooser->scroll_adjustment));
  gtk_list_box_set_header_func (chooser->input_sources_listbox, cc_list_box_update_header_func, NULL, NULL);
  gtk_list_box_invalidate_filter (chooser->input_sources_listbox);
  gtk_list_box_set_selection_mode (chooser->input_sources_listbox, GTK_SELECTION_SINGLE);
  gtk_list_box_set_activate_on_single_click (chooser->input_sources_listbox, FALSE);
  gtk_list_box_unselect_all (chooser->input_sources_listbox);

  if (gtk_widget_is_visible (GTK_WIDGET (chooser->filter_entry)) &&
      !gtk_widget_is_focus (GTK_WIDGET (chooser->filter_entry)))
    gtk_widget_grab_focus (GTK_WIDGET (chooser->filter_entry));
}
...
show_locale_rows (CcInputChooser *chooser)
{
  ...
  while (g_hash_table_iter_next (&iter, NULL, (gpointer *) &info))
    {
      ...
      gtk_container_add (GTK_CONTAINER (chooser->input_sources_listbox), GTK_WIDGET (info->locale_row));
    }

  gtk_container_add (GTK_CONTAINER (chooser->input_sources_listbox), GTK_WIDGET (chooser->more_row));
  ...
  gtk_list_box_set_header_func (chooser->input_sources_listbox, cc_list_box_update_header_func, NULL, NULL);
  gtk_list_box_invalidate_filter (chooser->input_sources_listbox);
  gtk_list_box_set_selection_mode (chooser->input_sources_listbox, GTK_SELECTION_NONE);
  gtk_list_box_set_activate_on_single_click (chooser->input_sources_listbox, TRUE);
...
do_filter (CcInputChooser *chooser)
{
  ...
  if (!chooser->filter_words[0])
    {
      gtk_list_box_invalidate_filter (chooser->input_sources_listbox);
      gtk_list_box_set_placeholder (chooser->input_sources_listbox, NULL);
    }
  else if (previous_words == NULL || strvs_differ (chooser->filter_words, previous_words))
    {
      gtk_list_box_invalidate_filter (chooser->input_sources_listbox);
      gtk_list_box_set_placeholder (chooser->input_sources_listbox, chooser->no_results);
    }
...
static void
on_filter_entry_search_changed_cb (CcInputChooser *chooser)
{
...
{
  gtk_widget_show (GTK_WIDGET (chooser->filter_entry));
  gtk_widget_grab_focus (GTK_WIDGET (chooser->filter_entry));
...
static void
on_input_sources_listbox_row_activated_cb (CcInputChooser *chooser, GtkListBoxRow  *row)
{
  gpointer data;
...
static void
on_input_sources_listbox_selected_rows_changed_cb (CcInputChooser *chooser)
{
  gboolean sensitive = TRUE;
  GtkListBoxRow *row;

  row = gtk_list_box_get_selected_row (chooser->input_sources_listbox);
  if (!row || g_object_get_data (G_OBJECT (row), "back"))
    sensitive = FALSE;

  gtk_widget_set_sensitive (GTK_WIDGET (chooser->add_button), sensitive);
}

static gboolean
on_input_sources_listbox_button_release_event_cb (CcInputChooser *chooser, GdkEvent *event)
{
  gdouble x, y;
  GtkListBoxRow *row;

  gdk_event_get_coords (event, &x, &y);
  row = gtk_list_box_get_row_at_y (chooser->input_sources_listbox, y);
  if (row && g_object_get_data (G_OBJECT (row), "back"))
    {
...
get_locale_infos (CcInputChooser *chooser)
{
  ...
  for (locale = locale_ids; *locale; ++locale)
	...
	simple_locale = g_strdup_printf ("%s.UTF-8", lang_code);
      ...
      g_hash_table_replace (chooser->locales, simple_locale, info);
...
static gboolean
on_filter_entry_key_release_event_cb (CcInputChooser *chooser, GdkEventKey *event)
{
  if (event->keyval == GDK_KEY_Escape) {
    chooser->showing_extra = FALSE;
    gtk_entry_set_text (GTK_ENTRY (chooser->filter_entry), "");
    gtk_widget_hide (GTK_WIDGET (chooser->filter_entry));
                      ...
                      GHashTable   *ibus_engines)
{
  ...
  chooser->xkb_info = g_object_ref (xkb_info);
  if (ibus_engines)
    chooser->ibus_engines = g_hash_table_ref (ibus_engines);
  ...
  gtk_list_box_set_filter_func (chooser->input_sources_listbox, list_filter, chooser, NULL);
  gtk_list_box_set_sort_func (chooser->input_sources_listbox, list_sort, chooser, NULL);

  if (chooser->is_login)
    gtk_widget_show (GTK_WIDGET (chooser->login_label));
                               ...
                               gchar         **name)
{
  ...
  selected = gtk_list_box_get_selected_row (chooser->input_sources_listbox);
  if (!selected)
    return FALSE;
