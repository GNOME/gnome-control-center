               ...
               GdkDragContext *drag_context)
{
  ...
  self->drag_widget = gtk_list_box_new ();
  gtk_widget_show (GTK_WIDGET (self->drag_widget));
  gtk_widget_set_size_request (self->drag_widget, alloc.width, alloc.height);
  ...
  gtk_list_box_drag_highlight_row (GTK_LIST_BOX (self->drag_widget), GTK_LIST_BOX_ROW (drag_row));
...
{
  g_clear_pointer (&self->drag_widget, gtk_widget_destroy);
}
...
cc_input_row_new (CcInputSource *source)
{
  ...
  gtk_widget_set_visible (self->settings_button, CC_IS_INPUT_SOURCE_IBUS (source));
