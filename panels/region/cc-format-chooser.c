 ...
 * Written by:
 *     Matthias Clasen
 *     Mohammed Sadiq <sadiq@sadqipk.org>
 */
...
struct _CcFormatChooser {
        ...
        GtkSearchEntry *common_region_title;
        GtkListBox     *common_region_listbox;
        GtkLabel       *region_title;
        GtkListBox     *region_listbox;
        GtkLabel       *date_format_label;
        GtkLabel       *time_format_label;
        GtkLabel       *date_time_format_label;
        GtkLabel       *number_format_label;
        GtkLabel       *measurement_format_label;
        GtkLabel       *paper_format_label;
        gboolean        adding;
...
display_date (GtkLabel *label, GDateTime *dt, const gchar *format)
{
        ...
        gtk_label_set_text (label, g_strstrip (s));
}
...
format_chooser_back_button_clicked_cb (CcFormatChooser *self)
{
  ...
  gtk_header_bar_set_title (self->title_bar, _("Formats"));
  hdy_leaflet_set_visible_child_name (self->main_leaflet, "region-list");
  gtk_stack_set_visible_child (self->title_buttons, GTK_WIDGET (self->cancel_button));
...
format_chooser_leaflet_fold_changed_cb (CcFormatChooser *self)
{
  ...
  folded = hdy_leaflet_get_fold (self->main_leaflet) == HDY_FOLD_FOLDED;
                           ...
                           GtkWidget       *button)
{
  ...
  hdy_leaflet_set_visible_child_name (self->main_leaflet, "preview");
  gtk_stack_set_visible_child (self->title_buttons, GTK_WIDGET (self->back_button));
  gtk_widget_hide (GTK_WIDGET (self->done_button));

  if (locale_name)
    gtk_header_bar_set_title (self->title_bar, locale_name);
}
...
filter_changed (CcFormatChooser *chooser)
{
        ...
        gtk_widget_set_visible (GTK_WIDGET (chooser->common_region_listbox), visible);
        gtk_widget_set_visible (GTK_WIDGET (chooser->common_region_title), visible);
        gtk_widget_set_visible (GTK_WIDGET (chooser->region_title), visible);
        ...
        if (g_strcmp0 (new_locale_id, chooser->region) == 0) {
                gtk_dialog_response (GTK_DIALOG (chooser),
                                     gtk_dialog_get_response_for_widget (GTK_DIALOG (chooser),
                                                                         GTK_WIDGET (chooser->done_button)));
        } else {
...
cc_format_chooser_init (CcFormatChooser *chooser)
{
        ...
        gtk_widget_show (GTK_WIDGET (chooser->no_results));
