...
{
    return self->has_accelerometer && self->selected_output && cc_display_monitor_is_builtin (self->selected_output);
}

static void
on_auto_orientation_changed_cb (CcDisplaySettings *self)
{
    gboolean auto_rotate = show_auto_rotate_row (self) && adw_switch_row_get_active (self->auto_orientation_row);
...
{
    return cc_display_monitor_supports_rotation (self->selected_output, CC_DISPLAY_ROTATION_90 | CC_DISPLAY_ROTATION_180
                                                                            | CC_DISPLAY_ROTATION_270);
}

static const gchar *
string_for_rotation (CcDisplayRotation rotation, CcDisplayRatio ratio)
{
    switch (ratio) {
...
static const gchar *
make_aspect_string (gint width, gint height)
{
    int ratio;
...
static gchar *
make_variable_refresh_rate_string (CcDisplayMonitor *output, CcDisplayMode *mode)
{
    int min_freq;
    ...
    if (min_freq > 0) {
         ...
         */
        return g_strdup_printf (_("Variable (%d\u2013%.2lf Hz)"), min_freq, cc_display_mode_get_freq_f (mode));
    } else {
        /* Translators: "Variable" is an adjective that refers to the refresh rate */
        return g_strdup_printf (_("Variable (up to %.2lf Hz)"), cc_display_mode_get_freq_f (mode));
    }
}

static gchar *
make_expander_refresh_rate_string (CcDisplayMonitor *output, CcDisplayMode *mode)
{
...
static gboolean
mode_to_refresh_rate_transform_func (GBinding *binding, const GValue *source_value, GValue *target_value,
                                     CcDisplaySettings *self)
{
    ...
    refresh_rate_string = make_expander_refresh_rate_string (self->selected_output, mode);
...
cc_display_settings_rebuild_ui (CcDisplaySettings *self)
{
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->auto_orientation_row), show_auto_rotate_row (self));
    ...
    adw_switch_row_set_active (self->enabled_row, cc_display_monitor_is_active (self->selected_output));
    ...
    if (should_show_rotation (self)) {
        guint i;
        CcDisplayRotation rotations[] = { CC_DISPLAY_ROTATION_NONE, CC_DISPLAY_ROTATION_90, CC_DISPLAY_ROTATION_270,
        ...
        gtk_string_list_splice (GTK_STRING_LIST (self->orientation_list), 0,
                                g_list_model_get_n_items (self->orientation_list), NULL);
    ...
    if (!cc_display_config_is_cloning (self->config)) {
        ...
        for (item = modes; item != NULL; item = item->next) {
             ...
             */
            new = g_list_store_insert_sorted (self->refresh_rate_list, mode,
                                              (GCompareDataFunc) sort_modes_by_refresh_rate_desc, NULL);
        ...
        gtk_widget_set_sensitive (GTK_WIDGET (self->variable_refresh_rate_row), has_variable_refresh_rate_modes);
    ...
    gtk_string_list_splice (GTK_STRING_LIST (self->scale_list), 0, g_list_model_get_n_items (self->scale_list), NULL);
    ...
    for (i = 0; i < scales->len; i++) {
        ...
        g_object_set_data_full (G_OBJECT (value_object), "scale", g_memdup2 (&scale, sizeof (double)), g_free);
        if (is_selected)
        ...
        g_object_set_data_full (G_OBJECT (scale_toggle), "scale", g_memdup2 (&scale, sizeof (double)), g_free);
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->hdr_row), cc_display_monitor_supports_color_mode (
                                                            self->selected_output, CC_DISPLAY_COLOR_MODE_BT2100));
    adw_switch_row_set_active (self->hdr_row, cc_display_monitor_get_color_mode (self->selected_output)
    ...
    adw_switch_row_set_active (self->underscanning_row, cc_display_monitor_get_underscanning (self->selected_output));
...
static void
on_output_changed_cb (CcDisplaySettings *self, GParamSpec *pspec, CcDisplayMonitor *output)
{
...
on_enabled_row_active_changed_cb (CcDisplaySettings *self)
{
    ...
    cc_display_monitor_set_active (self->selected_output, adw_switch_row_get_active (self->enabled_row));
...
on_variable_refresh_rate_active_changed_cb (CcDisplaySettings *self)
{
    ...
    if (!adw_switch_row_get_active (self->variable_refresh_rate_row)) {
        cc_display_monitor_set_refresh_rate_mode (self->selected_output, MODE_REFRESH_RATE_MODE_FIXED);
    } else {
        cc_display_monitor_set_refresh_rate_mode (self->selected_output, MODE_REFRESH_RATE_MODE_VARIABLE);
    }
...
on_hdr_row_active_changed_cb (CcDisplaySettings *self)
{
    ...
    cc_display_monitor_set_color_mode (self->selected_output, color_mode);
...
on_underscanning_row_active_changed_cb (CcDisplaySettings *self)
{
    ...
    cc_display_monitor_set_underscanning (self->selected_output, adw_switch_row_get_active (self->underscanning_row));

    g_signal_emit_by_name (G_OBJECT (self), "updated", self->selected_output);
}

static void
cc_display_settings_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_display_settings_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcDisplaySettings *self = CC_DISPLAY_SETTINGS (object);
...
cc_display_settings_class_init (CcDisplaySettingsClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/display/cc-display-settings.ui");

    props[PROP_HAS_ACCELEROMETER] = g_param_spec_boolean (
        "has-accelerometer", "Has Accelerometer", "If an accelerometre is available for the builtin display", FALSE,
        G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);

    props[PROP_CONFIG] = g_param_spec_object ("config", "Display Config", "The display configuration to work with",
                                              CC_TYPE_DISPLAY_CONFIG,
                                              G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);

    props[PROP_SELECTED_OUTPUT] = g_param_spec_object (
        "selected-output", "Selected Output", "The output that is currently selected on the configuration",
        CC_TYPE_DISPLAY_MONITOR, G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY | G_PARAM_STATIC_STRINGS);

    g_object_class_install_properties (gobject_class, PROP_LAST, props);

    g_signal_new ("updated", CC_TYPE_DISPLAY_SETTINGS, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 1,
...
cc_display_settings_init (CcDisplaySettings *self)
{
    ...
    g_settings_bind (self->touchscreen_settings, "orientation-lock", self->auto_orientation_row, "active",
    ...
    adw_combo_row_set_model (ADW_COMBO_ROW (self->orientation_row), G_LIST_MODEL (self->orientation_list));
    adw_combo_row_set_model (ADW_COMBO_ROW (self->scale_combo_row), G_LIST_MODEL (self->scale_list));

    expression =
        gtk_cclosure_expression_new (G_TYPE_STRING, NULL, 0, NULL, G_CALLBACK (make_refresh_rate_string), self, NULL);
    adw_combo_row_set_expression (ADW_COMBO_ROW (self->refresh_rate_row), expression);
    adw_combo_row_set_model (ADW_COMBO_ROW (self->refresh_rate_row), G_LIST_MODEL (self->refresh_rate_list));

    adw_combo_row_set_expression (self->preferred_refresh_rate_row, expression);
    adw_combo_row_set_model (self->preferred_refresh_rate_row, G_LIST_MODEL (self->refresh_rate_list));
    gtk_expression_unref (expression);

    g_object_bind_property_full (self->preferred_refresh_rate_row, "selected-item",
                                 self->refresh_rate_expander_suffix_label, "label", G_BINDING_DEFAULT,
                                 (GBindingTransformFunc) mode_to_refresh_rate_transform_func, NULL, self, NULL);

    expression =
        gtk_cclosure_expression_new (G_TYPE_STRING, NULL, 0, NULL, G_CALLBACK (make_resolution_string), self, NULL);
    adw_combo_row_set_expression (ADW_COMBO_ROW (self->resolution_row), expression);
    adw_combo_row_set_model (ADW_COMBO_ROW (self->resolution_row), G_LIST_MODEL (self->resolution_list));
    gtk_expression_unref (expression);
...
void
cc_display_settings_set_has_accelerometer (CcDisplaySettings *self, gboolean has_accelerometer)
{
...
void
cc_display_settings_set_config (CcDisplaySettings *self, CcDisplayConfig *config)
{
...
void
cc_display_settings_set_selected_output (CcDisplaySettings *self, CcDisplayMonitor *output)
{
...
void
cc_display_settings_refresh_layout (CcDisplaySettings *self, gboolean collapsed)
{
    gboolean use_combo;
...
void
cc_display_settings_set_multimonitor (CcDisplaySettings *self, gboolean multimonitor)
{
    gtk_widget_set_visible (self->enabled_listbox, multimonitor);

    if (multimonitor && self->config && self->selected_output) {
        /* Make Enable Row insensitive if this is the last display available. */
        gtk_widget_set_sensitive (self->enabled_listbox, cc_display_config_count_useful_monitors (self->config) != 1
                                                             || !cc_display_monitor_is_useful (self->selected_output));
    }
