...
config_find_types (CcDisplayPanel *panel)
{
  ...
  g_debug ("finding types has %i monitors and %i active ones", n_outputs, n_active_outputs);
          ...
          if (cc_display_monitor_is_primary (output))
            {
              ...
              g_print ("setting a monitor active");
            }
          else
            {
              cc_display_monitor_set_active (output, FALSE);
              g_print ("setting a monitor inactive");
            }
        }
      break;

    case CC_DISPLAY_CONFIG_JOIN:
      g_debug ("making join config");
...
on_output_selection_combo_changed_cb (CcDisplayPanel *panel)
{
  ...
  gtk_combo_box_get_active_iter (panel->output_selection_combo, &iter);

  if (!gtk_list_store_iter_is_valid (panel->output_selection_list, &iter))
    return;
...
rebuild_ui (CcDisplayPanel *panel)
{
      ...
      if (n_usable_outputs == 1)
        {
          ...
          g_object_set_data_full (G_OBJECT (panel->output_selection_two_first),
                                  "display",
                                  g_object_ref (output),
                                  g_object_unref);
        }
      else if (n_usable_outputs == 2)
        {
          gtk_button_set_label (GTK_BUTTON (panel->output_selection_two_second),
                                cc_display_monitor_get_ui_name (output));
          g_object_set_data_full (G_OBJECT (panel->output_selection_two_second),
                                  "display",
                                  g_object_ref (output),
                                  g_object_unref);
        }
  ...
  if (n_outputs == 2 && !panel->lid_is_closed)
    {
      ...
      g_debug ("currently selected: %x, valid %x", type, types);
      if (!(type & types))
  ...
  /* Arrangement is shown if we are not in single mode, i.e. more than 1 usable monitor */
  gtk_widget_set_visible (panel->arrangement_frame, n_outputs > (panel->lid_is_closed ? 2 : 1));
