...
static void
get_bounding_box (CcDisplayConfig *config, gint *x1, gint *y1, gint *x2, gint *y2, gint *max_w, gint *max_h)
{
    GList *outputs, *l;
...
static void
monitor_get_drawing_rect (CcDisplayArrangement *self, CcDisplayMonitor *output, gint *x1, gint *y1, gint *x2, gint *y2)
{
    gdouble x, y;
...
static void
get_snap_distance (SnapData *snap_data, gint mon_x, gint mon_y, gint new_x, gint new_y, gdouble *dist_x,
                   gdouble *dist_y)
{
...
static void
maybe_update_snap (SnapData *snap_data, gint mon_x, gint mon_y, gint new_x, gint new_y, SnapDirection snapped,
                   SnapDirection major_axis, gint minor_unlimited)
{
...
static void
find_best_snapping (CcDisplayConfig *config, CcDisplayMonitor *snap_output, SnapData *snap_data)
{
    GList *outputs, *l;
    gint x1, y1, x2, y2;
    gint w, h;
...
cc_display_arrangement_find_monitor_at (CcDisplayArrangement *self, gint x, gint y)
{
...
static void
on_output_changed_cb (CcDisplayArrangement *self, CcDisplayMonitor *output)
{
...
static void
cc_display_arrangement_draw (GtkDrawingArea *drawing_area, cairo_t *cr, gint width, gint height, gpointer user_data)
{
...
static gboolean
on_click_gesture_pressed_cb (CcDisplayArrangement *self, gint n_press, gdouble x, gdouble y)
{
...
static gboolean
on_click_gesture_released_cb (CcDisplayArrangement *self, gint n_press, gdouble x, gdouble y)
{
    CcDisplayMonitor *output;
    ...
    output = cc_display_arrangement_find_monitor_at (self, x, y);
    gtk_widget_set_cursor_from_name (GTK_WIDGET (self), output != NULL ? "grab" : NULL);
...
static gboolean
on_motion_controller_motion_cb (CcDisplayArrangement *self, gdouble x, gdouble y)
{
    gdouble event_x, event_y;
    gint mon_x, mon_y;
    SnapData snap_data;
...
static void
cc_display_arrangement_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_display_arrangement_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
cc_display_arrangement_class_init (CcDisplayArrangementClass *klass)
{
    ...
    props[PROP_CONFIG] = g_param_spec_object ("config", "Display Config", "The display configuration to work with",
                                              CC_TYPE_DISPLAY_CONFIG, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
    ...
    props[PROP_SELECTED_OUTPUT] = g_param_spec_object (
        ...
        CC_TYPE_DISPLAY_MONITOR, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    g_object_class_install_properties (gobject_class, PROP_LAST, props);

    g_signal_new ("updated", CC_TYPE_DISPLAY_ARRANGEMENT, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);
...
cc_display_arrangement_init (CcDisplayArrangement *self)
{
    ...
    gtk_drawing_area_set_draw_func (GTK_DRAWING_AREA (self), cc_display_arrangement_draw, self, NULL);
...
void
cc_display_arrangement_set_config (CcDisplayArrangement *self, CcDisplayConfig *config)
{
...
void
cc_display_arrangement_set_selected_output (CcDisplayArrangement *self, CcDisplayMonitor *output)
{
...
static gboolean
try_snap_output (CcDisplayConfig *config, CcDisplayMonitor *output)
{
    SnapData snap_data;
    gint x, y, w, h;
