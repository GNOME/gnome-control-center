...
static gint
markers_cmp (gconstpointer a, gconstpointer b)
{
...
sanitize_printer_model (const gchar *printer_make_and_model)
{
    ...
    gchar backup;
    size_t length = 0;
    gchar *forbidden[] = { "foomatic", ",", "hpijs", "hpcups", "(recommended)", "postscript (recommended)", NULL };
    int i;
    ...
    for (i = 0; i < g_strv_length (forbidden); i++) {
        ...
        if (breakpoint == NULL || (tmp2 != NULL && tmp2 < breakpoint))
            breakpoint = tmp2;
    }
...
{
    return !((self->inklevel->marker_levels != NULL) && (self->inklevel->marker_colors != NULL)
             && (self->inklevel->marker_names != NULL) && (self->inklevel->marker_types != NULL));
}
 ...
 */
static void
tone_down_color (GdkRGBA *color, gdouble hue_ratio, gdouble saturation_ratio, gdouble value_ratio)
{
    gfloat h, s, v;

    gtk_rgb_to_hsv (color->red, color->green, color->blue, &h, &s, &v);
    gtk_hsv_to_rgb (h * hue_ratio, s * saturation_ratio, v * value_ratio, &color->red, &color->green, &color->blue);
}

static void
supply_levels_draw_cb (GtkDrawingArea *drawing_area, cairo_t *cr, int width, int height, gpointer user_data)
{
        ...
        if (g_strv_length (marker_levelsv) == g_strv_length (marker_colorsv)
            ...
            for (tmp_list = markers; tmp_list; tmp_list = tmp_list->next) {
                ...
                if (tooltip_text) {
                    ...
                    tooltip_text = g_strdup_printf ("%s\n%s", old_tooltip_text, ((MarkerItem *) tmp_list->data)->name);
                } else
                    tooltip_text = g_strdup_printf ("%s", ((MarkerItem *) tmp_list->data)->name);
            }
...
static void
on_printer_rename_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
...
static void
show_printer_details_response_cb (PpPrinterEntry *self, PpDetailsDialog *dialog)
{
    ...
    if (g_strcmp0 (self->printer_name, new_name) != 0 && printer_name_is_valid (new_name)) {
        ...
        pp_printer_rename_async (printer, new_name, NULL, on_printer_rename_cb, self);
    }
...
static void
printer_details_cb (PpPrinterEntry *self, const char *action, GVariant *parameter)
{
    PpDetailsDialog *dialog = pp_details_dialog_new (self->printer_name, self->printer_location, self->printer_hostname,
                                                     self->printer_make_and_model, self->is_authorized);
...
static void
printer_options_cb (PpPrinterEntry *self, const char *action_name, GVariant *parameter)
{
    PpOptionsDialog *dialog;

    dialog = pp_options_dialog_new (self->printer_name, self->is_authorized);

    gtk_window_set_transient_for (GTK_WINDOW (dialog), GTK_WINDOW (gtk_widget_get_native (GTK_WIDGET (self))));
...
static void
check_clean_heads_maintenance_command_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    PpPrinterEntry *self = user_data;
...
check_clean_heads_maintenance_command (PpPrinterEntry *self)
{
    ...
    pp_maintenance_command_is_supported_async (self->clean_command, self->check_clean_heads_cancellable,
                                               check_clean_heads_maintenance_command_cb, self);
}

static void
clean_heads_maintenance_command_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    PpPrinterEntry *self = user_data;
    g_autoptr(GError) error = NULL;
...
static void
printer_clean_heads_cb (PpPrinterEntry *self, const char *action_name, GVariant *parameter)
{
    if (self->clean_command == NULL)
        return;

    g_object_ref (self->clean_command);
    pp_maintenance_command_execute_async (self->clean_command, NULL, clean_heads_maintenance_command_cb, self);
}

static void
printer_remove_cb (PpPrinterEntry *self, const char *action_name, GVariant *parameter)
{
    g_signal_emit_by_name (self, "printer-delete");
}

static void
get_jobs_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    PpPrinterEntry *self = user_data;
    g_autoptr(GError) error = NULL;
...
pp_printer_entry_update_jobs_count (PpPrinterEntry *self)
{
    ...
    pp_printer_get_jobs_async (printer, TRUE, CUPS_WHICHJOBS_ACTIVE, self->get_jobs_cancellable, get_jobs_cb, self);
}
...
pp_printer_entry_show_jobs_dialog (PpPrinterEntry *self)
{
    if (self->pp_jobs_dialog == NULL) {
        ...
        g_object_add_weak_pointer (G_OBJECT (self->pp_jobs_dialog), (gpointer *) &self->pp_jobs_dialog);

        g_signal_connect_object (self->pp_jobs_dialog, "close-attempt", G_CALLBACK (jobs_dialog_close_request_cb), self,
                                 G_CONNECT_SWAPPED);
...
static void
show_jobs_dialog (GtkButton *button, gpointer user_data)
{
...
static void
restart_printer (GtkButton *button, PpPrinterEntry *self)
{
...
PpPrinterEntry *
pp_printer_entry_new (cups_dest_t printer, gboolean is_authorized)
{
    PpPrinterEntry *self;
    ...
    self->clean_command =
        pp_maintenance_command_new (self->printer_name, "Clean", "all",
                                    /* Translators: Name of job which makes printer to clean its heads */
                                    _("Clean print heads"));
    check_clean_heads_maintenance_command (self);

    gtk_drawing_area_set_draw_func (self->supply_drawing_area, supply_levels_draw_cb, self, NULL);
...
void
pp_printer_entry_update (PpPrinterEntry *self, cups_dest_t printer, gboolean is_authorized)
{
    ...
    int i, j;
    static const char *const reasons[] = { "toner-low",
                                           "toner-empty",
                                           "developer-low",
                                           "developer-empty",
                                           "marker-supply-low",
                                              ...
                                              N_("Waste receptacle almost full"),
                                                 ...
                                                 N_("Waste receptacle full"),
                                                    ...
                                                    N_("The optical photo conductor is near end of life"),
                                                       /* Translators: Optical photo conductors are used in laser
                                                          printers */
    ...
    if ((self->printer_state == PRINTER_STOPPED || !is_accepting_jobs) && status != NULL && status[0] != '\0') {
...
static void
set_compact (PpPrinterEntry *self, gboolean compact)
{
    compact = !!compact;
...
static void
pp_printer_entry_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    PpPrinterEntry *self = PP_PRINTER_ENTRY (object);
...
static void
pp_printer_entry_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    PpPrinterEntry *self = PP_PRINTER_ENTRY (object);
...
pp_printer_entry_class_init (PpPrinterEntryClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/printers/pp-printer-entry.ui");
    ...
    properties[PROP_DEFAULT] =
        g_param_spec_boolean ("default", "default", "default", FALSE, G_PARAM_READWRITE | G_PARAM_EXPLICIT_NOTIFY);

    properties[PROP_COMPACT] = g_param_spec_boolean ("compact", "compact", "compact", FALSE, G_PARAM_READWRITE);

    g_object_class_install_properties (object_class, N_PROPS, properties);

    signals[IS_DEFAULT_PRINTER] = g_signal_new ("printer-changed", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST, 0,
                                                NULL, NULL, NULL, G_TYPE_NONE, 0);

    signals[PRINTER_DELETE] = g_signal_new ("printer-delete", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST, 0, NULL,
                                            NULL, NULL, G_TYPE_NONE, 0);

    signals[PRINTER_RENAMED] = g_signal_new ("printer-renamed", G_TYPE_FROM_CLASS (klass), G_SIGNAL_RUN_LAST, 0, NULL,
