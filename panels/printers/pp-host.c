...
static void
pp_host_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *param_spec)
{
    PpHost *self = PP_HOST (object);
    PpHostPrivate *priv = pp_host_get_instance_private (self);
...
static void
pp_host_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *param_spec)
{
    PpHost *self = PP_HOST (object);
    PpHostPrivate *priv = pp_host_get_instance_private (self);
...
pp_host_class_init (PpHostClass *klass)
{
    ...
    props[PROP_HOSTNAME] = g_param_spec_string ("hostname", "Hostname", "The hostname", NULL, G_PARAM_READWRITE);

    props[PROP_PORT] =
        g_param_spec_int ("port", "Port", "The port", -1, G_MAXINT32, PP_HOST_UNSET_PORT, G_PARAM_READWRITE);

    g_object_class_install_properties (gobject_class, N_PROPS, props);

    signals[AUTHENTICATION_REQUIRED] = g_signal_new ("authentication-required", G_TYPE_FROM_CLASS (klass),
...
{
    return g_object_new (PP_TYPE_HOST, "hostname", hostname, NULL);
}
...
static void
_pp_host_get_snmp_devices_thread (GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
    ...
    g_spawn_sync (NULL, argv, NULL, G_SPAWN_STDERR_TO_DEV_NULL, NULL, NULL, &stdout_string, NULL, &exit_status, &error);
    ...
    if (exit_status == 0 && stdout_string) {
        ...
        if (length >= 4) {
            ...
            device = g_object_new (PP_TYPE_PRINT_DEVICE, "is-network-device", is_network_device, "device-uri",
                                   printer_informations[1], "device-make-and-model", printer_informations[2],
                                   "device-info", printer_informations[3], "acquisition-method",
                                   ACQUISITION_METHOD_SNMP, "device-name", device_name, NULL);
...
void
pp_host_get_snmp_devices_async (PpHost *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                                gpointer user_data)
{
...
GPtrArray *
pp_host_get_snmp_devices_finish (PpHost *self, GAsyncResult *res, GError **error)
{
...
static void
_pp_host_get_remote_cups_devices_thread (GTask *task, gpointer source_object, gpointer task_data,
                                         GCancellable *cancellable)
{
    ...
    http = httpConnect2 (priv->hostname, port, NULL, AF_UNSPEC, HTTP_ENCRYPTION_IF_REQUESTED, 1, 30000, NULL);
#else
    ...
    if (http) {
        ...
        if (num_of_devices > 0) {
            for (i = 0; i < num_of_devices; i++) {
                ...
                device_uri = g_strdup_printf ("ipp://%s:%d/printers/%s", priv->hostname, port, dests[i].name);

                device_location = cupsGetOption ("printer-location", dests[i].num_options, dests[i].options);

                device = g_object_new (PP_TYPE_PRINT_DEVICE, "is-network-device", TRUE, "device-uri", device_uri,
                                       "device-name", dests[i].name, "device-location", device_location, "host-name",
                                       priv->hostname, "host-port", port, "acquisition-method",
                                       ACQUISITION_METHOD_REMOTE_CUPS_SERVER, NULL);
                g_ptr_array_add (devices, device);
            }
        }
...
void
pp_host_get_remote_cups_devices_async (PpHost *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                                       gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
...
GPtrArray *
pp_host_get_remote_cups_devices_finish (PpHost *self, GAsyncResult *res, GError **error)
{
    g_return_val_if_fail (g_task_is_valid (res, self), NULL);
    g_return_val_if_fail (error == NULL || *error == NULL, NULL);
    return g_task_propagate_pointer (G_TASK (res), error);
...
static void
jetdirect_connection_test_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    connection = g_socket_client_connect_to_host_finish (G_SOCKET_CLIENT (source_object), res, &error);
    ...
    if (connection != NULL) {
        ...
        device_uri = g_strdup_printf ("socket://%s:%d", priv->hostname, data->port);

        device = g_object_new (PP_TYPE_PRINT_DEVICE, "is-network-device", TRUE, "device-uri", device_uri,
                               /* Translators: The found device is a JetDirect printer */
                               "device-name",
                               _("JetDirect Printer"), "host-name", priv->hostname, "host-port", data->port,
                                 "acquisition-method", ACQUISITION_METHOD_JETDIRECT, NULL);
        g_ptr_array_add (devices, device);
    }
...
void
pp_host_get_jetdirect_devices_async (PpHost *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                                     gpointer user_data)
{
    PpHostPrivate *priv = pp_host_get_instance_private (self);
    JetDirectData *data;
    ...
    if (address != NULL && address[0] != '/') {
        ...
        g_socket_client_connect_to_host_async (client, address, data->port, cancellable, jetdirect_connection_test_cb,
...
GPtrArray *
pp_host_get_jetdirect_devices_finish (PpHost *self, GAsyncResult *res, GError **error)
{
    g_return_val_if_fail (g_task_is_valid (res, self), NULL);
    g_return_val_if_fail (error == NULL || *error == NULL, NULL);
    return g_task_propagate_pointer (G_TASK (res), error);
...
static gboolean
test_lpd_queue (GSocketClient *client, gchar *address, gint port, GCancellable *cancellable, gchar *queue_name)
{
    g_autoptr(GSocketConnection) connection = NULL;
    gboolean result = FALSE;
    g_autoptr(GError) error = NULL;

    connection = g_socket_client_connect_to_host (client, address, port, cancellable, &error);
    ...
    if (connection != NULL) {
        if (G_IS_TCP_CONNECTION (connection)) {
            ...
            bytes_written = g_output_stream_write (output, buffer, length, NULL, &error);

            if (bytes_written != -1) {
                bytes_read = g_input_stream_read (input, buffer, BUFFER_LENGTH, NULL, &error);
...
static void
_pp_host_get_lpd_devices_thread (GTask *task, gpointer source_object, gpointer task_data, GCancellable *cancellable)
{
    g_autoptr(GSocketConnection) connection = NULL;
    PpHost *self = source_object;
    PpHostPrivate *priv = pp_host_get_instance_private (self);
    ...
    if (connection != NULL) {
        ...
        for (iter = candidates; iter != NULL; iter = iter->next) {
            ...
            if (test_lpd_queue (client, address, port, cancellable, candidate)) {
        ...
        if (found_queue != NULL) {
            ...
            device_uri = g_strdup_printf ("lpd://%s:%d/%s", priv->hostname, port, found_queue);

            device = g_object_new (PP_TYPE_PRINT_DEVICE, "is-network-device", TRUE, "device-uri", device_uri,
                                   /* Translators: The found device is a Line Printer Daemon printer */
                                   "device-name",
                                   _("LPD Printer"), "host-name", priv->hostname, "host-port", port,
                                     "acquisition-method", ACQUISITION_METHOD_LPD, NULL);
            g_ptr_array_add (devices, device);
        }
...
void
pp_host_get_lpd_devices_async (PpHost *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                               gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
...
GPtrArray *
pp_host_get_lpd_devices_finish (PpHost *self, GAsyncResult *res, GError **error)
{
    g_return_val_if_fail (g_task_is_valid (res, self), NULL);
    g_return_val_if_fail (error == NULL || *error == NULL, NULL);
    return g_task_propagate_pointer (G_TASK (res), error);
