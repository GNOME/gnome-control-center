...
pp_new_printer_dialog_finalize (GObject *object)
{
  ...
  if (self->remote_host_cancellable)
    {
  ...
  g_list_free_full (self->local_cups_devices, (GDestroyNotify) g_object_unref);
  self->local_cups_devices = NULL;
                           ...
                           gpointer    user_data)
{
  ...
  if (device_uris != NULL)
  ...
  else
    {
      ...
      g_list_free_full (self->local_cups_devices, g_object_unref);
      self->local_cups_devices = NULL;
    }
