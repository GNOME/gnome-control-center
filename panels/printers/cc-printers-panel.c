...
static void set_current_page (GObject *source_object, GAsyncResult *result, gpointer user_data);

static void
execute_action (CcPrintersPanel *self, GVariant *action)
{
...
static void
cc_printers_panel_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_printers_panel_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    CcPrintersPanel *self = CC_PRINTERS_PANEL (object);
    GVariant *parameters;
...
static void
printer_removed_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
...
static void
on_get_job_attributes_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (attributes != NULL) {
        ...
        if ((username = g_variant_lookup_value (attributes, "job-originating-user-name", G_VARIANT_TYPE ("as")))
            != NULL) {
            ...
            if ((printer_uri = g_variant_lookup_value (attributes, "job-printer-uri", G_VARIANT_TYPE ("as"))) != NULL) {
                ...
                if (job_originating_user_name != NULL && job_printer_uri != NULL
                    && g_strcmp0 (job_originating_user_name, cupsUser ()) == 0 && g_strrstr (job_printer_uri, "/") != 0
                    && self->dests != NULL) {
                    PpPrinterEntry *printer_entry;
                    gchar *printer_name;
...
static void
on_cups_notification (GDBusConnection *connection, const char *sender_name, const char *object_path,
                      const char *interface_name, const char *signal_name, GVariant *parameters, gpointer user_data)
{
    ...
    static gchar *requested_attrs[] = { "job-printer-uri", "job-originating-user-name", NULL };

    if (g_strcmp0 (signal_name, "PrinterAdded") != 0 && g_strcmp0 (signal_name, "PrinterDeleted") != 0
        && g_strcmp0 (signal_name, "PrinterStateChanged") != 0 && g_strcmp0 (signal_name, "PrinterStopped") != 0
        && g_strcmp0 (signal_name, "JobCreated") != 0 && g_strcmp0 (signal_name, "JobCompleted") != 0)
        return;

    if (g_variant_n_children (parameters) == 1)
        g_variant_get (parameters, "(&s)", &text);
    else if (g_variant_n_children (parameters) == 6) {
        g_variant_get (parameters, "(&s&s&su&sb)", &text, &printer_uri, &printer_name, &printer_state,
                       &printer_state_reasons, &printer_is_accepting_jobs);
    } else if (g_variant_n_children (parameters) == 11) {
        g_variant_get (parameters, "(&s&s&su&sbuu&s&su)", &text, &printer_uri, &printer_name, &printer_state,
                       &printer_state_reasons, &printer_is_accepting_jobs, &job_id, &job_state, &job_state_reasons,
                       &job_name, &job_impressions_completed);
    }

    if (g_strcmp0 (signal_name, "PrinterAdded") == 0 || g_strcmp0 (signal_name, "PrinterDeleted") == 0
        || g_strcmp0 (signal_name, "PrinterStateChanged") == 0 || g_strcmp0 (signal_name, "PrinterStopped") == 0)
        actualize_printers_list (self);
    else if (g_strcmp0 (signal_name, "JobCreated") == 0 || g_strcmp0 (signal_name, "JobCompleted") == 0) {
        g_autoptr(PpJob) job = NULL;

        job = pp_job_new (job_id, NULL, 0, JOB_DEFAULT_PRIORITY, NULL);
        pp_job_get_attributes_async (job, requested_attrs, cc_panel_get_cancellable (CC_PANEL (self)),
                                     on_get_job_attributes_cb, self);
    }
}

static gchar *subscription_events[] = {
    "printer-added", "printer-deleted", "printer-stopped", "printer-state-changed", "job-created", "job-completed", NULL
};

static void
renew_subscription_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    gint subscription_id;
...
renew_subscription (gpointer data)
{
    ...
    pp_cups_renew_subscription_async (self->cups, self->subscription_id, subscription_events, SUBSCRIPTION_DURATION,
                                      cc_panel_get_cancellable (CC_PANEL (self)), renew_subscription_cb, data);

    return G_SOURCE_CONTINUE;
}

static void
attach_to_cups_notifier_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    g_autoptr(GError) error = NULL;
    gint subscription_id;
    ...
    if (subscription_id > 0) {
        ...
        self->subscription_renewal_id = g_timeout_add_seconds (RENEW_INTERVAL, renew_subscription, self);

        self->cups_proxy =
            cc_object_storage_create_dbus_proxy_sync (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, CUPS_DBUS_NAME,
                                                      CUPS_DBUS_PATH, CUPS_DBUS_INTERFACE, NULL, &error);
        ...
        self->dbus_subscription_id =
            g_dbus_connection_signal_subscribe (self->cups_bus_connection, NULL, CUPS_DBUS_INTERFACE, NULL,
                                                CUPS_DBUS_PATH, NULL, 0, on_cups_notification, self, NULL);
    }
}
...
attach_to_cups_notifier (gpointer data)
{
    ...
    pp_cups_renew_subscription_async (self->cups, self->subscription_id, subscription_events, SUBSCRIPTION_DURATION,
                                      cc_panel_get_cancellable (CC_PANEL (self)), attach_to_cups_notifier_cb, data);
}

static void
subscription_cancel_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
...
detach_from_cups_notifier (gpointer data)
{
    ...
    if (self->dbus_subscription_id != 0) {
        g_dbus_connection_signal_unsubscribe (self->cups_bus_connection, self->dbus_subscription_id);
        self->dbus_subscription_id = 0;
    }

    pp_cups_cancel_subscription_async (self->cups, self->subscription_id, subscription_cancel_cb, NULL);
...
static void
on_printer_deleted (CcPrintersPanel *self, PpPrinterEntry *printer_entry)
{
    ...
    if (!self->toast) {
        ...
        g_signal_connect_swapped (self->toast, "button-clicked", G_CALLBACK (on_printer_deletion_undone), self);
        g_signal_connect_swapped (self->toast, "dismissed", G_CALLBACK (on_notification_dismissed), self);
    }

    /* Translators: %s is the printer name */
    notification_message =
        g_strdup_printf (_("Printer “%s” has been deleted"), pp_printer_entry_get_name (printer_entry));
...
static void
on_printer_renamed (CcPrintersPanel *self, gchar *new_name, PpPrinterEntry *printer_entry)
{
...
static void
add_printer_entry (CcPrintersPanel *self, cups_dest_t printer)
{
    PpPrinterEntry *printer_entry;
    GSList *widgets, *l;
    ...
    g_signal_connect_object (printer_entry, "printer-changed", G_CALLBACK (on_printer_changed), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (printer_entry, "printer-delete", G_CALLBACK (on_printer_deleted), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (printer_entry, "printer-renamed", G_CALLBACK (on_printer_renamed), self,
                             G_CONNECT_SWAPPED);

    g_object_bind_property (self, "compact", printer_entry, "compact", G_BINDING_SYNC_CREATE);
...
static void
set_current_page (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    gboolean success;
...
static gboolean
remove_nonexisting_entry (CcPrintersPanel *self, PpPrinterEntry *entry)
{
...
static void
actualize_printers_list_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    ...
    if ((self->num_dests == 0 && self->new_printer_name == NULL)
        || (self->num_dests == 1 + g_list_length (self->deleted_printers) && self->deleted_printer_name != NULL))
    ...
    if (self->new_printer_name != NULL) {
        ...
        printer_entry = GTK_WIDGET (g_hash_table_lookup (self->printer_entries, self->new_printer_name));
        if (printer_entry != NULL) {
...
{
    pp_cups_get_dests_async (self->cups, cc_panel_get_cancellable (CC_PANEL (self)), actualize_printers_list_cb, self);
}

static void
printer_add_async_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    gboolean success;
    g_autoptr(GError) error = NULL;
...
static void
new_printer_dialog_response_cb (GtkWindow *_dialog, gint response_id, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    ...
    if (response_id == GTK_RESPONSE_OK) {
        ...
        pp_new_printer_add_async (new_printer, cc_panel_get_cancellable (CC_PANEL (self)), printer_add_async_cb, self);
    }
...
printer_add_cb (CcPrintersPanel *self)
{
    ...
    self->pp_new_printer_dialog = pp_new_printer_dialog_new (self->all_ppds_list, new_printer_dialog_response_cb, self);

    gtk_window_set_transient_for (GTK_WINDOW (self->pp_new_printer_dialog), GTK_WINDOW (native));
...
update_sensitivity (gpointer user_data)
{
    ...
    if (cups_server && g_ascii_strncasecmp (cups_server, "localhost", 9) != 0
        && g_ascii_strncasecmp (cups_server, "127.0.0.1", 9) != 0 && g_ascii_strncasecmp (cups_server, "::1", 3) != 0
        && cups_server[0] != '/')
        local_server = FALSE;
...
static void
on_lockdown_settings_changed (CcPrintersPanel *self, const char *key)
{
...
static void
cups_status_check_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    gboolean success;
...
static void
connection_test_cb (GObject *source_object, GAsyncResult *result, gpointer user_data)
{
    ...
    if (!success) {
        self->cups_status_check_id = g_timeout_add_seconds (CUPS_STATUS_CHECK_INTERVAL, cups_status_check, self);
    }
}

static void
get_all_ppds_async_cb (PPDList *ppds, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
    ...
    if (self->pp_new_printer_dialog)
        pp_new_printer_dialog_set_ppd_list (self->pp_new_printer_dialog, self->all_ppds_list);
}

static gboolean
filter_function (GtkListBoxRow *row, gpointer user_data)
{
    CcPrintersPanel *self = (CcPrintersPanel *) user_data;
...
static gint
sort_function (GtkListBoxRow *row1, GtkListBoxRow *row2, gpointer user_data)
{
...
cc_printers_panel_class_init (CcPrintersPanelClass *klass)
{
    ...
    g_object_class_install_property (object_class, PROP_COMPACT,
                                     g_param_spec_boolean ("compact", "compact", "compact", FALSE, G_PARAM_READWRITE));

    g_type_ensure (CC_TYPE_PERMISSION_INFOBAR);

    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/printers/cc-printers-panel.ui");
...
cc_printers_panel_init (CcPrintersPanel *self)
{
    ...
    self->printer_entries = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, NULL);

    g_object_set_data_full (self->reference, "self", self, NULL);

    /* connect signals */
    gtk_list_box_set_filter_func (self->content, filter_function, self, NULL);
    g_signal_connect_swapped (self->search_entry, "search-changed", G_CALLBACK (gtk_list_box_invalidate_filter),
                              self->content);
    gtk_list_box_set_sort_func (self->content, sort_function, NULL, NULL);

    self->lockdown_settings = g_settings_new ("org.gnome.desktop.lockdown");
    if (self->lockdown_settings)
        g_signal_connect_object (self->lockdown_settings, "changed", G_CALLBACK (on_lockdown_settings_changed), self,
                                 G_CONNECT_SWAPPED | G_CONNECT_AFTER);

    /* Add unlock button */
    self->permission =
        (GPermission *) polkit_permission_new_sync ("org.opensuse.cupspkhelper.mechanism.all-edit", NULL, NULL, NULL);
...
Please check your installation");
    ...
    get_all_ppds_async (cc_panel_get_cancellable (CC_PANEL (self)), get_all_ppds_async_cb, self);
