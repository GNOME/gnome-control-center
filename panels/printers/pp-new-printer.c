...
static void
pp_new_printer_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *param_spec)
{
    PpNewPrinter *self = PP_NEW_PRINTER (object);
...
static void
pp_new_printer_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *param_spec)
{
    PpNewPrinter *self = PP_NEW_PRINTER (object);
...
pp_new_printer_class_init (PpNewPrinterClass *klass)
{
    ...
    props[PROP_NAME] = g_param_spec_string ("name", "Name", "The new printer's name", NULL, G_PARAM_READWRITE);

    props[PROP_ORIGINAL_NAME] = g_param_spec_string ("original-name", "Original name",
                                                     "Original name of the new printer", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_URI] =
        g_param_spec_string ("device-uri", "Device URI", "The new printer's device URI", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_ID] =
        g_param_spec_string ("device-id", "DeviceID", "The new printer's DeviceID", NULL, G_PARAM_READWRITE);

    props[PROP_PPD_NAME] =
        g_param_spec_string ("ppd-name", "PPD name", "Name of PPD for the new printer", NULL, G_PARAM_READWRITE);

    props[PROP_PPD_FILE_NAME] =
        g_param_spec_string ("ppd-file-name", "PPD file name", "PPD file for the new printer", NULL, G_PARAM_READWRITE);

    props[PROP_INFO] = g_param_spec_string ("info", "Printer info", "The new printer's info", NULL, G_PARAM_READWRITE);

    props[PROP_LOCATION] =
        g_param_spec_string ("location", "Printer location", "The new printer's location", NULL, G_PARAM_READWRITE);

    props[PROP_MAKE_AND_MODEL] = g_param_spec_string ("make-and-model", "Printer make and model",
                                                      "The new printer's make and model", NULL, G_PARAM_READWRITE);

    props[PROP_HOST_NAME] =
        g_param_spec_string ("host-name", "Hostname", "The new printer's hostname", NULL, G_PARAM_READWRITE);

    props[PROP_HOST_PORT] =
        g_param_spec_int ("host-port", "Host port", "The port of the host", 0, G_MAXINT32, 631, G_PARAM_READWRITE);

    props[PROP_IS_NETWORK_DEVICE] = g_param_spec_boolean (
        "is-network-device", "Network device", "Whether the new printer is a network device", FALSE, G_PARAM_READWRITE);

    props[PROP_WINDOW_ID] =
        g_param_spec_uint ("window-id", "WindowID", "Window ID of parent window", 0, G_MAXUINT, 0, G_PARAM_READWRITE);
...
static void
_pp_new_printer_add_async_cb (gboolean success, PpNewPrinter *self)
{
    if (!success) {
        g_task_return_new_error (self->task, G_IO_ERROR, G_IO_ERROR_FAILED, "Installation of the new printer failed.");
        return;
    }
...
static void
printer_add_real_async_cb (cups_dest_t *destination, gpointer user_data)
{
...
static void
printer_add_real_async_dbus_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    PpNewPrinter *self = user_data;
    ...
    if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
        get_named_dest_async (self->name, printer_add_real_async_cb, self);
    }
}
...
printer_add_real_async (PpNewPrinter *self)
{
    ...
    g_dbus_connection_call (
        bus, MECHANISM_BUS, "/", MECHANISM_BUS, self->ppd_name ? "PrinterAdd" : "PrinterAddWithPpdFile",
        g_variant_new ("(sssss)", self->name, self->device_uri, self->ppd_name ? self->ppd_name : self->ppd_file_name,
                       self->info ? self->info : "", self->location ? self->location : ""),
        G_VARIANT_TYPE ("(s)"), G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT, NULL, printer_add_real_async_dbus_cb, self);
}
...
static void
printer_add_async_scb3 (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    PpNewPrinter *self = user_data;
    g_autoptr(GVariant) output = NULL;
    ...
    if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED) && ppd_item && ppd_item->ppd_name) {
...
static void
install_printer_drivers_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    PpNewPrinter *self = user_data;
    g_autoptr(GVariant) output = NULL;
    g_autoptr(GError) error = NULL;
    ...
    if (!g_error_matches (error, G_IO_ERROR, G_IO_ERROR_CANCELLED)) {
        ...
        if (bus) {
            g_dbus_connection_call (bus, SCP_BUS, SCP_PATH, SCP_IFACE, "GetBestDrivers",
                                    g_variant_new ("(sss)", self->device_id,
                                                   self->make_and_model ? self->make_and_model : "",
                                                   self->device_uri ? self->device_uri : ""),
                                    G_VARIANT_TYPE ("(a(ss))"), G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT_LONG,
                                    self->cancellable, printer_add_async_scb3, self);
        } else {
...
static void
printer_add_async_scb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    PpNewPrinter *self = user_data;
    g_autoptr(GDBusConnection) bus = NULL;
    ...
    if (!cancelled) {
        if (ppd_item == NULL || ppd_item->ppd_match_level < PPD_EXACT_MATCH) {
            ...
            if (bus) {
                ...
                g_dbus_connection_call (
                    bus, PACKAGE_KIT_BUS, PACKAGE_KIT_PATH, PACKAGE_KIT_MODIFY_IFACE, "InstallPrinterDrivers",
                    g_variant_new ("(uass)", self->window_id, &array_builder, "hide-finished"), G_VARIANT_TYPE ("()"),
                    G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT_LONG, NULL, install_printer_drivers_cb, self);
            } else {
...
static void
printer_add_async_scb4 (const gchar *ppd_filename, gpointer user_data)
{
    PpNewPrinter *self = user_data;
...
glist_uniq (GList *list)
{
    ...
    for (iter = list; iter; iter = iter->next) {
        if (tmp == NULL || g_strcmp0 ((gchar *) tmp->data, (gchar *) iter->data) != 0) {
            tmp = iter;
...
printer_configure_async_finish (PCData *data)
{
    ...
    if (data->set_accept_jobs_finished && data->set_enabled_finished
        && (data->autoconfigure_finished || self->is_network_device) && data->set_media_size_finished
...
static void
pao_cb (gboolean success, gpointer user_data)
{
...
static void
printer_set_accepting_jobs_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) output = NULL;
    PCData *data = (PCData *) user_data;
    g_autoptr(GError) error = NULL;
...
static void
printer_set_enabled_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) output = NULL;
    PCData *data = (PCData *) user_data;
    g_autoptr(GError) error = NULL;
...
static void
install_package_names_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) output = NULL;
...
static void
search_files_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) output = NULL;
    IMEData *data = (IMEData *) user_data;
    g_autoptr(GError) error = NULL;
    ...
    if (output) {
        ...
        g_variant_get (output, "(bs)", &installed, &package);
    ...
    if (data->executables) {
        ...
        g_dbus_connection_call (G_DBUS_CONNECTION (source_object), PACKAGE_KIT_BUS, PACKAGE_KIT_PATH,
                                PACKAGE_KIT_QUERY_IFACE, "SearchFile", g_variant_new ("(ss)", (gchar *) item->data, ""),
                                G_VARIANT_TYPE ("(bs)"), G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT_LONG, data->cancellable,
                                search_files_cb, data);
        ...
        if (data->packages) {
            ...
            for (pkg_iter = data->packages; pkg_iter; pkg_iter = pkg_iter->next)
                g_variant_builder_add (&array_builder, "s", (gchar *) pkg_iter->data);

            g_dbus_connection_call (
                G_DBUS_CONNECTION (source_object), PACKAGE_KIT_BUS, PACKAGE_KIT_PATH, PACKAGE_KIT_MODIFY_IFACE,
                "InstallPackageNames", g_variant_new ("(uass)", data->window_id, &array_builder, "hide-finished"), NULL,
                G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT_LONG, data->cancellable, install_package_names_cb, data);
...
static void
get_missing_executables_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) output = NULL;
    IMEData *data = (IMEData *) user_data;
    g_autoptr(GError) error = NULL;
...
DBus method \"MissingExecutables\" to find missing executables and filters.");
    ...
    if (executables) {
        ...
        item = data->executables;
        g_dbus_connection_call (g_object_ref (G_DBUS_CONNECTION (source_object)), PACKAGE_KIT_BUS, PACKAGE_KIT_PATH,
                                PACKAGE_KIT_QUERY_IFACE, "SearchFile", g_variant_new ("(ss)", (gchar *) item->data, ""),
                                G_VARIANT_TYPE ("(bs)"), G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT_LONG, data->cancellable,
                                search_files_cb, data);
...
printer_get_ppd_cb (const gchar *ppd_filename, gpointer user_data)
{
    ...
    if (data->ppd_file_name) {
        ...
        if (!bus) {
            ...
            g_dbus_connection_call (bus, SCP_BUS, SCP_PATH, SCP_IFACE, "MissingExecutables",
                                    g_variant_new ("(s)", data->ppd_file_name), G_VARIANT_TYPE ("(as)"),
                                    G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT, data->cancellable, get_missing_executables_cb,
                                    data);
            return;
        }
    }
...
printer_configure_async (PpNewPrinter *self)
{
    ...
    if (self->name) {
        bus = g_bus_get_sync (G_BUS_TYPE_SYSTEM, NULL, &error);
        if (bus) {
            g_dbus_connection_call (bus, MECHANISM_BUS, "/", MECHANISM_BUS, "PrinterSetAcceptJobs",
                                    g_variant_new ("(sbs)", self->name, TRUE, ""), G_VARIANT_TYPE ("(s)"),
                                    G_DBUS_CALL_FLAGS_NONE, -1, NULL, printer_set_accepting_jobs_cb, data);

            g_dbus_connection_call (g_object_ref (bus), MECHANISM_BUS, "/", MECHANISM_BUS, "PrinterSetEnabled",
                                    g_variant_new ("(sb)", self->name, TRUE), G_VARIANT_TYPE ("(s)"),
                                    G_DBUS_CALL_FLAGS_NONE, -1, NULL, printer_set_enabled_cb, data);
        } else {
            g_warning ("Failed to get system bus: %s", error->message);
            data->set_accept_jobs_finished = TRUE;
            data->set_enabled_finished = TRUE;
    ...
    if (!self->is_network_device) {
        ...
        command = pp_maintenance_command_new (self->name, "autoconfigure", NULL,
                                              /* Translators: Name of job which makes printer to autoconfigure itself */
                                              _("Automatic configuration"));

        pp_maintenance_command_execute_async (command, NULL, pp_maintenance_command_execute_cb, data);
    }
    ...
    printer_get_ppd_async (self->name, NULL, 0, printer_get_ppd_cb, ime_data);
}

void
pp_new_printer_add_async (PpNewPrinter *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                          gpointer user_data)
{
    ...
    if (self->ppd_name || self->ppd_file_name) {
        ...
        if (bus) {
            g_dbus_connection_call (bus, SCP_BUS, SCP_PATH, SCP_IFACE, "GetBestDrivers",
                                    g_variant_new ("(sss)", self->device_id,
                                                   self->make_and_model ? self->make_and_model : "",
                                                   self->device_uri ? self->device_uri : ""),
                                    G_VARIANT_TYPE ("(a(ss))"), G_DBUS_CALL_FLAGS_NONE, DBUS_TIMEOUT_LONG, cancellable,
                                    printer_add_async_scb, self);
        } else {
            g_warning ("Failed to get system bus: %s", error->message);
            _pp_new_printer_add_async_cb (FALSE, self);
        }
        ...
        printer_get_ppd_async (self->original_name, self->host_name, self->host_port, printer_add_async_scb4, self);
    } else {
        _pp_new_printer_add_async_cb (FALSE, self);
    }
}
...
gboolean
pp_new_printer_add_finish (PpNewPrinter *self, GAsyncResult *res, GError **error)
{
