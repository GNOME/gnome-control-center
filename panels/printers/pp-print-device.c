...
static void
pp_print_device_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *param_spec)
{
    PpPrintDevice *self = PP_PRINT_DEVICE (object);
...
static void
pp_print_device_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *param_spec)
{
    PpPrintDevice *self = PP_PRINT_DEVICE (object);
...
pp_print_device_class_init (PpPrintDeviceClass *klass)
{
    ...
    props[PROP_DEVICE_NAME] =
        g_param_spec_string ("device-name", "Device name", "Name of the device", NULL, G_PARAM_READWRITE);

    props[PROP_DISPLAY_NAME] = g_param_spec_string ("display-name", "Display name",
                                                    "Name of the device formatted for users", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_ORIGINAL_NAME] = g_param_spec_string ("device-original-name", "Device original name",
                                                            "Original name of the device", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_MAKE_AND_MODEL] = g_param_spec_string ("device-make-and-model", "Device make and model",
                                                             "Make and model of the device", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_LOCATION] =
        g_param_spec_string ("device-location", "Device location", "Locaton of the device", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_INFO] =
        g_param_spec_string ("device-info", "Device info", "Information about the device", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_URI] =
        g_param_spec_string ("device-uri", "Device URI", "URI of the device", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_ID] =
        g_param_spec_string ("device-id", "DeviceID", "DeviceID of the device", NULL, G_PARAM_READWRITE);

    props[PROP_DEVICE_PPD] =
        g_param_spec_string ("device-ppd", "Device PPD", "Name of the PPD of the device", NULL, G_PARAM_READWRITE);

    props[PROP_HOST_NAME] =
        g_param_spec_string ("host-name", "Host name", "Hostname of the device", NULL, G_PARAM_READWRITE);

    props[PROP_HOST_PORT] =
        g_param_spec_int ("host-port", "Host port", "The port of the host", 0, G_MAXINT32, 0, G_PARAM_READWRITE);

    props[PROP_IS_AUTHENTICATED_SERVER] =
        g_param_spec_boolean ("is-authenticated-server", "Is authenticated server",
                              "Whether the device is a server which needs authentication", FALSE, G_PARAM_READWRITE);

    props[PROP_ACQUISITION_METHOD] =
        g_param_spec_int ("acquisition-method", "Acquisition method", "Acquisition method of the device", 0, G_MAXINT32,
                          0, G_PARAM_READWRITE);

    props[PROP_IS_NETWORK_DEVICE] = g_param_spec_boolean (
        "is-network-device", "Network device", "Whether the device is a network device", FALSE, G_PARAM_READWRITE);
...
{
    return g_object_new (
        PP_TYPE_PRINT_DEVICE, "device-name", pp_print_device_get_device_name (self), "display-name",
        pp_print_device_get_display_name (self), "device-original-name",
        pp_print_device_get_device_original_name (self), "device-make-and-model",
        pp_print_device_get_device_make_and_model (self), "device-location", pp_print_device_get_device_location (self),
        "device-info", pp_print_device_get_device_info (self), "device-uri", pp_print_device_get_device_uri (self),
        "device-id", pp_print_device_get_device_id (self), "device-ppd", pp_print_device_get_device_ppd (self),
        "host-name", pp_print_device_get_host_name (self), "host-port", pp_print_device_get_host_port (self),
        "is-authenticated-server", pp_print_device_is_authenticated_server (self), "acquisition-method",
        pp_print_device_get_acquisition_method (self), "is-network-device", pp_print_device_is_network_device (self),
        NULL);
}
