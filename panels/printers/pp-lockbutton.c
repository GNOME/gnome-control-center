...
static void update_state (PpLockButton *button);
...
G_DEFINE_TYPE (PpLockButton, pp_lock_button, GTK_TYPE_BIN);

static void
pp_lock_button_finalize (GObject *object)
{
  PpLockButton *button = PP_LOCK_BUTTON (object);
  PpLockButtonPrivate *priv = button->priv;
  ...
  G_OBJECT_CLASS (pp_lock_button_parent_class)->finalize (object);
}

static void
pp_lock_button_get_property (GObject    *object,
                             guint       property_id,
                             GValue     *value,
                             GParamSpec *pspec)
{
...
static void
pp_lock_button_set_property (GObject      *object,
                             guint         property_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
...
static void
pp_lock_button_init (PpLockButton *button)
{
...
static void
pp_lock_button_constructed (GObject *object)
{
  PpLockButton *button = PP_LOCK_BUTTON (object);
  PpLockButtonPrivate *priv = button->priv;
  ...
  if (G_OBJECT_CLASS (pp_lock_button_parent_class)->constructed != NULL)
    G_OBJECT_CLASS (pp_lock_button_parent_class)->constructed (object);
}

static void
pp_lock_button_get_preferred_width (GtkWidget *widget,
                                    gint      *minimum,
                                    gint      *natural)
{
  PpLockButtonPrivate *priv = PP_LOCK_BUTTON (widget)->priv;

  gtk_widget_get_preferred_width (priv->box, minimum, natural);
}

static void
pp_lock_button_get_preferred_height (GtkWidget *widget,
                                     gint      *minimum,
                                     gint      *natural)
{
  PpLockButtonPrivate *priv = PP_LOCK_BUTTON (widget)->priv;
...
static void
pp_lock_button_size_allocate (GtkWidget     *widget,
...
static void
pp_lock_button_class_init (PpLockButtonClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  gobject_class->finalize     = pp_lock_button_finalize;
  gobject_class->get_property = pp_lock_button_get_property;
  gobject_class->set_property = pp_lock_button_set_property;
  gobject_class->constructed  = pp_lock_button_constructed;

  widget_class->get_preferred_width = pp_lock_button_get_preferred_width;
  widget_class->get_preferred_height = pp_lock_button_get_preferred_height;
  widget_class->size_allocate = pp_lock_button_size_allocate;

  g_type_class_add_private (klass, sizeof (PpLockButtonPrivate));
 ...
 *
 * Returns: a new #PpLockButton
 *
 * Since: 3.0
 */
GtkWidget *
pp_lock_button_new (GPermission *permission)
{
  g_return_val_if_fail (permission != NULL, NULL);

  return GTK_WIDGET (g_object_new (PP_TYPE_LOCK_BUTTON,
                                   "permission", permission,
                                   NULL));
}

static void
update_state (PpLockButton *button)
{
  PpLockButtonPrivate *priv = button->priv;
  gint page;
  const gchar *tooltip;
...
on_permission_changed (GPermission *permission,
                       ...
                       gpointer     user_data)
{
  PpLockButton *button = PP_LOCK_BUTTON (user_data);
...
static void
handle_click (PpLockButton *button)
{
  PpLockButtonPrivate *priv = button->priv;
...
{
  handle_click (PP_LOCK_BUTTON (user_data));
}
...
/**
 * pp_lock_button_get_permission:
 * @button: a #PpLockButton
 *
 ...
 */
GPermission *
pp_lock_button_get_permission (PpLockButton *button)
{
  g_return_val_if_fail (PP_IS_LOCK_BUTTON (button), NULL);
