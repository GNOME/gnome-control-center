...
static void
pp_maintenance_command_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *param_spec)
{
    PpMaintenanceCommand *self = PP_MAINTENANCE_COMMAND (object);
...
static void
pp_maintenance_command_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *param_spec)
{
    PpMaintenanceCommand *self = PP_MAINTENANCE_COMMAND (object);
...
pp_maintenance_command_class_init (PpMaintenanceCommandClass *klass)
{
    ...
    props[PROP_PRINTER_NAME] =
        g_param_spec_string ("printer-name", "Printer name", "Name of the printer", NULL, G_PARAM_READWRITE);

    props[PROP_COMMAND] =
        g_param_spec_string ("command", "Maintenance command", "Command to execute", NULL, G_PARAM_READWRITE);

    props[PROP_PARAMETERS] =
        g_param_spec_string ("parameters", "Optional parameters", "Optional parameters for the maintenance command",
                             NULL, G_PARAM_READWRITE);

    props[PROP_TITLE] = g_param_spec_string (
        "title", "Command title", "Title of the job by which the command will be executed", NULL, G_PARAM_READWRITE);
...
pp_maintenance_command_new (const gchar *printer_name, const gchar *command, const gchar *parameters,
                            const gchar *title)
{
    return g_object_new (PP_TYPE_MAINTENANCE_COMMAND, "printer-name", printer_name, "command", command, "parameters",
                         parameters, "title", title, NULL);
}

static gboolean _pp_maintenance_command_is_supported (const gchar *printer_name, const gchar *command);

static void
_pp_maintenance_command_execute_thread (GTask *task, gpointer source_object, gpointer task_data,
                                        GCancellable *cancellable)
{
    ...
    if (_pp_maintenance_command_is_supported (self->printer_name, self->command)) {
        ...
        ippAddString (request, IPP_TAG_OPERATION, IPP_TAG_NAME, "job-name", NULL, self->title);
        ippAddString (request, IPP_TAG_JOB, IPP_TAG_MIMETYPE, "document-format", NULL, "application/vnd.cups-command");
    ...
    if (!success) {
        g_task_return_new_error (task, G_IO_ERROR, G_IO_ERROR_FAILED, "Execution of maintenance command failed.");
    }
...
void
pp_maintenance_command_execute_async (PpMaintenanceCommand *self, GCancellable *cancellable,
...
gboolean
pp_maintenance_command_execute_finish (PpMaintenanceCommand *self, GAsyncResult *result, GError **error)
{
...
static gboolean
_pp_maintenance_command_is_supported (const gchar *printer_name, const gchar *command)
{
    ...
    ippAddString (request, IPP_TAG_OPERATION, IPP_TAG_URI, "printer-uri", NULL, printer_uri);
    ippAddString (request, IPP_TAG_OPERATION, IPP_TAG_KEYWORD, "requested-attributes", NULL, "printer-commands");
    ...
    if (response != NULL) {
        if (ippGetStatusCode (response) <= IPP_OK_CONFLICT) {
            ...
            if (commands_count > 0 && ippGetValueTag (attr) != IPP_TAG_NOVALUE
...
static void
_pp_maintenance_command_is_supported_thread (GTask *task, gpointer source_object, gpointer task_data,
                                             GCancellable *cancellable)
{
    PpMaintenanceCommand *self = PP_MAINTENANCE_COMMAND (source_object);
    gboolean success = FALSE;
...
void
pp_maintenance_command_is_supported_async (PpMaintenanceCommand *self, GCancellable *cancellable,
                                           GAsyncReadyCallback callback, gpointer user_data)
{
    g_autoptr(GTask) task = NULL;
...
gboolean
pp_maintenance_command_is_supported_finish (PpMaintenanceCommand *self, GAsyncResult *result, GError **error)
{
    g_return_val_if_fail (g_task_is_valid (result, self), FALSE);
