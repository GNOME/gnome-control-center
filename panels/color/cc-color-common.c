...
cc_color_device_get_title (CdDevice *device)
{
    ...
    /* is internal panel */
    if (cd_device_get_kind (device) == CD_DEVICE_KIND_DISPLAY && cd_device_get_embedded (device)) {
    ...
    /* is internal webcam */
    if (cd_device_get_kind (device) == CD_DEVICE_KIND_WEBCAM && cd_device_get_embedded (device)) {
...
cc_color_device_get_sortable_base (CdDevice *device)
{
    ...
    return g_strdup_printf ("%s-%s-%s", cc_color_device_kind_to_sort (device), cd_device_get_id (device), title);
}
