...
static const char *
get_profile_prefix_and_kind (CdProfile *profile, guint *kind_out)
{
    const char *prefix = NULL;
    const char *id;
    guint kind;

    id = cd_profile_get_metadata_item (profile, CD_PROFILE_METADATA_DATA_SOURCE);
...
static char *
get_profile_description (GtkListItem *list_item, CdProfile *profile)
{
...
static char *
get_warning_icon (GtkListItem *list_item, CdProfile *profile)
{
    char **warnings;
...
static gboolean
gcm_prefs_ensure_connected_profile (CcColorPanel *self, CdProfile *profile)
{
    gboolean ret;
    ...
    ret = cd_profile_connect_sync (profile, cc_panel_get_cancellable (CC_PANEL (self)), &error);
    if (!ret)
...
gcm_prefs_default_cb (CcColorPanel *self)
{
    ...
    ret = cd_profile_install_system_wide_sync (profile, cc_panel_get_cancellable (CC_PANEL (self)), &error);
    if (!ret)
        g_warning ("failed to set profile system-wide: %s", error->message);
}

static void
icc_prefs_imported_cb (GObject *source, GAsyncResult *res, gpointer user_data)
{
    ...
    profile = cd_client_import_profile_sync (self->client, file, cc_panel_get_cancellable (CC_PANEL (self)), &error);
    if (profile == NULL) {
...
gcm_prefs_file_chooser_get_icc_profile (CcColorPanel *self)
{
    ...
    gtk_file_dialog_open (dialog, toplevel, NULL, icc_prefs_imported_cb, self);
}
...
gcm_prefs_calib_apply_cb (CcColorPanel *self)
{
    ...
    ret = cc_color_calibrate_setup (self->calibrate, &error);
    if (!ret) {
    ...
    ret = cc_color_calibrate_start (self->calibrate, window, &error);
    if (!ret) {
...
static void
gcm_prefs_calib_temp_treeview_clicked_cb (CcColorPanel *self, GtkTreeSelection *selection)
{
    gboolean ret;
    GtkTreeIter iter;
    GtkTreeModel *model;
    ...
    gtk_tree_model_get (model, &iter, COLUMN_CALIB_TEMP_VALUE_K, &target_whitepoint, -1);
    cc_color_calibrate_set_temperature (self->calibrate, target_whitepoint);
}

static void
gcm_prefs_calib_kind_treeview_clicked_cb (CcColorPanel *self, GtkTreeSelection *selection)
{
    ...
    gtk_tree_model_get (model, &iter, COLUMN_CALIB_KIND_CAP_VALUE, &device_kind, -1);
    cc_color_calibrate_set_kind (self->calibrate, device_kind);
}

static void
gcm_prefs_calib_quality_treeview_clicked_cb (CcColorPanel *self, GtkTreeSelection *selection)
{
    ...
    /* save the values if we have a selection */
    gtk_tree_model_get (model, &iter, COLUMN_CALIB_QUALITY_VALUE, &quality, -1);
    cc_color_calibrate_set_quality (self->calibrate, quality);
}

static gboolean
gcm_prefs_calib_set_sensor_cap_supported_cb (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
    CdSensorCap cap;
    CdSensor *sensor = CD_SENSOR (data);
    gboolean supported;

    gtk_tree_model_get (model, iter, COLUMN_CALIB_KIND_CAP_VALUE, &cap, -1);
    supported = cd_sensor_has_cap (sensor, cap);
    g_debug ("%s(%s) is %s", cd_sensor_get_model (sensor), cd_sensor_cap_to_string (cap),
             supported ? "supported" : "not-supported");
    gtk_list_store_set (GTK_LIST_STORE (model), iter, COLUMN_CALIB_KIND_VISIBLE, supported, -1);
    return FALSE;
}
...
static void
gcm_prefs_calib_set_sensor (CcColorPanel *self, CdSensor *sensor)
{
    guint64 caps;
    guint8 i;
    ...
    gtk_tree_model_foreach (self->liststore_calib_kind, gcm_prefs_calib_set_sensor_cap_supported_cb, sensor);
...
static void
gcm_prefs_calib_sensor_treeview_clicked_cb (CcColorPanel *self, GtkTreeSelection *selection)
{
    gboolean ret;
    GtkTreeIter iter;
    GtkTreeModel *model;
    ...
    /* save the values if we have a selection */
    gtk_tree_model_get (model, &iter, COLUMN_CALIB_SENSOR_OBJECT, &sensor, -1);
...
gcm_prefs_calibrate_display (CcColorPanel *self)
{
    ...
    if (self->sensors->len > 1) {
        for (i = 0; i < self->sensors->len; i++) {
            ...
            gtk_list_store_set (GTK_LIST_STORE (self->liststore_calib_sensor), &iter, COLUMN_CALIB_SENSOR_OBJECT,
                                sensor_tmp, COLUMN_CALIB_SENSOR_DESCRIPTION, cd_sensor_get_model (sensor_tmp), -1);
        }
...
static gboolean
gcm_prefs_is_profile_suitable_for_device (CdProfile *profile, CdDevice *device)
{
    ...
    if (cd_device_get_kind (device) == CD_DEVICE_KIND_DISPLAY
        ...
        data_source = cd_profile_get_metadata_item (profile, CD_PROFILE_METADATA_STANDARD_SPACE);
        standard_space = cd_standard_space_from_string (data_source);
        if (standard_space == CD_STANDARD_SPACE_SRGB || standard_space == CD_STANDARD_SPACE_ADOBE_RGB) {
            ret = TRUE;
            goto out;
        }
    }
    ...
    data_source = cd_profile_get_metadata_item (profile, CD_PROFILE_METADATA_DATA_SOURCE);
...
static gint
gcm_prefs_combo_sort_func_cb (gconstpointer a, gconstpointer b, gpointer user_data)
{
    guint type_a, type_b;
...
static gboolean
gcm_prefs_ensure_connected_profiles (CcColorPanel *self, GPtrArray *profiles)
{
    guint i;
...
static void
gcm_prefs_add_profiles_suitable_for_devices (CcColorPanel *self, GPtrArray *profiles)
{
    CdProfile *profile_tmp;
    gboolean ret;
    g_autoptr(GError) error = NULL;
    ...
    /* get profiles */
    profile_array = cd_client_get_profiles_sync (self->client, cc_panel_get_cancellable (CC_PANEL (self)), &error);
    ...
    for (i = 0; i < profile_array->len; i++) {
        ...
        ret = gcm_prefs_is_profile_suitable_for_device (profile_tmp, self->current_device);
        if (!ret)
            continue;
...
static void
profile_exported_cb (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
...
gcm_prefs_calib_export_cb (CcColorPanel *self)
{
    ...
    gtk_file_dialog_save (dialog, GTK_WINDOW (gtk_widget_get_root (GTK_WIDGET (self))), NULL, profile_exported_cb,
                          profile);
}

static void
gcm_prefs_calib_export_link_cb (CcColorPanel *self, const gchar *url)
{
...
gcm_prefs_profile_remove_cb (CcColorPanel *self)
{
    ...
    ret = cd_device_remove_profile_sync (self->current_device, profile, cc_panel_get_cancellable (CC_PANEL (self)),
                                         &error);
    if (!ret)
...
static void
gcm_prefs_make_profile_default_cb (GObject *object, GAsyncResult *res, CcColorPanel *self)
{
    CdDevice *device = CD_DEVICE (object);
    gboolean ret = FALSE;
    g_autoptr(GError) error = NULL;

    ret = cd_device_make_profile_default_finish (device, res, &error);
    if (!ret) {
        g_warning ("failed to set default profile on %s: %s", cd_device_get_id (device), error->message);
    } else {
...
gcm_prefs_device_profile_enable_cb (CcColorPanel *self)
{
    ...
    /* just set it default */
    g_debug ("setting %s default on %s", cd_profile_get_id (profile), cd_device_get_id (self->current_device));
    cd_device_make_profile_default (self->current_device, profile, cc_panel_get_cancellable (CC_PANEL (self)),
                                    (GAsyncReadyCallback) gcm_prefs_make_profile_default_cb, self);
}
...
gcm_prefs_profile_view (CcColorPanel *self, CdProfile *profile)
{
    ...
    ret = g_spawn_async (NULL, (gchar **) argv->pdata, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, &error);
    if (!ret)
...
gcm_prefs_button_assign_ok_cb (CcColorPanel *self)
{
    ...
    if (!cd_device_get_enabled (self->current_device)) {
        ret =
            cd_device_set_enabled_sync (self->current_device, TRUE, cc_panel_get_cancellable (CC_PANEL (self)), &error);
        if (!ret) {
    ...
    ret = cd_device_add_profile_sync (self->current_device, CD_DEVICE_RELATION_HARD, profile,
                                      cc_panel_get_cancellable (CC_PANEL (self)), &error);
    if (!ret) {
...
gcm_prefs_set_calibrate_button_sensitivity (CcColorPanel *self)
{
    ...
    if (kind == CD_DEVICE_KIND_DISPLAY) {
        ...
        if (self->sensors == NULL || self->sensors->len == 0) {
            ...
            tooltip =
                _("The measuring instrument is not detected. Please check it is turned on and correctly connected.");
            goto out;
        }
...
gcm_prefs_device_clicked (CcColorPanel *self, CdDevice *device)
{
    ...
    g_debug ("selected device is: %s", cd_device_get_id (device));
...
static void
gcm_prefs_profiles_listview_selection_changed_cb (GtkSelectionModel *model, guint position, guint n_items,
                                                  CcColorPanel *self)
{
    CdProfile *profile;
#if CD_CHECK_VERSION(0, 1, 25)
...
static void
gcm_prefs_client_sensor_changed_cb (CdClient *client, CdSensor *sensor, CcColorPanel *self)
{
...
static void
gcm_prefs_add_device_profile (CcColorPanel *self, CdDevice *device, CdProfile *profile, gboolean is_default)
{
    gboolean ret;
    g_autoptr(GError) error = NULL;
    GtkWidget *widget;
    ...
    if (!cd_profile_has_access (profile)) {
        ...
        if (cd_profile_get_filename (profile) != NULL) {
            g_warning ("%s is not usable by this user", cd_profile_get_filename (profile));
        } else {
            g_warning ("%s is not usable by this user", cd_profile_get_id (profile));
        }
        return;
    }
...
static gboolean
gcm_prefs_find_profile_by_object_path (GPtrArray *profiles, const gchar *object_path)
{
    CdProfile *profile_tmp;
    guint i;
...
static gboolean
gcm_prefs_find_widget_by_object_path (GList *list, const gchar *object_path_device, const gchar *object_path_profile)
{
    GList *l;
    CdDevice *device_tmp;
    CdProfile *profile_tmp;
    ...
    for (l = list; l != NULL; l = l->next) {
        ...
        if (g_strcmp0 (object_path_device, cd_device_get_object_path (device_tmp)) != 0) {
            continue;
        }

        /* this profile */
        profile_tmp = cc_color_profile_get_profile (CC_COLOR_PROFILE (l->data));
        if (g_strcmp0 (object_path_profile, cd_profile_get_object_path (profile_tmp)) == 0) {
            return TRUE;
        }
    }
    return FALSE;
...
gcm_prefs_device_changed_cb (CcColorPanel *self, CdDevice *device)
{
    ...
    while (child) {
        ...
        ret = gcm_prefs_find_profile_by_object_path (profiles, cd_profile_get_object_path (profile_tmp));
        if (!ret)
    ...
    for (i = 0; i < profiles->len; i++) {
        profile_tmp = g_ptr_array_index (profiles, i);
        ret = gcm_prefs_find_widget_by_object_path (list, cd_device_get_object_path (device),
...
static void
gcm_prefs_device_expanded_changed_cb (CcColorPanel *self, gboolean is_expanded, CcColorDevice *widget)
{
    ...
    if (is_expanded) {
        ...
        for (child = gtk_widget_get_first_child (GTK_WIDGET (self->list_box)); child != NULL;
...
gcm_prefs_add_device (CcColorPanel *self, CdDevice *device)
{
    ...
    g_signal_connect_object (widget, "expanded-changed", G_CALLBACK (gcm_prefs_device_expanded_changed_cb), self,
                             G_CONNECT_SWAPPED);
    gtk_list_box_append (self->list_box, widget);
    gtk_size_group_add_widget (self->list_box_size, widget);
    ...
    g_signal_connect_object (device, "changed", G_CALLBACK (gcm_prefs_device_changed_cb), self, G_CONNECT_SWAPPED);
    gtk_list_box_invalidate_sort (self->list_box);
}
...
gcm_prefs_remove_device (CcColorPanel *self, CdDevice *device)
{
    ...
    g_signal_handlers_disconnect_by_func (device, G_CALLBACK (gcm_prefs_device_changed_cb), self);
...
gcm_prefs_update_device_list_extra_entry (CcColorPanel *self)
{
    ...
    if (first_row != NULL && gtk_list_box_get_row_at_index (self->list_box, 1) == NULL)
        cc_color_device_set_expanded (CC_COLOR_DEVICE (first_row), TRUE);
}

static void
gcm_prefs_device_added_cb (CdClient *client, CdDevice *device, CcColorPanel *self)
{
    /* add the device */
...
static void
gcm_prefs_device_removed_cb (CdClient *client, CdDevice *device, CcColorPanel *self)
{
    /* remove from the UI */
...
static void
gcm_prefs_get_devices_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
...
static void
gcm_prefs_list_box_row_selected_cb (CcColorPanel *self, GtkListBoxRow *row)
{
    if (!self->toolbar_devices || gtk_widget_in_destruction (self->toolbar_devices))
        return;
...
gcm_prefs_refresh_toolbar_buttons (CcColorPanel *self)
{
    ...
    gtk_widget_set_visible (self->toolbutton_device_default,
                            !is_device && cc_color_profile_get_is_default (CC_COLOR_PROFILE (row)));
    if (profile)
        gtk_widget_set_sensitive (self->toolbutton_device_default, !cd_profile_get_is_system_wide (profile));
    gtk_widget_set_visible (self->toolbutton_device_enable,
                            !is_device && !cc_color_profile_get_is_default (CC_COLOR_PROFILE (row)));
...
static void
gcm_prefs_list_box_row_activated_cb (CcColorPanel *self, GtkListBoxRow *row)
{
...
static void
gcm_prefs_connect_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    CcColorPanel *self;
    gboolean ret;
    g_autoptr(GError) error = NULL;
    ...
    /* get devices */
    cd_client_get_devices (self->client, cc_panel_get_cancellable (CC_PANEL (self)), gcm_prefs_get_devices_cb, self);
}
...
gcm_prefs_is_livecd (void)
{
    ...
    if (!ret) {
        g_warning ("failed to get kernel command line: %s", error->message);
        return TRUE;
    }
    return (g_strstr_len (data, -1, "liveimg") != NULL || g_strstr_len (data, -1, "casper") != NULL);
#else
    return FALSE;
#endif
}
...
static void
cc_color_panel_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    switch (property_id) {
    default:
...
static void
cc_color_panel_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    switch (property_id) {
    default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
...
static gint
cc_color_panel_sort_func (GtkListBoxRow *a, GtkListBoxRow *b, gpointer user_data)
{
...
static gboolean
cc_color_panel_treeview_quality_default_cb (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
    CdProfileQuality quality;
...
static void
gcm_prefs_calib_prepare_cb (CcColorPanel *self, GtkWidget *page)
{
...
cc_color_panel_init (CcColorPanel *self)
{
    ...
    panel_description =
        g_strdup_printf (_("Each device needs an up to date color profile to be color managed — %s."), learn_more_link);
    adw_preferences_page_set_description (self->color_page, panel_description);

    /* assign buttons */
    g_signal_connect_object (self->toolbutton_profile_add, "clicked", G_CALLBACK (gcm_prefs_profile_add_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->toolbutton_profile_remove, "clicked", G_CALLBACK (gcm_prefs_profile_remove_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->toolbutton_profile_view, "clicked", G_CALLBACK (gcm_prefs_profile_view_cb), self,
                             G_CONNECT_SWAPPED);
    ...
    list_model = G_LIST_MODEL (
        gtk_sort_list_model_new (G_LIST_MODEL (g_object_ref (self->liststore_assign)),
    ...
    g_signal_connect (list_model, "selection-changed", G_CALLBACK (gcm_prefs_profiles_listview_selection_changed_cb),
                      self);
    g_signal_connect_object (self->listview_assign, "activate", G_CALLBACK (gcm_prefs_button_assign_ok_cb), self,
                             G_CONNECT_SWAPPED);
    g_object_unref (list_model);

    g_signal_connect_object (self->toolbutton_device_default, "clicked", G_CALLBACK (gcm_prefs_default_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->toolbutton_device_enable, "clicked", G_CALLBACK (gcm_prefs_device_profile_enable_cb),
                             self, G_CONNECT_SWAPPED);
    g_signal_connect_object (self->toolbutton_device_calibrate, "clicked", G_CALLBACK (gcm_prefs_calibrate_cb), self,
                             G_CONNECT_SWAPPED);

    /* set up assign dialog */
    g_signal_connect_object (self->button_assign_ok, "clicked", G_CALLBACK (gcm_prefs_button_assign_ok_cb), self,
                             G_CONNECT_SWAPPED);

    /* setup icc profiles list */
    g_signal_connect_object (self->button_assign_import, "clicked", G_CALLBACK (gcm_prefs_button_assign_import_cb),
                             self, G_CONNECT_SWAPPED);

    /* setup the calibration helper */
    g_signal_connect_object (self->assistant_calib, "apply", G_CALLBACK (gcm_prefs_calib_apply_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->assistant_calib, "cancel", G_CALLBACK (gcm_prefs_calib_cancel_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->assistant_calib, "close", G_CALLBACK (gcm_prefs_calib_cancel_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->assistant_calib, "prepare", G_CALLBACK (gcm_prefs_calib_prepare_cb), self,
                             G_CONNECT_SWAPPED);

    /* setup the calibration helper ::TreeView */
    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (self->treeview_calib_quality));
    model = gtk_tree_view_get_model (GTK_TREE_VIEW (self->treeview_calib_quality));
    gtk_tree_model_foreach (model, cc_color_panel_treeview_quality_default_cb, selection);
    g_signal_connect_object (selection, "changed", G_CALLBACK (gcm_prefs_calib_quality_treeview_clicked_cb), self,
                             G_CONNECT_SWAPPED);
    column = gtk_tree_view_column_new ();
    renderer = gtk_cell_renderer_text_new ();
    g_object_set (renderer, "xpad", 9, "ypad", 9, NULL);
    gtk_tree_view_column_pack_start (column, renderer, TRUE);
    gtk_tree_view_column_add_attribute (column, renderer, "markup", COLUMN_CALIB_QUALITY_DESCRIPTION);
    gtk_tree_view_column_set_expand (column, TRUE);
    gtk_tree_view_append_column (GTK_TREE_VIEW (self->treeview_calib_quality), GTK_TREE_VIEW_COLUMN (column));
    column = gtk_tree_view_column_new ();
    renderer = cc_color_cell_renderer_text_new ();
    g_object_set (renderer, "xpad", 9, "ypad", 9, "is-dim-label", TRUE, NULL);
    gtk_tree_view_column_pack_start (column, renderer, TRUE);
    gtk_tree_view_column_add_attribute (column, renderer, "markup", COLUMN_CALIB_QUALITY_APPROX_TIME);
    gtk_tree_view_column_set_expand (column, FALSE);
    gtk_tree_view_append_column (GTK_TREE_VIEW (self->treeview_calib_quality), GTK_TREE_VIEW_COLUMN (column));

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (self->treeview_calib_sensor));
    g_signal_connect_object (selection, "changed", G_CALLBACK (gcm_prefs_calib_sensor_treeview_clicked_cb), self,
                             G_CONNECT_SWAPPED);
    column = gtk_tree_view_column_new ();
    renderer = gtk_cell_renderer_text_new ();
    g_object_set (renderer, "xpad", 9, "ypad", 9, NULL);
    gtk_tree_view_column_pack_start (column, renderer, TRUE);
    gtk_tree_view_column_add_attribute (column, renderer, "markup", COLUMN_CALIB_SENSOR_DESCRIPTION);
    gtk_tree_view_column_set_expand (column, TRUE);
    gtk_tree_view_append_column (GTK_TREE_VIEW (self->treeview_calib_sensor), GTK_TREE_VIEW_COLUMN (column));

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (self->treeview_calib_kind));
    g_signal_connect_object (selection, "changed", G_CALLBACK (gcm_prefs_calib_kind_treeview_clicked_cb), self,
                             G_CONNECT_SWAPPED);
    column = gtk_tree_view_column_new ();
    renderer = gtk_cell_renderer_text_new ();
    g_object_set (renderer, "xpad", 9, "ypad", 9, NULL);
    gtk_tree_view_column_pack_start (column, renderer, TRUE);
    gtk_tree_view_column_add_attribute (column, renderer, "markup", COLUMN_CALIB_KIND_DESCRIPTION);
    ...
    gtk_tree_model_filter_set_visible_column (GTK_TREE_MODEL_FILTER (model_filter), COLUMN_CALIB_KIND_VISIBLE);

    gtk_tree_view_column_set_expand (column, TRUE);
    gtk_tree_view_append_column (GTK_TREE_VIEW (self->treeview_calib_kind), GTK_TREE_VIEW_COLUMN (column));

    selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (self->treeview_calib_temp));
    g_signal_connect_object (selection, "changed", G_CALLBACK (gcm_prefs_calib_temp_treeview_clicked_cb), self,
                             G_CONNECT_SWAPPED);
    column = gtk_tree_view_column_new ();
    renderer = gtk_cell_renderer_text_new ();
    g_object_set (renderer, "xpad", 9, "ypad", 9, NULL);
    gtk_tree_view_column_pack_start (column, renderer, TRUE);
    gtk_tree_view_column_add_attribute (column, renderer, "markup", COLUMN_CALIB_TEMP_DESCRIPTION);
    gtk_tree_view_column_set_expand (column, TRUE);
    gtk_tree_view_append_column (GTK_TREE_VIEW (self->treeview_calib_temp), GTK_TREE_VIEW_COLUMN (column));
    g_signal_connect_object (self->entry_calib_title, "notify::text", G_CALLBACK (gcm_prefs_title_entry_changed_cb),
                             self, G_CONNECT_SWAPPED);

    /* use a device client array */
    self->client = cd_client_new ();
    g_signal_connect_object (self->client, "device-added", G_CALLBACK (gcm_prefs_device_added_cb), self, 0);
    g_signal_connect_object (self->client, "device-removed", G_CALLBACK (gcm_prefs_device_removed_cb), self, 0);

    /* use a listbox for the main UI */
    gtk_list_box_set_filter_func (self->list_box, cc_color_panel_filter_func, self, NULL);
    gtk_list_box_set_sort_func (self->list_box, cc_color_panel_sort_func, self, NULL);
    g_signal_connect_object (self->list_box, "row-selected", G_CALLBACK (gcm_prefs_list_box_row_selected_cb), self,
                             G_CONNECT_SWAPPED);
    g_signal_connect_object (self->list_box, "row-activated", G_CALLBACK (gcm_prefs_list_box_row_activated_cb), self,
                             G_CONNECT_SWAPPED);
    self->list_box_size = gtk_size_group_new (GTK_SIZE_GROUP_VERTICAL);

    /* connect to colord */
    cd_client_connect (self->client, cc_panel_get_cancellable (CC_PANEL (self)), gcm_prefs_connect_cb, self);

    /* use the color sensor */
    g_signal_connect_object (self->client, "sensor-added", G_CALLBACK (gcm_prefs_client_sensor_changed_cb), self, 0);
    g_signal_connect_object (self->client, "sensor-removed", G_CALLBACK (gcm_prefs_client_sensor_changed_cb), self, 0);
    ...
    g_signal_connect_object (self->button_calib_export, "clicked", G_CALLBACK (gcm_prefs_calib_export_cb), self,
                             G_CONNECT_SWAPPED);
