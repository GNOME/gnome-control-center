...
sort_regions (GtkListBoxRow *row1, GtkListBoxRow *row2, gpointer user_data)
{
    ...
    if (!cc_locale_row_get_locale_id (CC_LOCALE_ROW (row1)))
        return 1;
    if (!cc_locale_row_get_locale_id (CC_LOCALE_ROW (row2)))
        return -1;

    la = cc_locale_row_get_country (CC_LOCALE_ROW (row1));
    lb = cc_locale_row_get_country (CC_LOCALE_ROW (row2));
...
region_visible (GtkListBoxRow *row, gpointer user_data)
{
    ...
    locale_name = cc_util_normalize_casefold_and_unaccent (cc_locale_row_get_country (CC_LOCALE_ROW (row)));
...
static void
region_list_box_row_selected_cb (CcFormatChooser *self, CcLocaleRow *row)
{
...
cc_format_chooser_class_init (CcFormatChooserClass *klass)
{
    ...
    gtk_widget_class_bind_template_callback (widget_class, region_list_box_row_selected_cb);
