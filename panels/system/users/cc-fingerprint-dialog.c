...
on_fingerprint_deleted_cb (GObject *object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (error) {
        ...
        error_message =
            g_strdup_printf (_("Failed to delete fingerprint: %s"), dbus_error_to_human (self, error));
...
delete_fingerprint (GtkButton *button, gpointer user_data)
{
    ...
    cc_fprintd_device_call_delete_enrolled_finger (self->device, finger_id, self->cancellable, on_fingerprint_deleted_cb, self);
}
