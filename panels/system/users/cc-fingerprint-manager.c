...
static void
cc_fingerprint_manager_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcFingerprintManager *self = CC_FINGERPRINT_MANAGER (object);
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
...
static void
cc_fingerprint_manager_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcFingerprintManager *self = CC_FINGERPRINT_MANAGER (object);
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
...
cc_fingerprint_manager_class_init (CcFingerprintManagerClass *klass)
{
    ...
    properties[PROP_USER] =
        g_param_spec_object ("user", "User", "The user account we manage the fingerprint for", ACT_TYPE_USER,
                             G_PARAM_STATIC_STRINGS | G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

    properties[PROP_STATE] =
        g_param_spec_enum ("state", "State", "The state of the fingerprint for the user", CC_TYPE_FINGERPRINT_STATE,
                           CC_FINGERPRINT_STATE_NONE, G_PARAM_STATIC_STRINGS | G_PARAM_READABLE);
...
static void
on_device_owner_changed (CcFingerprintManager *self, GParamSpec *spec, CcFprintdDevice *device)
{
...
static void *
cached_devices_copy_cb (const void *src, void *user_data)
{
...
static void
cache_devices (CcFingerprintManager *self, GList *devices)
{
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
    CcFprintdDevice *target_device;
    ...
    g_signal_connect_object (target_device, "notify::g-name-owner", G_CALLBACK (on_device_owner_changed), self,
                             G_CONNECT_SWAPPED);
}
...
on_devices_list (GObject *object, GAsyncResult *res, gpointer user_data)
{
    ...
    for (i = 0; devices_list[i] != NULL; ++i) {
        ...
        cc_fprintd_device_proxy_new_for_bus (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, CC_FPRINTD_NAME, device_path,
                                             g_task_get_cancellable (task), on_device_proxy, g_object_ref (task));
    }
}
...
static void
fprintd_manager_connect (CcFingerprintManager *self, GAsyncReadyCallback callback, GTask *task)
{
    g_assert (G_IS_TASK (task));

    cc_fprintd_manager_proxy_new_for_bus (G_BUS_TYPE_SYSTEM, G_DBUS_PROXY_FLAGS_NONE, CC_FPRINTD_NAME,
                                          CC_FPRINTD_MANAGER_PATH, g_task_get_cancellable (task), callback, task);
}

void
cc_fingerprint_manager_get_devices (CcFingerprintManager *self, GCancellable *cancellable, GAsyncReadyCallback callback,
                                    gpointer user_data)
{
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
 ...
 */
GList *
cc_fingerprint_manager_get_devices_finish (CcFingerprintManager *self, GAsyncResult *res, GError **error)
{
...
static void
set_state (CcFingerprintManager *self, CcFingerprintState state)
{
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
...
static void
update_state_callback (GObject *object, GAsyncResult *res, gpointer user_data)
{
    CcFingerprintManager *self = CC_FINGERPRINT_MANAGER (object);
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
    g_autoptr(GError) error = NULL;
    ...
    if (error) {
        ...
        g_warning ("Impossible to update fingerprint manager state: %s", error->message);
...
on_device_list_enrolled (GObject *object, GAsyncResult *res, gpointer user_data)
{
    ...
    g_debug ("Device %s has %u enrolled fingers", cc_fprintd_device_get_name (fprintd_device), num_enrolled_fingers);
...
static void
on_manager_devices_list (GObject *object, GAsyncResult *res, gpointer user_data)
{
    CcFingerprintManager *self = CC_FINGERPRINT_MANAGER (object);
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
...
void
cc_fingerprint_manager_update_state (CcFingerprintManager *self, CcFingerprintStateUpdated callback, gpointer user_data)
{
    CcFingerprintManagerPrivate *priv = cc_fingerprint_manager_get_instance_private (self);
    ...
    cc_fingerprint_manager_get_devices (self, cancellable, on_manager_devices_list, priv->current_task);
}
