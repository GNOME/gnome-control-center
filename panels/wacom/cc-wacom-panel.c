...
static void
update_visibility (GsdDeviceManager *manager, GsdDevice *device, gpointer user_data)
{
    ...
    cc_shell_model_set_panel_visibility (cc_application_get_model (application), "wacom",
...
cc_wacom_panel_static_init_func (void)
{
    ...
    g_signal_connect (G_OBJECT (manager), "device-added", G_CALLBACK (update_visibility), NULL);
    g_signal_connect (G_OBJECT (manager), "device-removed", G_CALLBACK (update_visibility), NULL);
    update_visibility (manager, NULL, NULL);
}

static CcWacomDevice *
lookup_wacom_device (CcWacomPanel *self, const gchar *name)
{
    GHashTableIter iter;
...
highlight_widget (CcWacomPanel *self, GtkWidget *widget)
{
    ...
    if (!gtk_widget_compute_point (widget, self->scrollable, &GRAPHENE_POINT_INIT (0, 0), &p))
        return;
...
static void
cc_wacom_panel_get_property (GObject *object, guint property_id, GValue *value, GParamSpec *pspec)
{
    switch (property_id) {
    default:
...
static void
cc_wacom_panel_set_property (GObject *object, guint property_id, const GValue *value, GParamSpec *pspec)
{
    CcWacomPanel *self;
...
update_initial_state (CcWacomPanel *self)
{
    gtk_stack_set_visible_child (GTK_STACK (self->initial_state_stack),
                                 g_hash_table_size (self->devices) == 0 ? self->panel_empty_state : self->panel_view);
}
...
static void
update_highlighted_stylus (CcWacomPanel *self, CcWacomTool *stylus_to_highlight)
{
    GHashTableIter iter;
    CcWacomTool *stylus;
    CcWacomStylusPage *page;
...
static void
update_current_tool (CcWacomPanel *self, GdkDevice *device, GdkDeviceTool *tool)
{
    ...
    gsd_device = gsd_device_manager_lookup_gdk_device (device_manager, device);
    if (!gsd_device)
        return;
    ...
    cc_tablet_tool_map_add_relation (self->tablet_tool_map, wacom_device, stylus);
}

static void
on_stylus_proximity_cb (CcWacomPanel *self, double x, double y, GtkGestureStylus *gesture)
{
    GdkDevice *device;
    GdkDeviceTool *tool;
...
static void
add_known_device (CcWacomPanel *self, GsdDevice *gsd_device)
{
    ...
    if ((device_type & (GSD_DEVICE_TYPE_TOUCHSCREEN | GSD_DEVICE_TYPE_TOUCHPAD)) != 0) {
        return;
    }
    ...
    for (l = tools; l != NULL; l = l->next) {
        if (cc_wacom_tool_is_puck (l->data)) {
            cc_wacom_page_set_mouse_config_visible (CC_WACOM_PAGE (page), TRUE);
        } else {
...
static void
device_removed_cb (CcWacomPanel *self, GsdDevice *gsd_device)
{
    CcWacomDevice *device;
    GtkWidget *page;
...
static void
device_added_cb (CcWacomPanel *self, GsdDevice *device)
{
...
static void
got_osd_proxy_cb (GObject *source_object, GAsyncResult *res, gpointer data)
{
    g_autoptr(GError) error = NULL;
    CcWacomPanel *self;
...
static void
got_input_mapping_proxy_cb (GObject *source_object, GAsyncResult *res, gpointer data)
{
    g_autoptr(GError) error = NULL;
    CcWacomPanel *self;
...
cc_wacom_panel_init (CcWacomPanel *self)
{
    ...
    g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.gnome.Shell",
                              "/org/gnome/Shell/Wacom", "org.gnome.Shell.Wacom.PadOsd",
                              cc_panel_get_cancellable (CC_PANEL (self)), got_osd_proxy_cb, self);

    g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION, G_DBUS_PROXY_FLAGS_NONE, NULL, "org.gnome.Shell",
                              "/org/gnome/Mutter/InputMapping", "org.gnome.Mutter.InputMapping",
                              cc_panel_get_cancellable (CC_PANEL (self)), got_input_mapping_proxy_cb, self);
    ...
    device_manager = gsd_device_manager_get ();
    g_signal_connect_object (device_manager, "device-added", G_CALLBACK (device_added_cb), self, G_CONNECT_SWAPPED);
    g_signal_connect_object (device_manager, "device-removed", G_CALLBACK (device_removed_cb), self, G_CONNECT_SWAPPED);

    devices = gsd_device_manager_list_devices (device_manager, GSD_DEVICE_TYPE_TABLET);
