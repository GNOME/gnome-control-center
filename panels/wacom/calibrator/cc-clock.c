...
static void
cc_clock_snapshot (GtkWidget *widget, GtkSnapshot *snapshot)
{
    ...
    cairo_arc (cr, allocation.width / 2, allocation.height / 2,
               (CLOCK_RADIUS - CLOCK_LINE_WIDTH - CLOCK_LINE_PADDING) / 2, 3 * M_PI_2,
...
cc_clock_map (GtkWidget *widget)
{
    ...
    g_signal_connect_object (frame_clock, "update", G_CALLBACK (on_frame_clock_update), widget, G_CONNECT_SWAPPED);
    cc_clock_reset (CC_CLOCK (widget));
}

static void
cc_clock_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
cc_clock_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcClock *clock = CC_CLOCK (object);
...
static void
cc_clock_measure (GtkWidget *widget, GtkOrientation orientation, gint for_size, gint *minimum, gint *natural,
...
cc_clock_class_init (CcClockClass *klass)
{
    ...
    signals[FINISHED] =
        g_signal_new ("finished", CC_TYPE_CLOCK, G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL, G_TYPE_NONE, 0);

    props[PROP_DURATION] = g_param_spec_uint ("duration", "Duration", "Duration", 0, G_MAXUINT, 0,
...
{
    return g_object_new (CC_TYPE_CLOCK, "duration", duration, NULL);
}
