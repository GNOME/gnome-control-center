...
G_DEFINE_FINAL_TYPE_WITH_CODE (CcWacomDevice, cc_wacom_device, G_TYPE_OBJECT,
                               G_IMPLEMENT_INTERFACE (G_TYPE_INITABLE, cc_wacom_device_initable_iface_init))
...
static void
cc_wacom_device_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
cc_wacom_device_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    CcWacomDevice *device = CC_WACOM_DEVICE (object);
...
cc_wacom_device_class_init (CcWacomDeviceClass *klass)
{
    ...
    props[PROP_DEVICE] =
        g_param_spec_object ("device", "device", "device", GSD_TYPE_DEVICE, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);

    g_object_class_install_properties (object_class, N_PROPS, props);
}

static gboolean
cc_wacom_device_initable_init (GInitable *initable, GCancellable *cancellable, GError **error)
{
    ...
    if (device->is_fallback) {
        ...
        device->description =
            g_strdup_printf (_("This device is unknown and may present wrong capabilities — %s."), learn_more_link);
    }
...
{
    return g_initable_new (CC_TYPE_WACOM_DEVICE, NULL, NULL, "device", device, NULL);
}
...
cc_wacom_device_new_fake (const gchar *name)
{
    ...
    device = g_object_new (CC_TYPE_WACOM_DEVICE, NULL);

    wacom_error = libwacom_error_new ();
    wacom_device = libwacom_new_from_name (cc_wacom_device_database_get (), name, wacom_error);
...
const gint *
cc_wacom_device_get_supported_tools (CcWacomDevice *device, gint *n_tools)
{
    *n_tools = 0;
...
find_output_by_edid (CcDisplayConfig *config, const gchar *vendor, const gchar *product, const gchar *serial,
                     const gchar *name)
{
    ...
    for (l = monitors; l; l = l->next) {
        ...
        g_debug ("Checking for match between '%s','%s','%s', '%s' and '%s','%s','%s', '%s'", vendor, product, serial,
                 name, o_vendor, o_product, o_serial, o_name);

        match = (g_strcmp0 (vendor, o_vendor) == 0) && (g_strcmp0 (product, o_product) == 0)
                && (g_strcmp0 (serial, o_serial) == 0) && (g_strcmp0 (name, o_name) == 0);
    ...
    if (retval == NULL)
        g_debug ("Did not find a matching output for EDID '%s,%s,%s'", vendor, product, serial);

    return retval;
}

static CcDisplayMonitor *
find_output (CcDisplayConfig *config, CcWacomDevice *device)
{
...
CcDisplayMonitor *
cc_wacom_device_get_output (CcWacomDevice *device, CcDisplayConfig *config)
{
...
void
cc_wacom_device_set_output (CcWacomDevice *device, CcDisplayMonitor *monitor)
{
    g_autoptr(GSettings) settings = NULL;
...
GSettings *
cc_wacom_device_get_button_settings (CcWacomDevice *device, guint button)
{
    ...
    settings = g_settings_new_with_path ("org.gnome.desktop.peripherals.tablet.pad-button", button_path);
