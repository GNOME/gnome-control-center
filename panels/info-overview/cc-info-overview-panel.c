...
get_renderer_from_switcheroo (void)
{
  ...
  variant = g_dbus_proxy_get_cached_property (switcheroo_proxy, "Renderers");

  if (!variant)
    {
      g_debug ("Unable to retrieve net.hadess.SwitcherooControl.Renderers property, the daemon is likely not running");
      return NULL;
    }
...
get_graphics_hardware_string (void)
{
  ...
  for (i = 0; renderers[i] != NULL; i++)
    {
      g_string_append (s, renderers[i]);
      if (renderers[i + 1] != NULL)
        g_string_append (s, " / ");
    }
