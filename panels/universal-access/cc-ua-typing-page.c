...
G_DEFINE_FINAL_TYPE (CcUaTypingPage, cc_ua_typing_page, ADW_TYPE_NAVIGATION_PAGE)
 ...
 */
static gboolean
get_inverted_mapping (GValue *value, GVariant *variant, gpointer user_data)
{
...
static GVariant *
set_inverted_mapping (const GValue *value, const GVariantType *expected_type, gpointer user_data)
{
    int val;
...
static void
bind_scale_with_mapping (CcUaTypingPage *self, GSettings *settings, const char *settings_key, GtkScale *scale)
{
    GtkAdjustment *adj;

    adj = gtk_range_get_adjustment (GTK_RANGE (scale));

    g_settings_bind_with_mapping (settings, settings_key, adj, "value", G_SETTINGS_BIND_DEFAULT, get_inverted_mapping,
                                  set_inverted_mapping, self, NULL);
}
...
cc_ua_typing_page_init (CcUaTypingPage *self)
{
    ...
    g_settings_bind (self->application_settings, KEY_SCREEN_KEYBOARD_ENABLED, self->screen_kb_row, "active",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_KEYBOARD_TOGGLE, self->shortcuts_by_kb_row, "active",
                     G_SETTINGS_BIND_DEFAULT);

    /* Text Cursor */
    g_settings_bind (self->interface_settings, KEY_CURSOR_BLINKING, self->cursor_blink_row, "active",
                     G_SETTINGS_BIND_DEFAULT);
    bind_scale_with_mapping (self, self->interface_settings, KEY_CURSOR_BLINKING_TIME, self->blink_time_scale);

    /* Repeat keys */
    g_settings_bind (self->kb_desktop_settings, KEY_REPEAT_KEYS, self->repeat_keys_row, "enable-expansion",
                     G_SETTINGS_BIND_DEFAULT);

    bind_scale_with_mapping (self, self->kb_desktop_settings, KEY_REPEAT_INTERVAL, self->repeat_speed_scale);
    g_settings_bind (self->kb_desktop_settings, KEY_REPEAT_DELAY,
                     gtk_range_get_adjustment (GTK_RANGE (self->repeat_delay_scale)), "value", G_SETTINGS_BIND_DEFAULT);

    /* Sticky keys */
    g_settings_bind (self->kb_settings, KEY_STICKYKEYS_ENABLED, self->sticky_keys_row, "enable-expansion",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_STICKYKEYS_TWO_KEY_OFF, self->disable_sticky_keys_row, "active",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_STICKYKEYS_MODIFIER_BEEP, self->beep_sticky_keys_row, "active",
                     G_SETTINGS_BIND_NO_SENSITIVITY);

    /* Slow Keys */
    g_settings_bind (self->kb_settings, KEY_SLOWKEYS_ENABLED, self->slow_keys_row, "enable-expansion",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_SLOWKEYS_DELAY,
                     gtk_range_get_adjustment (GTK_RANGE (self->slow_keys_delay_scale)), "value",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_SLOWKEYS_BEEP_PRESS, self->slow_keys_beep_row, "active",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_SLOWKEYS_BEEP_ACCEPT, self->slow_keys_beep_accept_row, "active",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_SLOWKEYS_BEEP_REJECT, self->slow_keys_beep_reject_row, "active",
                     G_SETTINGS_BIND_DEFAULT);

    /* Bounce Keys */
    g_settings_bind (self->kb_settings, KEY_BOUNCEKEYS_ENABLED, self->bounce_keys_row, "enable-expansion",
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_BOUNCEKEYS_DELAY,
                     ...
                     G_SETTINGS_BIND_DEFAULT);
    g_settings_bind (self->kb_settings, KEY_BOUNCEKEYS_BEEP_REJECT, self->bounce_keys_beep_reject_row, "active",
                     G_SETTINGS_BIND_NO_SENSITIVITY);
}
