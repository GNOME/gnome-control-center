...
{
  if (self->current_app_id)
    set_shortcuts_allowed (self, cc_toggle_row_get_allowed (CC_TOGGLE_ROW (self->shortcuts)));
}
  ...
  if (app_id != NULL)
    {
      ...
      gtk_widget_set_visible (self->shortcuts, set);
      cc_toggle_row_set_allowed (CC_TOGGLE_ROW (self->shortcuts), allowed);
    }
  else
    {
      gtk_widget_hide (self->shortcuts);
    }
