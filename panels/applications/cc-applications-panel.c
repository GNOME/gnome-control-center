...
static void update_handler_dialog (CcApplicationsPanel *self,
                                   GAppInfo            *info);

static void update_usage_section (CcApplicationsPanel *self,
                                  GAppInfo            *info);
                      ...
                      const gchar         *app_id)
{
  ...
  for (guint i = 0; i < json_array_get_length (plugs); i++) {
    ...
    const gchar * const hidden_interfaces[] = {
      "content",
