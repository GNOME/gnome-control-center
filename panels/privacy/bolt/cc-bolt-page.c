...
/* initialization */
static void bolt_client_ready (GObject *source, GAsyncResult *res, gpointer user_data);

/* panel functions */
static void cc_bolt_page_set_no_thunderbolt (CcBoltPage *self, const char *custom_msg);

static void cc_bolt_page_name_owner_changed (CcBoltPage *self);

static CcBoltDeviceEntry *cc_bolt_page_add_device (CcBoltPage *self, BoltDevice *dev);

static void cc_bolt_page_del_device_entry (CcBoltPage *self, CcBoltDeviceEntry *entry);

static void cc_bolt_page_authmode_sync (CcBoltPage *self);

static void cc_panel_list_box_migrate (CcBoltPage *self, GtkListBox *from, GtkListBox *to, CcBoltDeviceEntry *entry);

/* bolt client signals */
static void on_bolt_name_owner_changed_cb (GObject *object, GParamSpec *pspec, gpointer user_data);

static void on_bolt_device_added_cb (BoltClient *cli, const char *path, CcBoltPage *self);

static void on_bolt_device_removed_cb (BoltClient *cli, const char *opath, CcBoltPage *self);

static void on_bolt_notify_authmode_cb (GObject *gobject, GParamSpec *pspec, gpointer user_data);

/* panel signals */
static gboolean on_authmode_state_set_cb (CcBoltPage *self, gboolean state, GtkSwitch *toggle);

static void on_device_entry_row_activated_cb (CcBoltPage *self, GtkListBoxRow *row);

static void on_device_entry_status_changed_cb (CcBoltDeviceEntry *entry, BoltStatus new_status, CcBoltPage *self);
/* polkit */
static void on_permission_ready (GObject *source_object, GAsyncResult *res, gpointer user_data);

static void on_permission_notify_cb (GPermission *permission, GParamSpec *pspec, CcBoltPage *self);

G_DEFINE_FINAL_TYPE (CcBoltPage, cc_bolt_page, ADW_TYPE_NAVIGATION_PAGE)

static void
update_visibility (BoltClient *client, const char *path, gpointer user_data)
{
...
static void
on_visibility_client_ready (GObject *source, GAsyncResult *res, gpointer user_data)
{
    BoltClient *client;
    CcBoltPage *self;
    ...
    g_signal_connect_object (client, "device-added", G_CALLBACK (update_visibility), self, 0);
    g_signal_connect_object (client, "device-removed", G_CALLBACK (update_visibility), self, 0);
    update_visibility (client, NULL, self);
}

static void
bolt_client_ready (GObject *source, GAsyncResult *res, gpointer user_data)
{
    ...
    g_signal_connect_object (client, "notify::g-name-owner", G_CALLBACK (on_bolt_name_owner_changed_cb), self, 0);

    g_signal_connect_object (client, "device-added", G_CALLBACK (on_bolt_device_added_cb), self, 0);

    g_signal_connect_object (client, "device-removed", G_CALLBACK (on_bolt_device_removed_cb), self, 0);

    g_signal_connect_object (client, "notify::auth-mode", G_CALLBACK (on_bolt_notify_authmode_cb), self, 0);

    /* Treat security-level changes, which should rarely happen, as
     * if the name owner changed, i.e. as if boltd got restarted */
    g_signal_connect_object (client, "notify::security-level", G_CALLBACK (on_bolt_name_owner_changed_cb), self, 0);
    ...
    g_object_bind_property (self->authmode_switch, "active", self->devices_box, "sensitive",
                            G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);

    g_object_bind_property (self->authmode_switch, "active", self->pending_box, "sensitive",
                            G_BINDING_DEFAULT | G_BINDING_SYNC_CREATE);
...
static gboolean
devices_table_transfer_entry (GHashTable *from, GHashTable *to, gconstpointer key)
{
    gpointer k, v;
    gboolean found;
...
static void
devices_table_clear_entries (GHashTable *table, CcBoltPage *self)
{
    GHashTableIter iter;
    gpointer key, value;
...
static GtkWidget *
cc_bolt_page_box_for_listbox (CcBoltPage *self, GtkListBox *lstbox)
{
...
cc_bolt_page_add_device (CcBoltPage *self, BoltDevice *dev)
{
    ...
    g_signal_connect_object (entry, "status-changed", G_CALLBACK (on_device_entry_status_changed_cb), self, 0);
...
static void
cc_bolt_page_del_device_entry (CcBoltPage *self, CcBoltDeviceEntry *entry)
{
    BoltDevice *dev;
    GtkWidget *box;
    GtkWidget *p;
...
cc_bolt_page_authmode_sync (CcBoltPage *self)
{
    ...
    adw_preferences_row_set_title (ADW_PREFERENCES_ROW (self->direct_access_row),
                                   enabled ? _("Allow direct access to devices such as docks and external GPUs.")
                                           : _("Only USB and Display Port devices can attach."));
}

static void
cc_panel_list_box_migrate (CcBoltPage *self, GtkListBox *from, GtkListBox *to, CcBoltDeviceEntry *entry)
{
    GtkWidget *from_box;
    GtkWidget *to_box;
    gboolean show;
...
/* bolt client signals */
static void
cc_bolt_page_set_no_thunderbolt (CcBoltPage *self, const char *msg)
{
    if (!msg) {
...
cc_bolt_page_name_owner_changed (CcBoltPage *self)
{
    ...
    if (self->permission) {
        ...
        polkit_permission_new ("org.freedesktop.bolt.manage", NULL, self->cancellable, on_permission_ready,
                               g_object_ref (self));
    }
...
/* bolt client signals */
static void
on_bolt_name_owner_changed_cb (GObject *object, GParamSpec *pspec, gpointer user_data)
{
    cc_bolt_page_name_owner_changed (CC_BOLT_PAGE (user_data));
}

static void
on_bolt_device_added_cb (BoltClient *cli, const char *path, CcBoltPage *self)
{
    g_autoptr(GError) err = NULL;
    GDBusConnection *bus;
    BoltDevice *dev;
...
static void
on_bolt_device_removed_cb (BoltClient *cli, const char *path, CcBoltPage *self)
{
    CcBoltDeviceEntry *entry;
...
static void
on_bolt_notify_authmode_cb (GObject *gobject, GParamSpec *pspec, gpointer user_data)
{
...
static void
on_authmode_ready (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
...
static gboolean
on_authmode_state_set_cb (CcBoltPage *self, gboolean enable, GtkSwitch *toggle)
{
    BoltClient *client = self->client;
    BoltAuthMode mode;
...
static void
on_device_entry_row_activated_cb (CcBoltPage *self, GtkListBoxRow *row)
{
...
static void
on_device_entry_status_changed_cb (CcBoltDeviceEntry *entry, BoltStatus new_status, CcBoltPage *self)
{
...
static void
on_permission_ready (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    g_signal_connect_object (permission, "notify", G_CALLBACK (on_permission_notify_cb), self, G_CONNECT_AFTER);
    ...
    gtk_widget_set_visible (GTK_WIDGET (self->headerbar_box), bolt_streq (name, "devices-listing"));
}

static void
on_permission_notify_cb (GPermission *permission, GParamSpec *pspec, CcBoltPage *self)
{
...
static gint
device_entries_sort_by_recency_cb (GtkListBoxRow *a_row, GtkListBoxRow *b_row, gpointer user_data)
{
...
static gint
device_entries_sort_by_syspath_cb (GtkListBoxRow *a_row, GtkListBoxRow *b_row, gpointer user_data)
{
    CcBoltDeviceEntry *a_entry = CC_BOLT_DEVICE_ENTRY (a_row);
    CcBoltDeviceEntry *b_entry = CC_BOLT_DEVICE_ENTRY (b_row);
    BoltDevice *a = cc_bolt_device_entry_get_device (a_entry);
...
cc_bolt_page_class_init (CcBoltPageClass *klass)
{
    ...
    gtk_widget_class_set_template_from_resource (widget_class,
                                                 "/org/gnome/control-center/privacy/bolt/cc-bolt-page.ui");
...
cc_bolt_page_init (CcBoltPage *self)
{
    ...
    gtk_list_box_set_sort_func (self->devices_list, device_entries_sort_by_recency_cb, self, NULL);

    gtk_list_box_set_sort_func (self->pending_list, device_entries_sort_by_syspath_cb, self, NULL);
