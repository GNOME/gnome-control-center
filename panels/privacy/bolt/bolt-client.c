...
static void handle_dbus_device_added (GObject *self, GDBusProxy *bus_proxy, GVariant *params);
static void handle_dbus_device_removed (GObject *self, GDBusProxy *bus_proxy, GVariant *params);
...
G_DEFINE_FINAL_TYPE (BoltClient, bolt_client, BOLT_TYPE_PROXY);

static void
bolt_client_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
bolt_client_class_init (BoltClientClass *klass)
{
    ...
    props[PROP_VERSION] =
        g_param_spec_uint ("version", "Version", NULL, 0, G_MAXUINT, 0, G_PARAM_READABLE | G_PARAM_STATIC_NAME);

    props[PROP_PROBING] =
        g_param_spec_boolean ("probing", "Probing", NULL, FALSE, G_PARAM_READABLE | G_PARAM_STATIC_NAME);

    props[PROP_SECURITY] = g_param_spec_enum ("security-level", "SecurityLevel", NULL, BOLT_TYPE_SECURITY,
                                              BOLT_SECURITY_UNKNOWN, G_PARAM_READABLE | G_PARAM_STATIC_NAME);

    props[PROP_AUTHMODE] = g_param_spec_flags ("auth-mode", "AuthMode", NULL, BOLT_TYPE_AUTH_MODE, BOLT_AUTH_ENABLED,
                                               G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);

    g_object_class_install_properties (gobject_class, PROP_LAST, props);

    /* signals */
    signals[SIGNAL_DEVICE_ADDED] = g_signal_new ("device-added", G_TYPE_FROM_CLASS (gobject_class), G_SIGNAL_RUN_LAST,
                                                 0, NULL, NULL, NULL, G_TYPE_NONE, 1, G_TYPE_STRING);

    signals[SIGNAL_DEVICE_REMOVED] =
        g_signal_new ("device-removed", G_TYPE_FROM_CLASS (gobject_class), G_SIGNAL_RUN_LAST, 0, NULL, NULL, NULL,
...
bolt_client_new (GError **error)
{
    ...
    cli = g_initable_new (BOLT_TYPE_CLIENT, NULL, error, "g-flags", G_DBUS_PROXY_FLAGS_NONE, "g-connection", bus,
                          "g-name", BOLT_DBUS_NAME, "g-object-path", BOLT_DBUS_PATH, "g-interface-name",
                          BOLT_DBUS_INTERFACE, NULL);
...
static void
got_the_client (GObject *source, GAsyncResult *res, gpointer user_data)
{
    GError *error = NULL;
...
static void
got_the_bus (GObject *source, GAsyncResult *res, gpointer user_data)
{
    GError *error = NULL;
    GTask *task = user_data;
    ...
    g_async_initable_new_async (BOLT_TYPE_CLIENT, G_PRIORITY_DEFAULT, cancellable, got_the_client, task, "g-flags",
                                G_DBUS_PROXY_FLAGS_NONE, "g-connection", bus, "g-name", BOLT_DBUS_NAME, "g-object-path",
                                BOLT_DBUS_PATH, "g-interface-name", BOLT_DBUS_INTERFACE, NULL);
    g_object_unref (bus);
}

void
bolt_client_new_async (GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    GTask *task;
...
BoltClient *
bolt_client_new_finish (GAsyncResult *res, GError **error)
{
...
GPtrArray *
bolt_client_list_devices (BoltClient *client, GCancellable *cancel, GError **error)
{
    ...
    val =
        g_dbus_proxy_call_sync (G_DBUS_PROXY (client), "ListDevices", NULL, G_DBUS_CALL_FLAGS_NONE, -1, cancel, error);
    if (val == NULL)
        return NULL;
...
BoltDevice *
bolt_client_get_device (BoltClient *client, const char *uid, GCancellable *cancel, GError **error)
{
    g_autoptr(GVariant) val = NULL;
    ...
    val = g_dbus_proxy_call_sync (G_DBUS_PROXY (client), "DeviceByUid", g_variant_new ("(s)", uid),
...
BoltDevice *
bolt_client_enroll_device (BoltClient *client, const char *uid, BoltPolicy policy, BoltAuthCtrl flags, GError **error)
{
    g_autoptr(GVariant) val = NULL;
    g_autoptr(GError) err = NULL;
    ...
    val =
        g_dbus_proxy_call_sync (G_DBUS_PROXY (client), "EnrollDevice", params, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &err);
...
void
bolt_client_enroll_device_async (BoltClient *client, const char *uid, BoltPolicy policy, BoltAuthCtrl flags,
                                 GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    params = g_variant_new ("(sss)", uid, pstr, fstr);
    g_dbus_proxy_call (G_DBUS_PROXY (client), "EnrollDevice", params, G_DBUS_CALL_FLAGS_NONE, -1, cancellable, callback,
                       user_data);
}

gboolean
bolt_client_enroll_device_finish (BoltClient *client, GAsyncResult *res, char **path, GError **error)
{
    g_autoptr(GError) err = NULL;
    g_autoptr(GVariant) val = NULL;
...
static OpData *
op_data_new_enroll (const char *uid, const char *policy, const char *flags)
{
    GVariant *params;
    OpData *op;
...
static OpData *
op_data_new_authorize (const char *uid, const char *flags)
{
    OpData *op = NULL;
    GVariant *params;
    char *path;
...
static void allop_one_done (GObject *source_object, GAsyncResult *res, gpointer user_data);
...
allop_continue (BoltClient *client, GTask *task, GQueue *ops)
{
    ...
    g_dbus_connection_call (bus, BOLT_DBUS_NAME, op->path, op->iface, op->method, op->params, NULL,
                            G_DBUS_CALL_FLAGS_NONE, -1, cancel, allop_one_done, task);
...
static void
allop_one_done (GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    g_autoptr(GVariant) val = NULL;
    BoltClient *client;
    GDBusConnection *bus;
...
void
bolt_client_enroll_all_async (BoltClient *client, GPtrArray *uuids, BoltPolicy policy, BoltAuthCtrl flags,
                              GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    g_autofree char *fstr = NULL;
    GError *err = NULL;
...
gboolean
bolt_client_enroll_all_finish (BoltClient *client, GAsyncResult *res, GError **error)
{
    g_autoptr(GError) err = NULL;
    gboolean ok;
...
void
bolt_client_authorize_all_async (BoltClient *client, GPtrArray *uuids, BoltAuthCtrl flags, GCancellable *cancellable,
...
gboolean
bolt_client_authorize_all_finish (BoltClient *client, GAsyncResult *res, GError **error)
{
    g_autoptr(GError) err = NULL;
    gboolean ok;
...
void
bolt_client_connect_all_async (BoltClient *client, GPtrArray *devices, BoltPolicy policy, BoltAuthCtrl flags,
                               GCancellable *cancellable, GAsyncReadyCallback callback, gpointer user_data)
{
    g_autofree char *fstr = NULL;
    GError *err = NULL;
...
gboolean
bolt_client_connect_all_finish (BoltClient *client, GAsyncResult *res, GError **error)
{
    g_autoptr(GError) err = NULL;
    gboolean ok;
...
gboolean
bolt_client_forget_device (BoltClient *client, const char *uid, GError **error)
{
    g_autoptr(GVariant) val = NULL;
    g_autoptr(GError) err = NULL;
    ...
    val = g_dbus_proxy_call_sync (G_DBUS_PROXY (client), "ForgetDevice", g_variant_new ("(s)", uid),
...
void
bolt_client_forget_device_async (BoltClient *client, const char *uid, GCancellable *cancellable,
                                 GAsyncReadyCallback callback, gpointer user_data)
{
    g_return_if_fail (BOLT_IS_CLIENT (client));

    g_dbus_proxy_call (G_DBUS_PROXY (client), "ForgetDevice", g_variant_new ("(s)", uid), G_DBUS_CALL_FLAGS_NONE, -1,
                       cancellable, callback, user_data);
}

gboolean
bolt_client_forget_device_finish (BoltClient *client, GAsyncResult *res, GError **error)
{
    g_autoptr(GVariant) val = NULL;
    g_autoptr(GError) err = NULL;
...
void
bolt_client_set_authmode_async (BoltClient *client, BoltAuthMode mode, GCancellable *cancellable,
                                GAsyncReadyCallback callback, gpointer user_data)
{
    ...
    bolt_proxy_set_property_async (BOLT_PROXY (client), g_param_spec_get_nick (pspec), g_variant_new_string (str),
                                   cancellable, callback, user_data);
}

gboolean
bolt_client_set_authmode_finish (BoltClient *client, GAsyncResult *res, GError **error)
{
...
/* utility functions */
static gint
device_sort_by_syspath (gconstpointer ap, gconstpointer bp, gpointer data)
{
...
void
bolt_devices_sort_by_syspath (GPtrArray *devices, gboolean reverse)
{
    ...
    g_ptr_array_sort_with_data (devices, device_sort_by_syspath, sort_order);
}
