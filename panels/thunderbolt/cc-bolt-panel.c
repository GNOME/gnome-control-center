...
/* bolt client signals */
static void     on_bolt_name_owner_changed (GObject    *object,
                                            GParamSpec *pspec,
                                            gpointer    user_data);

static void     on_bolt_device_added (BoltClient  *cli,
                                      const char  *path,
                                      CcBoltPanel *panel);

static void     on_bolt_device_removed (BoltClient  *cli,
                                        const char  *opath,
                                        CcBoltPanel *panel);

static void     on_bolt_authmode_changed (GObject    *gobject,
                                          GParamSpec *pspec,
                                          gpointer    user_data);

/* panel signals */
static gboolean on_authmode_switched (CcBoltPanel *panel,
                                      gboolean     state,
                                      GtkSwitch   *toggle);

static void     on_device_entry_activated (CcBoltPanel   *panel,
                                           GtkListBoxRow *row);

static gboolean  on_device_dialog_delete (GtkWidget   *widget,
                                          GdkEvent    *event,
                                          CcBoltPanel *panel);

static void     on_entry_status_changed (CcBoltDeviceEntry *entry,
...
static gint         device_entries_sort (GtkListBoxRow *a_row,
                                         GtkListBoxRow *b_row,
                                         gpointer       user_data);
...
#define RESOURCE_PANEL_UI "/org/gnome/control-center/thunderbolt/panel.ui"
...
cc_bolt_panel_class_init (CcBoltPanelClass *klass)
{
  ...
  gtk_widget_class_bind_template_child (widget_class, CcBoltPanel, authmode_stack);
  ...
  gtk_widget_class_bind_template_callback (widget_class, on_authmode_switched);
  gtk_widget_class_bind_template_callback (widget_class, on_device_entry_activated);
}
...
cc_bolt_panel_init (CcBoltPanel *panel)
{
  ...
  g_signal_connect_object (panel->device_dialog, "delete-event",
                           G_CALLBACK (on_device_dialog_delete),
                           panel, 0);
                   ...
                   gpointer      user_data)
{
  ...
  g_signal_connect_object (client, "notify::g-name-owner",
                           G_CALLBACK (on_bolt_name_owner_changed),
                           panel, 0);

  g_signal_connect_object (client, "device-added",
                           G_CALLBACK (on_bolt_device_added),
                           panel, 0);

  g_signal_connect_object (client, "device-removed",
                           G_CALLBACK (on_bolt_device_removed),
                           panel, 0);

  g_signal_connect_object (client, "notify::auth-mode",
                           G_CALLBACK (on_bolt_authmode_changed),
                           panel, 0);
                          ...
                          BoltDevice  *dev)
{
  ...
  g_signal_connect_object (entry, "status-changed",
                           G_CALLBACK (on_entry_status_changed),
                           panel, 0);
...
cc_bolt_panel_name_owner_changed (CcBoltPanel *panel)
{
  ...
  if (name_owner == NULL)
    {
      text = _("The thunderbolt subsystem (boltd) is not running.");
  ...
  switch (sl)
    ...
    default:
      text = _("Thunderbolt could not be detected.\n"
...
static void
on_bolt_name_owner_changed (GObject    *object,
                            GParamSpec *pspec,
                            gpointer    user_data)
{
...
static void
on_bolt_device_added (BoltClient  *cli,
                      const char  *path,
                      CcBoltPanel *panel)
{
  g_autoptr(GError) err = NULL;
...
static void
on_bolt_device_removed (BoltClient  *cli,
                        const char  *path,
                        CcBoltPanel *panel)
{
  CcBoltDeviceEntry *entry;
...
static void
on_bolt_authmode_changed (GObject    *gobject,
                          GParamSpec *pspec,
                          gpointer    user_data)
{
  CcBoltPanel *panel = CC_BOLT_PANEL (user_data);
                   ...
                   gpointer      user_data)
{
  ...
  gtk_stack_set_visible_child_name (panel->authmode_stack, "authmode-switch");
}

static gboolean
on_authmode_switched (CcBoltPanel *panel,
                      gboolean     enable,
                      GtkSwitch   *toggle)
{
  BoltClient *client = panel->client;
  ...
  gtk_stack_set_visible_child_name (panel->authmode_stack, "authmode-spinner");
...
static void
on_device_entry_activated (CcBoltPanel   *panel,
                           GtkListBoxRow *row)
{
  CcBoltDeviceEntry *entry;
  BoltDevice *device;
...
static gboolean
on_device_dialog_delete (GtkWidget   *widget,
                         GdkEvent    *event,
                         CcBoltPanel *panel)
{
...
static void
on_entry_status_changed (CcBoltDeviceEntry *entry,
                         BoltStatus         new_status,
                         CcBoltPanel       *panel)
{
...
static gint
device_entries_sort (GtkListBoxRow *a_row,
                     GtkListBoxRow *b_row,
                     gpointer       user_data)
{
