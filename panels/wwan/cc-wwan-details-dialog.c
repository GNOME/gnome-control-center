...
G_DEFINE_TYPE (CcWwanDetailsDialog, cc_wwan_details_dialog, HDY_TYPE_DIALOG)
...
cc_wwan_details_signal_changed_cb (CcWwanDetailsDialog *self)
{
  ...
  str = cc_wwan_device_dup_operator_name (self->device);
  if (str)
    gtk_label_set_label (self->operator_name, str);
  g_free (str);

  str = cc_wwan_device_dup_network_type_string (self->device);
  if (str)
    gtk_label_set_label (self->network_type, str);
  g_free (str);

  str = cc_wwan_device_dup_signal_string (self->device);
  if (str)
    gtk_label_set_label (self->signal_strength, str);
  g_free (str);
...
cc_wwan_details_dialog_set_property (GObject      *object,
                                     ...
                                     GParamSpec   *pspec)
{
  CcWwanDetailsDialog *self = (CcWwanDetailsDialog *)object;
...
static void
cc_wwan_details_dialog_finalize (GObject *object)
{
  CcWwanDetailsDialog *self = (CcWwanDetailsDialog *)object;

  g_clear_object (&self->device);

  G_OBJECT_CLASS (cc_wwan_details_dialog_parent_class)->finalize (object);
}
...
cc_wwan_details_dialog_class_init (CcWwanDetailsDialogClass *klass)
{
  ...
  object_class->finalize = cc_wwan_details_dialog_finalize;
