...
cc_wwan_device_state_changed_cb (CcWwanDevice *self)
{
  ...
  g_assert (CC_IS_WWAN_DEVICE (self));
...
{
  g_assert (CC_IS_WWAN_DEVICE (self));
  g_assert (NM_IS_DEVICE (nm_device));

  if (!NM_IS_DEVICE_MODEM (nm_device))
    return;

  if(!self->sim || !cc_wwan_device_is_nm_device (self, nm_device))
    return;

  self->wwan_data = cc_wwan_data_new (self->mm_object, self->nm_client);
                             ...
                             GParamSpec *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_OPERATOR_NAME:
      g_value_take_string (value, cc_wwan_device_dup_operator_name (self));
      break;
...
static void
cc_wwan_device_finalize (GObject *object)
{
  CcWwanDevice *self = (CcWwanDevice *)object;
  ...
  G_OBJECT_CLASS (cc_wwan_device_parent_class)->finalize (object);
}
...
cc_wwan_device_class_init (CcWwanDeviceClass *klass)
{
  ...
  object_class->finalize = cc_wwan_device_finalize;
  ...
  if (self->sim)
    {
      ...
      self->wwan_data = cc_wwan_data_new (mm_object, self->nm_client);
    }
                                 ...
                                 gpointer             user_data)
{
  ...
  if (error)
    ...
    g_warning ("Error: Not allowed to change Modem settings");
  else
                                 ...
                                 gpointer      user_data)
{
  ...
  g_assert (MM_IS_MODEM_3GPP (modem_3gpp));
  g_assert (G_IS_ASYNC_RESULT (result));
  g_assert (G_IS_TASK (task));
...
/**
 * cc_wwan_device_dup_operator_name:
 * @self: a #CcWwanDevice
 *
 ...
 */
gchar *
cc_wwan_device_dup_operator_name (CcWwanDevice *self)
{
  g_return_val_if_fail (CC_IS_WWAN_DEVICE (self), NULL);
  ...
  return mm_modem_3gpp_dup_operator_name (self->modem_3gpp);
}
