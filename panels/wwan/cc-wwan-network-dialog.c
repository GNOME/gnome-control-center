...
G_DEFINE_TYPE (CcWwanNetworkDialog, cc_wwan_network_dialog, HDY_TYPE_DIALOG)
                                ...
                                gchar               *operator_code)
{
  ...
  g_assert (CC_WWAN_NETWORK_DIALOG (self));
  ...
  g_object_set_data_full (G_OBJECT (row), "operator-name", operator_name, g_free);
  g_object_set_data_full (G_OBJECT (row), "operator-code", operator_code, g_free);
...
static void
cc_wwan_network_dialog_update_current (CcWwanNetworkDialog *self)
{
  GtkWidget *row;
  gchar *operator_name;
  ...
  operator_name = cc_wwan_device_dup_operator_name (self->device);
  ...
  row = cc_wwan_network_dialog_row_new (self, operator_name, g_strdup (""));
  self->selected_row = row;
  gtk_container_add (GTK_CONTAINER (self->operator_list_box), row);
  ...
  for (item = self->operator_list; item; item = item->next)
    {
      ...
      row = cc_wwan_network_dialog_row_new (self, g_strdup (operator_name),
                                            g_strdup (operator_code));
      gtk_container_add (GTK_CONTAINER (self->operator_list_box), row);
    }
}
...
cc_wwan_network_scan_complete_cb (GObject      *object,
                                  ...
                                  gpointer      user_data)
{
  CcWwanNetworkDialog *self = user_data;
...
cc_wwan_network_dialog_show (GtkWidget *widget)
{
  ...
  cc_wwan_network_dialog_update_current (self);
...
static void
cc_wwan_network_dialog_finalize (GObject *object)
{
  CcWwanNetworkDialog *self = (CcWwanNetworkDialog *)object;
  ...
  G_OBJECT_CLASS (cc_wwan_network_dialog_parent_class)->finalize (object);
}
                                 ...
                                 gint       response_id)
{
  ...
  operator_code = g_object_get_data (G_OBJECT (self->selected_row), "operator-code");
  g_assert (operator_code);
  cc_wwan_device_register_network (self->device, operator_code, NULL, NULL, self);
}
...
cc_wwan_network_dialog_class_init (CcWwanNetworkDialogClass *klass)
{
  ...
  object_class->finalize = cc_wwan_network_dialog_finalize;
