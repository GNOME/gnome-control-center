...
static GtkWidget *
cc_wwan_panel_new_data_row (CcWwanDevice *device,
                            CcWwanPanel  *self)
{
...
static void
cc_wwan_panel_update_data_selection (GtkWidget *widget,
                                     CcWwanPanel *self)
{
  CcWwanDevice *device;
  GtkWidget *image;
  ...
  if (G_UNLIKELY (self->data_device == device))
    {
      g_autofree gchar *str = NULL;
      gint i;

      i = wwan_model_get_item_index (G_LIST_MODEL (self->devices), device);
      g_assert (i >= 0);
                          ...
                          CcWwanDevice *device)
{
  ...
  g_object_set_data (G_OBJECT (device_page), "index", GINT_TO_POINTER (n_items));
  g_object_set_data (G_OBJECT (device_page), "device", device);
  gtk_stack_add_titled (self->devices_stack,
                        device_page, stack_name, operator_name);
}
                                 ...
                                 CcWwanPanel      *self)
{
  ...
  device = g_object_get_data (G_OBJECT (device_page), "device");
  g_assert (CC_IS_WWAN_DEVICE (device));
  ...
  for (iter = devices; iter; iter = iter->next)
    {
      ...
      device = cc_wwan_device_new (mm_object, self->nm_client);
                            ...
                            GDBusObject *object)
{
  ...
  device = cc_wwan_device_new (MM_OBJECT (object), self->nm_client);
  cc_wwan_panel_add_device (self, device);
  g_signal_connect_object (device, "notify::has-data",
                           G_CALLBACK (cc_wwan_panel_update_data_connections),
                           self, G_CONNECT_SWAPPED);
...
static void
cc_wwan_panel_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (cc_wwan_panel_parent_class)->finalize (object);
}
...
cc_wwan_panel_class_init (CcWwanPanelClass *klass)
{
  ...
  object_class->finalize = cc_wwan_panel_finalize;
...
cc_wwan_panel_init (CcWwanPanel *self)
{
  ...
  gtk_list_box_bind_model (GTK_LIST_BOX (self->data_select_listbox),
                           ...
                           (GtkListBoxCreateWidgetFunc)cc_wwan_panel_new_data_row,
                           self, NULL);
