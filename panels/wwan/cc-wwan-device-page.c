...
wwan_data_show_apn_dialog (CcWwanDevicePage *self)
{
  ...
  g_assert (CC_IS_WWAN_DEVICE_PAGE (self));
                                ...
                                MMModemLock       lock)
{
  ...
  index = GPOINTER_TO_INT (g_object_get_data (G_OBJECT (self), "index"));

  if (lock == MM_MODEM_LOCK_SIM_PIN)
    {
      description = g_strdup_printf (_("Please provide PIN code for SIM %u"), index);
      message = _("Enter PIN to unlock your SIM card");
    }
  else if (lock == MM_MODEM_LOCK_SIM_PUK)
    {
      description = g_strdup_printf (_("Please provide PUK code for SIM %u"), index);
                                 ...
                                 gpointer      user_data)
{
  ...
  CcWwanDevice *device = CC_WWAN_DEVICE (object);
                               ...
                               CcListRow        *data_row)
{
  ...
  g_assert (CC_IS_WWAN_DEVICE_PAGE (self));
  g_assert (CC_IS_LIST_ROW (data_row));
...
static void
cc_wwan_device_page_finalize (GObject *object)
{
  CcWwanDevicePage *self = (CcWwanDevicePage *)object;
  ...
  G_OBJECT_CLASS (cc_wwan_device_page_parent_class)->finalize (object);
}
...
cc_wwan_device_page_class_init (CcWwanDevicePageClass *klass)
{
  ...
  object_class->finalize = cc_wwan_device_page_finalize;
