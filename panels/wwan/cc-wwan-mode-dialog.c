...
G_DEFINE_TYPE (CcWwanModeDialog, cc_wwan_mode_dialog, HDY_TYPE_DIALOG)
                             ...
                             MMModemMode       preferred)
{
  ...
  g_object_set_data (G_OBJECT (row), "allowed", GINT_TO_POINTER (allowed));
  g_object_set_data (G_OBJECT (row), "preferred", GINT_TO_POINTER (preferred));
  ...
  for (i = 0; i < G_N_ELEMENTS (modes); i++)
    {
      ...
      mode_item = cc_wwan_mode_dialog_row_new (self, modes[i][0], modes[i][1]);
      gtk_container_add (GTK_CONTAINER (self->network_mode_list), mode_item);

      /* show the check image if it's the current active item */
      if (modes[i][0] == self->allowed && modes[i][1] == self->preferred)
        {
          gtk_widget_show_all (mode_item);
          self->selected_row = mode_item;
        }
    }
}
...
cc_wwan_mode_dialog_set_property (GObject      *object,
                                  ...
                                  GParamSpec   *pspec)
{
  CcWwanModeDialog *self = (CcWwanModeDialog *)object;
...
static void
cc_wwan_mode_dialog_finalize (GObject *object)
{
  CcWwanModeDialog *self = (CcWwanModeDialog *)object;

  g_clear_object (&self->device);

  G_OBJECT_CLASS (cc_wwan_mode_dialog_parent_class)->finalize (object);
}

static void
cc_wwan_mode_dialog_update_mode (GtkWidget        *widget,
                                 CcWwanModeDialog *self)
{
  ...
  allowed = GPOINTER_TO_INT (g_object_get_data (G_OBJECT (widget), "allowed"));
  preferred = GPOINTER_TO_INT (g_object_get_data (G_OBJECT (widget), "preferred"));
  image = g_object_get_data (G_OBJECT (widget), "image");

  if (self->allowed == allowed && self->preferred == preferred)
    {
      self->selected_row = widget;
      gtk_widget_show (image);
    }
  else
    gtk_widget_hide (image);
...
{
  CcWwanModeDialog *self = (CcWwanModeDialog *)widget;
                              ...
                              gint       response_id)
{
  ...
  row = G_OBJECT (self->selected_row);
  allowed = GPOINTER_TO_INT (g_object_get_data (row, "allowed"));
  preferred = GPOINTER_TO_INT (g_object_get_data (row, "preferred"));

  cc_wwan_device_set_current_mode (self->device, allowed, preferred,
                                   NULL, NULL, NULL);
}
...
cc_wwan_mode_dialog_class_init (CcWwanModeDialogClass *klass)
{
  ...
  object_class->finalize = cc_wwan_mode_dialog_finalize;
