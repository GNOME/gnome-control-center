...
cc_wwan_apn_back_clicked_cb (CcWwanApnDialog *self)
{
  ...
  current = gtk_stack_get_visible_child_name (self->apn_settings_stack);
  g_assert (current != NULL);

  if (g_str_equal (current, "apn-edit"))
    {
      gtk_widget_hide (GTK_WIDGET (self->save_button));
      gtk_widget_show (GTK_WIDGET (self->add_button));
      gtk_stack_set_visible_child_name (self->apn_settings_stack, "apn-list");
    }
  else
...
cc_wwan_apn_add_clicked_cb (CcWwanApnDialog *self)
{
  ...
  g_object_set (self->name_entry, "text", "", NULL);
  g_object_set (self->apn_entry, "text", "", NULL);
  g_object_set (self->username_entry, "text", "", NULL);
  g_object_set (self->password_entry, "text", "", NULL);

  gtk_widget_hide (GTK_WIDGET (self->add_button));
  gtk_widget_show (GTK_WIDGET (self->save_button));
  g_object_set_data (G_OBJECT (self->save_button), "apn", NULL);
  gtk_stack_set_visible_child_name (self->apn_settings_stack, "apn-edit");
}
                          ...
                          GtkListBoxRow   *row)
{
  ...
  radio = g_object_get_data (G_OBJECT (row), "radio");
                        ...
                        GtkWidget       *widget)
{
  ...
  apn = g_object_get_data (G_OBJECT (widget), "apn");
  if (cc_wwan_data_set_default_apn (self->wwan_data, apn))
                             ...
                             GtkButton       *button)
{
  ...
  apn = g_object_get_data (G_OBJECT (button), "apn");
  g_object_set_data (G_OBJECT (self->save_button), "apn", apn);
  gtk_widget_show (GTK_WIDGET (self->save_button));
  gtk_widget_hide (GTK_WIDGET (self->add_button));
  ...
  gtk_entry_set_text (self->apn_entry,
                      cc_wwan_data_apn_get_apn (apn));

  gtk_entry_set_text (self->username_entry,
                      cc_wwan_data_apn_get_username (apn));

  gtk_entry_set_text (self->password_entry,
                      cc_wwan_data_apn_get_password (apn));

  gtk_stack_set_visible_child_name (self->apn_settings_stack,
                                    "apn-edit");
}
...
{
  GtkWidget *row, *grid, *name_label, *apn_label, *radio, *edit_button;
  ...
  g_object_set_data (G_OBJECT (radio), "apn", apn);
  ...
  g_object_set_data (G_OBJECT (edit_button), "apn", apn);
