...
cc_wwan_data_apn_reset (CcWwanDataApn *apn)
{
  ...
  g_assert (CC_IS_WWAN_DATA_APN (apn));
                           ...
                           NMAMobileAccessMethod *access_method)
{
  ...
  g_assert (NM_IS_REMOTE_CONNECTION (remote_connection));
  g_assert (access_method);
              ...
              if (sim_id && *sim_id && g_str_equal (sim_id, self->sim_id))
                {
                  ...
                  ip_setting = nm_connection_get_setting_ip4_config (NM_CONNECTION (apn->remote_connection));
...
static void
cc_wwan_data_finalize (GObject *object)
{
  CcWwanData *self = (CcWwanData *)object;
...
cc_wwan_data_class_init (CcWwanDataClass *klass)
{
  ...
  object_class->finalize = cc_wwan_data_finalize;
...
{
  CcWwanDataApn *apn = (CcWwanDataApn *)object;
...
#define CC_WWAN_DATA_APN_RETURN_FIELD(_apn, nm_val, nma_val) do {       \
    const gchar *val;                                                   \
    if (_apn->remote_connection)                                        \
      {                                                                 \
        NMSettingGsm *setting;                                          \
                                                                        \
        setting = nm_connection_get_setting_gsm (NM_CONNECTION (apn->remote_connection)); \
        val = nm_setting_gsm_get_ ## nm_val (setting);                  \
        if (!val)                                                       \
          val = "";                                                     \
        return val;                                                     \
      }                                                                 \
                                                                        \
    if (_apn->access_method)                                            \
      {                                                                 \
        val = nma_mobile_access_method_get_ ## nma_val (apn->access_method); \
        if (!val)                                                       \
          val = "";                                                     \
        return val;                                                     \
      }                                                                 \
...
cc_wwan_data_apn_get_apn (CcWwanDataApn *apn)
{
  ...
  CC_WWAN_DATA_APN_RETURN_FIELD (apn, apn, 3gpp_apn);
}
...
cc_wwan_data_apn_get_username (CcWwanDataApn *apn)
{
  ...
  CC_WWAN_DATA_APN_RETURN_FIELD (apn, username, username);
}
...
cc_wwan_data_apn_get_password (CcWwanDataApn *apn)
{
  ...
  CC_WWAN_DATA_APN_RETURN_FIELD (apn, password, password);
