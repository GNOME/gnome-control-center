...
add_all_languages (CcLanguageChooser *self)
{
        ...
        for (int i = 0; locale_ids[i] != NULL; i++) {
                ...
                row = cc_regional_language_row_new (locale_ids[i], CC_REGIONAL_LANGUAGE_TYPE_LANGUAGE);
                  ...
                  gpointer   user_data)
{
        ...
        if (!CC_IS_REGIONAL_LANGUAGE_ROW (list_box_row))
                return TRUE;

        row = CC_REGIONAL_LANGUAGE_ROW (list_box_row);
        ...
        language =
                cc_util_normalize_casefold_and_unaccent (cc_regional_language_row_get_language (row));
        visible = match_all (self->filter_words, language);
        if (visible)
                return TRUE;

        country =
                cc_util_normalize_casefold_and_unaccent (cc_regional_language_row_get_country (row));
        visible = match_all (self->filter_words, country);
        if (visible)
                return TRUE;

        language_local =
                cc_util_normalize_casefold_and_unaccent (cc_regional_language_row_get_language_local (row));
        visible = match_all (self->filter_words, language_local);
        if (visible)
                return TRUE;

        country_local =
                cc_util_normalize_casefold_and_unaccent (cc_regional_language_row_get_country_local (row));
...
sort_languages (GtkListBoxRow *a,
                ...
                gpointer   data)
{
        CcRegionalLanguageRow *row1, *row2;
        int d;
        ...
        row1 = CC_REGIONAL_LANGUAGE_ROW (a);
        row2 = CC_REGIONAL_LANGUAGE_ROW (b);

        d = g_strcmp0 (cc_regional_language_row_get_language (row1), cc_regional_language_row_get_language (row2));
        if (d != 0)
                return d;

        return g_strcmp0 (cc_regional_language_row_get_country (row1), cc_regional_language_row_get_country (row2));
}
