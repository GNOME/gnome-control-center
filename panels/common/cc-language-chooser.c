...
static gboolean
language_visible (GtkListBoxRow *list_box_row,
                  gpointer   user_data)
{
        ...
        if (list_box_row == self->more_row)
                return !self->showing_extra;

        if (!CC_IS_LOCALE_ROW (list_box_row))
                return TRUE;

        row = CC_LOCALE_ROW (list_box_row);
        ...
        language =
                cc_util_normalize_casefold_and_unaccent (cc_locale_row_get_language (row));
        visible = match_all (self->filter_words, language);
        if (visible)
                return TRUE;

        country =
                cc_util_normalize_casefold_and_unaccent (cc_locale_row_get_country (row));
        visible = match_all (self->filter_words, country);
        if (visible)
                return TRUE;

        language_local =
                cc_util_normalize_casefold_and_unaccent (cc_locale_row_get_language_local (row));
        visible = match_all (self->filter_words, language_local);
        if (visible)
                return TRUE;

        country_local =
                cc_util_normalize_casefold_and_unaccent (cc_locale_row_get_country_local (row));
                ...
                gpointer   data)
{
        ...
        row1 = CC_LOCALE_ROW (a);
        row2 = CC_LOCALE_ROW (b);

        d = g_strcmp0 (cc_locale_row_get_language (row1), cc_locale_row_get_language (row2));
        if (d != 0)
                return d;

        return g_strcmp0 (cc_locale_row_get_country (row1), cc_locale_row_get_country (row2));
}
...
language_listbox_row_activated_cb (CcLanguageChooser *self, GtkListBoxRow *row)
{
        ...
        if (g_strcmp0 (new_locale_id, self->language) == 0) {
                ...
                g_free (self->language);
                self->language = g_strdup (new_locale_id);
        }
}
