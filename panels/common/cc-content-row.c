...
G_DEFINE_TYPE_WITH_CODE (CcContentRow, cc_content_row, ADW_TYPE_PREFERENCES_ROW,
                         G_ADD_PRIVATE (CcContentRow)
                             G_IMPLEMENT_INTERFACE (GTK_TYPE_BUILDABLE, cc_content_row_buildable_init))
...
static gboolean
string_is_not_empty (CcContentRow *self, const char *string)
{
    return string && string[0];
}

static void
row_activated_cb (CcContentRow *self, GtkListBoxRow *row)
{
...
static void
cc_content_row_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_content_row_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcContentRow *self = CC_CONTENT_ROW (object);
...
cc_content_row_class_init (CcContentRowClass *klass)
{
    ...
    props[PROP_ACTIVATABLE_WIDGET] =
        g_param_spec_object ("activatable-widget", NULL, NULL, GTK_TYPE_WIDGET,
                             G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
     ...
     */
    props[PROP_TITLE_LINES] = g_param_spec_int ("title-lines", NULL, NULL, 0, G_MAXINT, 0,
                                                G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
    ...
    props[PROP_SUBTITLE_LINES] =
        g_param_spec_int ("subtitle-lines", NULL, NULL, 0, G_MAXINT, 0,
                          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
     ...
     * Since: 1.3
     */
    props[PROP_SUBTITLE_SELECTABLE] = g_param_spec_boolean (
        "subtitle-selectable", NULL, NULL, FALSE, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS | G_PARAM_EXPLICIT_NOTIFY);
...
static void
cc_content_row_buildable_add_child (GtkBuildable *buildable, GtkBuilder *builder, GObject *child, const char *type)
{
 ...
 */
void
cc_content_row_add_prefix (CcContentRow *self, GtkWidget *widget)
{
 ...
 */
void
cc_content_row_add_suffix (CcContentRow *self, GtkWidget *widget)
{
    CcContentRowPrivate *priv;
 ...
 */
void
cc_content_row_add_content (CcContentRow *self, GtkWidget *widget)
{
    CcContentRowPrivate *priv;
...
#define ADW_CRITICAL_CANNOT_REMOVE_CHILD(parent, child)                                                                \
    G_STMT_START                                                                                                       \
    {                                                                                                                  \
        g_critical ("%s:%d: tried to remove non-child %p of type '%s' from %p of type '%s'", __FILE__, __LINE__,       \
                    (child), G_OBJECT_TYPE_NAME ((GObject *) (child)), (parent),                                       \
 ...
 */
void
cc_content_row_remove (CcContentRow *self, GtkWidget *child)
{
    CcContentRowPrivate *priv;
    GtkWidget *parent;
 ...
 */
void
cc_content_row_set_subtitle (CcContentRow *self, const char *subtitle)
{
    CcContentRowPrivate *priv;
...
static void
activatable_widget_weak_notify (gpointer data, GObject *where_the_object_was)
{
 ...
 */
void
cc_content_row_set_activatable_widget (CcContentRow *self, GtkWidget *widget)
{
    CcContentRowPrivate *priv;
    ...
    if (priv->activatable_widget) {
        gtk_accessible_reset_relation (GTK_ACCESSIBLE (priv->activatable_widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY);
        gtk_accessible_reset_relation (GTK_ACCESSIBLE (priv->activatable_widget), GTK_ACCESSIBLE_RELATION_DESCRIBED_BY);

        g_object_weak_unref (G_OBJECT (priv->activatable_widget), activatable_widget_weak_notify, self);
    }

    priv->activatable_widget = widget;

    if (priv->activatable_widget != NULL) {
        g_object_weak_ref (G_OBJECT (priv->activatable_widget), activatable_widget_weak_notify, self);

        priv->activatable_binding =
            g_object_bind_property (widget, "sensitive", self, "activatable", G_BINDING_SYNC_CREATE);

        gtk_accessible_update_relation (GTK_ACCESSIBLE (priv->activatable_widget), GTK_ACCESSIBLE_RELATION_LABELLED_BY,
                                        priv->title, NULL, GTK_ACCESSIBLE_RELATION_DESCRIBED_BY, priv->subtitle, NULL,
                                        -1);
    }
 ...
 */
void
cc_content_row_set_title_lines (CcContentRow *self, int title_lines)
{
    CcContentRowPrivate *priv;
 ...
 */
void
cc_content_row_set_subtitle_lines (CcContentRow *self, int subtitle_lines)
{
    CcContentRowPrivate *priv;
 ...
 */
void
cc_content_row_set_subtitle_selectable (CcContentRow *self, gboolean subtitle_selectable)
{
    CcContentRowPrivate *priv = cc_content_row_get_instance_private (self);
