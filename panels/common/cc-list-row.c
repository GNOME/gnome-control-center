...
static void
cc_list_row_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
cc_list_row_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    CcListRow *self = (CcListRow *) object;
...
cc_list_row_class_init (CcListRowClass *klass)
{
    ...
    properties[PROP_SECONDARY_LABEL] = g_param_spec_string ("secondary-label", "Secondary Label", "Set Secondary Label",
                                                            NULL, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    properties[PROP_SHOW_ARROW] =
        g_param_spec_boolean ("show-arrow", "Show Arrow", "Whether to show an arrow at the end of the row", FALSE,
...
add_secondary_label_to_a11y_description (CcListRow *self)
{
    ...
    gtk_accessible_update_relation (GTK_ACCESSIBLE (self), GTK_ACCESSIBLE_RELATION_DESCRIBED_BY, subtitle_label,
                                    self->secondary_label, NULL, -1);
}
...
void
cc_list_row_set_show_arrow (CcListRow *self, gboolean show_arrow)
{
    ...
    if (show_arrow)
        g_object_set (G_OBJECT (self), "accessible-role", GTK_ACCESSIBLE_ROLE_BUTTON, NULL);
    g_object_notify_by_pspec (G_OBJECT (self), properties[PROP_SHOW_ARROW]);
}

void
cc_list_row_set_secondary_label (CcListRow *self, const gchar *label)
{
    g_return_if_fail (CC_IS_LIST_ROW (self));
...
void
cc_list_row_set_secondary_markup (CcListRow *self, const gchar *markup)
{
    g_return_if_fail (CC_IS_LIST_ROW (self));
