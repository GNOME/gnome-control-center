...
const gchar *udev_ids[] = {
    "ID_INPUT_MOUSE",       "ID_INPUT_KEYBOARD",   "ID_INPUT_TOUCHPAD",      "ID_INPUT_TABLET",
    "ID_INPUT_TOUCHSCREEN", "ID_INPUT_TABLET_PAD", "ID_INPUT_POINTINGSTICK", /* udev sets ID_INPUT_MOUSE too */
};
...
static void
gsd_device_set_property (GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
...
static void
gsd_device_get_property (GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
    GsdDevicePrivate *priv;
...
gsd_device_class_init (GsdDeviceClass *klass)
{
    ...
    properties[PROP_NAME] =
        g_param_spec_string ("name", "Name", "Name", NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_DEVICE_FILE] = g_param_spec_string ("device-file", "Device file", "Device file", NULL,
                                                        G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_VENDOR_ID] =
        g_param_spec_string ("vendor-id", "Vendor ID", "Vendor ID", NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_PRODUCT_ID] = g_param_spec_string ("product-id", "Product ID", "Product ID", NULL,
                                                       G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_TYPE] = g_param_spec_flags ("type", "Device type", "Device type", GSD_TYPE_DEVICE_TYPE, 0,
                                                G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_WIDTH] =
        g_param_spec_uint ("width", "Width", "Width", 0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_HEIGHT] =
        g_param_spec_uint ("height", "Height", "Height", 0, G_MAXUINT, 0, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
    properties[PROP_GROUP] =
        g_param_spec_string ("group", "Group", "Group", NULL, G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY);
...
static GList *
gsd_device_manager_real_list_devices (GsdDeviceManager *manager, GsdDeviceType type)
{
    GsdDeviceManagerPrivate *priv = gsd_device_manager_get_instance_private (manager);
...
static GsdDevice *
gsd_device_manager_real_lookup_device (GsdDeviceManager *manager, GdkDevice *gdk_device)
{
    GsdDeviceManagerPrivate *priv = gsd_device_manager_get_instance_private (manager);
    ...
    while (g_hash_table_iter_next (&iter, NULL, (gpointer *) &device)) {
        if (g_strcmp0 (node_path, gsd_device_get_device_file (device)) == 0) {
            return device;
        }
    }
...
gsd_device_manager_class_init (GsdDeviceManagerClass *klass)
{
    ...
    signals[DEVICE_ADDED] = g_signal_new ("device-added", GSD_TYPE_DEVICE_MANAGER, G_SIGNAL_RUN_LAST,
                                          G_STRUCT_OFFSET (GsdDeviceManagerClass, device_added), NULL, NULL, NULL,
                                          G_TYPE_NONE, 1, GSD_TYPE_DEVICE | G_SIGNAL_TYPE_STATIC_SCOPE);

    signals[DEVICE_REMOVED] = g_signal_new ("device-removed", GSD_TYPE_DEVICE_MANAGER, G_SIGNAL_RUN_LAST,
                                            G_STRUCT_OFFSET (GsdDeviceManagerClass, device_removed), NULL, NULL, NULL,
                                            G_TYPE_NONE, 1, GSD_TYPE_DEVICE | G_SIGNAL_TYPE_STATIC_SCOPE);

    signals[DEVICE_CHANGED] = g_signal_new ("device-changed", GSD_TYPE_DEVICE_MANAGER, G_SIGNAL_RUN_LAST,
                                            G_STRUCT_OFFSET (GsdDeviceManagerClass, device_changed), NULL, NULL, NULL,
                                            G_TYPE_NONE, 1, GSD_TYPE_DEVICE | G_SIGNAL_TYPE_STATIC_SCOPE);
}
...
create_device (GUdevDevice *udev_device, GUdevDevice *parent)
{
    ...
    return g_object_new (GSD_TYPE_DEVICE, "name", name, "device-file", g_udev_device_get_device_file (udev_device),
                         "type", udev_device_get_device_type (udev_device), "vendor-id", vendor, "product-id", product,
                         "width", width, "height", height, "group", group, NULL);
}

static void
add_device (GsdDeviceManager *manager, GUdevDevice *udev_device)
{
    GsdDeviceManagerPrivate *priv = gsd_device_manager_get_instance_private (manager);
...
static void
remove_device (GsdDeviceManager *manager, GUdevDevice *udev_device)
{
    GsdDeviceManagerPrivate *priv = gsd_device_manager_get_instance_private (manager);
    GsdDevice *device;
    const gchar *syspath;
...
static void
udev_event_cb (GUdevClient *client, gchar *action, GUdevDevice *device, GsdDeviceManager *manager)
{
...
gsd_device_manager_init (GsdDeviceManager *manager)
{
    ...
    g_signal_connect (priv->udev_client, "uevent", G_CALLBACK (udev_event_cb), manager);
...
gsd_device_manager_get (void)
{
    ...
    if (!manager) {
        ...
        g_object_set_data_full (G_OBJECT (display), "gsd-device-manager-data", manager,
...
GList *
gsd_device_manager_list_devices (GsdDeviceManager *manager, GsdDeviceType type)
{
...
void
gsd_device_get_device_ids (GsdDevice *device, const gchar **vendor, const gchar **product)
{
    GsdDevicePrivate *priv;
...
gsd_device_get_settings (GsdDevice *device)
{
    ...
    if (type & (GSD_DEVICE_TYPE_TOUCHSCREEN | GSD_DEVICE_TYPE_TABLET)) {
        ...
        if (type & GSD_DEVICE_TYPE_TOUCHSCREEN) {
            ...
            path = g_strdup_printf ("/org/gnome/desktop/peripherals/touchscreens/%s:%s/", vendor, product);
        } else if (type & GSD_DEVICE_TYPE_TABLET) {
            schema = "org.gnome.desktop.peripherals.tablet";
            path = g_strdup_printf ("/org/gnome/desktop/peripherals/tablets/%s:%s/", vendor, product);
        }
...
gboolean
gsd_device_get_dimensions (GsdDevice *device, guint *width, guint *height)
{
    GsdDevicePrivate *priv;
...
GsdDevice *
gsd_device_manager_lookup_gdk_device (GsdDeviceManager *manager, GdkDevice *gdk_device)
{
...
gboolean
gsd_device_shares_group (GsdDevice *device1, GsdDevice *device2)
{
