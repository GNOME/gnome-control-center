               ...
               CcWindow    *self)
{
  ...
  /* Display the panel only if it wasn't activated by an event causing
   * navigation between lists, otherwise let the new list be displayed.
   */
  if (!self->expect_list_change)
    hdy_leaflet_set_visible_child_name (HDY_LEAFLET (self->main_leaflet), "panel");
}
...
static void
show_nested_list_cb (CcPanelList *panel_list,
                     CcWindow    *self)
{
  /* Navigation into a nested list will cause the activation of a panel, this
   * flags this expectation so we can display the entered list instead of the
   * activated panel.
   */
  self->expect_list_change = TRUE;
}
                            ...
                            CcWindow  *shell)
{
  ...
  /* Navigation back to a previous list will cause the activation of a panel,
   * this flags this expectation so we can display the entered list instead of
   * the activated panel.
   */
  shell->expect_list_change = TRUE;
...
cc_window_class_init (CcWindowClass *klass)
{
  ...
  gtk_widget_class_bind_template_callback (widget_class, show_nested_list_cb);
