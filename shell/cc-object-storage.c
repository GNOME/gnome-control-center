...
/* Singleton instance */
CcObjectStorage *cc_object_storage_instance = NULL;

static CcObjectStorage*
cc_object_storage_get_default (void)
{
  if (g_once_init_enter (&cc_object_storage_instance))
    {
      CcObjectStorage *instance = g_object_new (CC_TYPE_OBJECT_STORAGE, NULL);

      g_debug ("Initializing object storage");

      g_once_init_leave (&cc_object_storage_instance, instance);
    }

  return cc_object_storage_instance;
}
...
{
  CcObjectStorage *self = cc_object_storage_get_default ();

  g_return_val_if_fail (CC_IS_OBJECT_STORAGE (self), FALSE);
  g_return_val_if_fail (key != NULL, FALSE);

  return g_hash_table_contains (self->id_to_object, key);
}
                              ...
                              gpointer     object)
{
  ...
  g_return_if_fail (CC_IS_OBJECT_STORAGE (self));
  ...
  g_assert (!g_hash_table_contains (self->id_to_object, key));
  ...
  g_hash_table_insert (self->id_to_object, g_strdup (key), g_object_ref (object));
}
...
cc_object_storage_get_object (const gchar *key)
{
  ...
  return g_object_ref (g_hash_table_lookup (self->id_to_object, key));
}
                                          ...
                                          GError          **error)
{
  ...
  self = cc_object_storage_get_default ();
   ...
   */
  if (g_hash_table_contains (self->id_to_object, key))
                                     ...
                                     gpointer             user_data)
{
  ...
  g_task_set_task_data (task,
                        task_data_new (bus_type, flags, name, path, interface),
                                            ...
                                            GError       **error)
{
  ...
  /* It is a programming error to not cancel the D-Bus proxy creation when finalizing a panel.
   * We must never create the same proxy synchronously while the asynchronous function is running.
   */
 ...
 */
void
cc_object_storage_dispose (void)
{
  g_assert (cc_object_storage_instance != NULL);

  g_clear_object (&cc_object_storage_instance);
}
