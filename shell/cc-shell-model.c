...
cc_shell_model_init (CcShellModel *self)
{
  GType types[] = {G_TYPE_STRING, G_TYPE_STRING, G_TYPE_APP_INFO, G_TYPE_STRING, G_TYPE_UINT,
                   G_TYPE_STRING, G_TYPE_STRING, G_TYPE_ICON, G_TYPE_STRV, G_TYPE_BOOLEAN };
...
void
cc_shell_model_set_panel_visible (CcShellModel *self,
                                  const gchar  *id,
                                  gboolean      visible)
{
  GtkTreeModel *model;
  ...
  gtk_list_store_set (GTK_LIST_STORE (self), &iter, COL_VISIBLE, visible, -1);
}
