                  ...
                  CcPanel      *panel)
{
  ...
  if (panel)
    gtk_box_pack_end (GTK_BOX (shell->main_box), GTK_WIDGET (panel), TRUE, TRUE, 0);
}
...
static void
cc_test_window_finalize (GObject *object)
{
  G_OBJECT_CLASS (cc_test_window_parent_class)->finalize (object);
}
...
cc_test_window_class_init (CcTestWindowClass *klass)
{
  ...
  object_class->finalize = cc_test_window_finalize;
